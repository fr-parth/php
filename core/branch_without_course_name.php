...
$query = mysql_query("SELECT id, branch_id,Branch_code,branch_Name,Course_Name FROM tbl_branch_master where (Course_Name ='' or Course_Name is null) AND school_id = '$school_id' LIMIT $start_from, $limit") or die("could not Search!");

$sql1 = "SELECT id FROM  tbl_branch_master where (Course_Name ='' or Course_Name is null) AND school_id = '$school_id' ";
...
$rs_result = mysql_query($sql1);  
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
        ...
        if ($colname != ''and $colname != 'Select')
				{
            		$query1=mysql_query(" SELECT id ,branch_id,Branch_code,branch_Name,Course_Name FROM tbl_branch_master where (Course_Name ='' or Course_Name is null) AND school_id = '$school_id'
									AND `$colname` LIKE '%$searchq%' LIMIT $start_from, $limit") or die("could not Search!");

				$sql1 =mysql_query("SELECT id FROM  tbl_branch_master where (Course_Name ='' or Course_Name is null) AND school_id = '$school_id'AND `$colname` LIKE '%$searchq%' ") or die("could not Search!");
				...
				$sql1 =mysql_query("SELECT id FROM tbl_branch_master  where (Course_Name ='' or Course_Name is null) AND school_id = '$school_id' 
									AND (Course_Name LIKE '$searchq%' or branch_id LIKE '$searchq%' or Branch_code LIKE '%$searchq%' 
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('branch_without_course_name.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('branch_without_course_name.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<!-- Headline -->
	<div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white">Branch Data without Course Name  </h2>
			</div>
            ...
            <div class="col-md-2" style="width:17%;">
                <select name="colname" class="form-control">
					...
					<?php if (($_GET['colname']) == "Branch_code") {
                            echo $_GET['colname']; ?> selected="selected" <?php } ?>>Branch Code</option>  
