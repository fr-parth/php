...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT Semester_Id ,  Semester_Name , class ,CourseLevel
						FROM tbl_semester_master 
						WHERE (CourseLevel  ='' or CourseLevel is null ) and School_ID='$school_id'
						LIMIT $start_from, $limit") or die("could not Search!");
 
	$sql1=" SELECT Semester_Id,  Semester_Name, class ,CourseLevel
			FROM tbl_semester_master 
			WHERE (CourseLevel  ='' or CourseLevel is null ) and School_ID='$school_id' ";
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
		...
		if ($colname != ''and $colname != 'Select')
				{
			$query1=mysql_query("SELECT Semester_Id,  Semester_Name, class ,CourseLevel 
								FROM tbl_semester_master 
								WHERE (CourseLevel  ='' or CourseLevel is null ) and School_ID='$school_id'
									AND `$colname` LIKE '%$searchq%' LIMIT $start_from, $limit") or die("could not Search!");

				
			$sql1=mysql_query("SELECT Semester_Id,  Semester_Name, class ,CourseLevel
								FROM tbl_semester_master 
				...
				$query1=mysql_query("SELECT Semester_Id ,  Semester_Name , class,CourseLevel
									FROM tbl_semester_master 
									WHERE (CourseLevel  ='' or CourseLevel is null ) and School_ID='$school_id' 
									AND (CourseLevel LIKE '$searchq%' or Semester_Name LIKE '$searchq%' or Semester_Id LIKE '$searchq%'or class LIKE '$searchq%' ) LIMIT $start_from, $limit") or die("could not Search!");
			
				$sql1 =mysql_query("SELECT Semester_Id 
									FROM tbl_semester_master
                					WHERE (CourseLevel  ='' or CourseLevel is null )
               					 	AND School_ID='$school_id' 
									AND (CourseLevel LIKE '$searchq%' or Semester_Name LIKE '$searchq%' or Semester_Id LIKE '$searchq%' or class LIKE '$searchq%')") or die("could not Search!");   
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('sem_without_courselevel.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('sem_without_courselevel.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<!-- Headline -->
        <div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white"><?php echo $dynamic_semester;?> Data without<?php echo $dynamic_course_level;?></h2>
			</div>
...
<form>
	...
	<div class="col-md-1">
	<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('sem_without_courselevel.php','_self')" />
	</div>	
</form>
<!-- search form END -->	
