...
<head>
    ...
    <title>ACR Activity Given</title>
...
FROM tbl_teacher  t
 where t.school_id='$school_id' and t.t_academic_year='$yr' and t.t_dept='$dpt' and t.t_emp_type_pid ='137' and t.t_id IN
 (
select ft.tID
...
elseif(($yr!='ALL' and $yr!='') and ($dpt=='' or $dpt=='ALL')){
  $sql3=mysql_query("SELECT distinct t.t_id,t.t_complete_name,t.t_dept ,t.school_id,t.t_academic_year
  FROM tbl_teacher  t
   where t.school_id='$school_id' and t.t_academic_year='$yr'  and t.t_emp_type_pid ='137' and t.t_id  IN
   (
  select ft.tID
  from tbl_360_activities_data ft where ft.activity_level_id='4' and  ft.tID is not NULL and  ft.schoolID='$school_id' )
  group by t.t_id");
  ...
  elseif(($yr=='ALL' or $yr=='') and ($dpt!='' and $dpt!='ALL')){
    $sql3=mysql_query("SELECT distinct t.t_id,t.t_complete_name,t.t_dept ,t.school_id,t.t_academic_year
    FROM tbl_teacher  t
     where t.school_id='$school_id' and  t.t_dept='$dpt' and t.t_emp_type_pid ='137' and t.t_id  IN
     (
    select ft.tID
    from tbl_360_activities_data ft where ft.activity_level_id='4' and  ft.tID is not NULL and  ft.schoolID='$school_id' )
    group by t.t_id");
  ...
  $sql3=mysql_query("SELECT distinct t.t_id,t.t_complete_name,t.t_dept ,t.school_id,t.t_academic_year
  FROM tbl_teacher  t
   where t.school_id='$school_id'  and t.t_emp_type_pid ='137' and t.t_id IN
   (
  select ft.tID
  from tbl_360_activities_data ft where ft.activity_level_id='4' and  ft.tID is not NULL and  ft.schoolID='$school_id' )
  group by t.t_id");
...
elseif (($yr!='' and $yr!='ALL') and ($dpt=='' or $dpt=='ALL')) {
  ...
  from tbl_360_activities_data ft where ft.activity_level_id='4' and  ft.tID is not NULL and  ft.schoolID='$school_id' )
   ...
   t.t_dept LIKE '%$searchq%'or
   t.t_id LIKE '%$searchq%'or
   t.t_academic_year LIKE '%$searchq%')") or die("could not Search!");
}
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
           ...
           console.log(page);
      			window.location.assign('acr360_given.php?yr=<?php echo $yr."&dpt=".$dpt; ?>&page='+page);
...
<script type="text/javascript">
    $(function () {
     ...
     window.location.assign('acr360_given.php?yr=<?php echo $yr."&dpt=".$dpt; ?>&Search=<?php echo $searchq; ?>&spage='+page);
         <?php } else{ ?>
     window.location.assign('acr360_given.php?Search=<?php echo $searchq; ?>&spage='+page);
         <?php } ?>
        });
    });
</script>
...
<div align="center">
    <div class="container" style="width:100%;">
       ...
       <input type='hidden' name='dpt' value='<?php echo $_GET['dpt']?>'> </input>
			...
			<div class="col-md-1" >
			<input type="button" class="btn btn-info" value="Reset" onclick="window.open('acr360_given.php?yr=<?php echo $yr; ?>&dpt=<?php echo $dpt; ?>','_self')" />
			</div>
