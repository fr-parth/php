...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT semester.student_id , semester.Semesterid, semester.BranchName, semester.DeptName ,semester.SemesterName, semester.ExtCourseLevelID FROM  StudentSemesterRecord  semester 
...
and semester.`IsCurrentSemester`='1' 
   ...
   and(semester.ExtCourseLevelID  ='' or semester.ExtCourseLevelID is null ) 
	...
	$sql1=" SELECT id FROM StudentSemesterRecord 
			where(ExtCourseLevelID  ='' or ExtCourseLevelID is null ) and school_id='$school_id'  and AcdemicYear='$Academic_Year' ";
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
		...
		if ($colname != ''and $colname != 'Select')
				{
					echo "a6";	echo	$query1=mysql_query("SELECT semester.student_id , semester.Semesterid, semester.BranchName, semester.DeptName ,semester.SemesterName, semester.ExtCourseLevelID FROM  StudentSemesterRecord  semester 
			JOIN tbl_student std ON std.std_PRN = semester.student_id 
		  ...
		  and AcdemicYear='$Academic_Year'
		   and(semester.ExtCourseLevelID  ='' or semester.ExtCourseLevelID is null ) 
				...
				$sql1 =mysql_query("SELECT id FROM StudentSemesterRecord 
									WHERE  (ExtCourseLevelID ='' or ExtCourseLevelID  is null ) 
									AND school_id='$school_id' 
									AND `$colname` LIKE '%$searchq%' ") or die("could not Search!");
				$total_records = mysql_num_rows($sql1);   
				$total_pages = ceil($total_records / $limit);
		}else{	
				$query1=mysql_query("SELECT semester.student_id , semester.Semesterid, semester.BranchName, semester.DeptName ,semester.SemesterName, semester.ExtCourseLevelID FROM  StudentSemesterRecord  semester 
				JOIN tbl_student std ON std.std_PRN = semester.student_id 
			...
			and semester.`IsCurrentSemester`='1' 
			    and AcdemicYear='$Academic_Year'
			   and(semester.ExtCourseLevelID  ='' or semester.ExtCourseLevelID is null ) 
									AND (student_id LIKE '$searchq%' or Semesterid LIKE '$searchq%' or BranchName LIKE '$searchq%' or DeptName LIKE '%$searchq%'or SemesterName LIKE '$searchq%'or ExtCourseLevelID LIKE '$searchq%'  ) LIMIT $start_from, $limit") or die("could not Search!");
			
				$sql1 =mysql_query("SELECT id FROM StudentSemesterRecord 
                WHERE  (ExtCourseLevelID ='' or ExtCourseLevelID  is null ) 
                AND school_id='$school_id'   and AcdemicYear='$Academic_Year'
									AND (student_id LIKE '$searchq%' or Semesterid LIKE '$searchq%' or BranchName LIKE '$searchq%' or DeptName LIKE '%$searchq%'or SemesterName LIKE '$searchq%'or ExtCourseLevelID LIKE '$searchq%' )") or die("could not Search!");   
...
//$result = mysql_query("SELECT student_id , Semesterid, BranchName, DeptName ,SemesterName, ExtCourseLevelID FROM StudentSemesterRecord where(ExtCourseLevelID  ='' or ExtCourseLevelID is null ) and School_ID='$school_id'");
?>
<html>
<head>
<style>
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('studsem_without_courselevelid.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('studsem_without_courselevelid.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<div style="padding-top:30px" align="center" > 
                <h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white"> Student Semester Data without Courselevel Id(<?php echo $total_records; ?>)</h2>
			</div>
...
<!--End of dropdown ----------------------- --- -->
	...
	<div class="col-md-1">
	<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('studsem_without_courselevelid.php','_self')" />
	</div>	
</form>
<!-- search form END -->	
