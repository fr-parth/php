...
if(isset($_GET['id']))
{
     ...
     $table="tbl_school_adminstaff";
     	$query="select * from ".$table." where id='$id'";       // uploaded by
	$row1=mysql_query($query);
	$value1=mysql_fetch_array($row1);
	$uploaded_by=$value1['stf_name'];
}
				...
				if ( isset($_FILES["file"]))
					{
						...
						if ($_FILES["file"]["error"] > 0) 
						{
							...
							//$errorreport="file is successfully uploaded";
						}
	...
	{	
		...
		for($i=2;$i<=$min;$i++)
		{
			...
			{ 
				...
				$arr[$i]["F"]=str_replace($replace1,"", trim($allDataInSheet[$i]["F"]));    // Year
                ...
                $semester_id=explode(" ",$s_cls);
                $sem_id="Sem"." ".$semester_id[1];
									...
									$reports="Inserted data from Excel Sheet is not valid data";
										 ...
										 { 
											 ...
											 }else
											 {
												...
												{
													$insert_studsem="INSERT INTO `StudentSemesterRecord` (student_id,school_id,BranchName,AcdemicYear,CourseLevel,DeptName,Specialization,Semesterid)
													VALUES('$s_reg_no','$school_id','".$arr[$i]["E"]."','".$arr[$i]["W"]."','".$arr[$i]["V"]."','".$arr[$i]["X"]."','".$arr[$i]["U"]."','$sem_id')";
												...
												}else
													{
														 ...
														 BranchName='".$arr[$i]["E"]."',AcdemicYear='".$arr[$i]["W"]."',CourseLevel='".$arr[$i]["V"]."',DeptName='".$arr[$i]["X"]."',Specialization='".$arr[$i]["U"]."',Semesterid='$sem_id'
