...
$query = mysql_query("SELECT subject_id,Subject_Code,subject,Subject_type,subject_credit FROM tbl_school_subject  where (Subject_Code ='' or Subject_Code is null) AND school_id = '$school_id' LIMIT $start_from, $limit") or die("could not Search!");

$sql1 = "SELECT id FROM  tbl_school_subject where (Subject_Code ='' or Subject_Code is null) AND school_id = '$school_id' ";
...
$rs_result = mysql_query($sql1);  
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
        ...
        if ($colname != ''and $colname != 'Select')
				{
            		$query1=mysql_query(" SELECT subject_id,Subject_Code,subject,Subject_type,subject_credit FROM tbl_school_subject where (Subject_Code ='' or Subject_Code is null) AND school_id = '$school_id'
				...
				$sql1 =mysql_query("SELECT id FROM tbl_school_subject  where (Subject_Code ='' or Subject_Code is null) AND school_id = '$school_id' 
									AND (Subject_Code LIKE '$searchq%' or subject_id LIKE '$searchq%' or subject LIKE '%$searchq%' 
								or Subject_type LIKE '%$searchq%'
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('subject_without_subject_code.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('subject_without_subject_code.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<!-- Headline -->
	<div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white"><?php echo $dynamic_subjects; ?> Data without <?php echo $dynamic_subjects; ?> Code  </h2>
			</div>
            ...
            <div class="col-md-2" style="width:17%;">
                <select name="colname" class="form-control">
					...
					<?php if (($_GET['colname']) == "subject") {
                            echo $_GET['colname']; ?> selected="selected" <?php } ?>><?php echo $dynamic_subjects; ?></option>  

                    <option value="Subject_type"
					<?php if (($_GET['colname']) == "Subject_type") {
        ...
        <div class="col-md-2" style="width:17%;">
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('subject_without_subject_code.php','_self')" />
			</div>	
        </form>
		<!-- search form END -->	
