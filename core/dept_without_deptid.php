...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT  id,Dept_code,Dept_Name,ExtDeptId FROM tbl_department_master where(ExtDeptId =''OR ExtDeptId IS NULL) AND School_ID='$school_id' 
						LIMIT $start_from, $limit") or die("could not Search!");
 
	$sql1=" SELECT id FROM tbl_department_master
			where( ExtDeptId ='' or ExtDeptId is null ) and School_ID='$school_id' ";
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
		...
		if ($colname != ''and $colname != 'Select')
				{
			$query1=mysql_query("SELECT  id,Dept_code,Dept_Name,ExtDeptId FROM tbl_department_master where(ExtDeptId =''OR ExtDeptId IS NULL) AND School_ID='$school_id'
				...
				$sql1 =mysql_query("SELECT id FROM tbl_department_master
									WHERE  (ExtDeptId='' or ExtDeptId is null ) 
				...
				$query1=mysql_query("SELECT  id,Dept_code,Dept_Name,ExtDeptId FROM tbl_department_master where(ExtDeptId =''OR ExtDeptId IS NULL) AND School_ID='$school_id'
									...
									AND (id LIKE '$searchq%' or Dept_code LIKE '$searchq%' or Dept_Name LIKE '$searchq%'  or ExtDeptId  LIKE '$searchq%') LIMIT $start_from, $limit") or die("could not Search!");
			
				$sql1 =mysql_query("SELECT id FROM tbl_department_master
                					WHERE  (ExtDeptId='' or ExtDeptId is null ) 
               					 	AND School_ID='$school_id' 
									AND (id LIKE '$searchq%' or Dept_code LIKE '$searchq%' or Dept_Name LIKE '$searchq%' or ExtDeptId  LIKE '$searchq%' )") or die("could not Search!");   
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('dept_without_deptid.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('dept_without_deptid.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<!-- Headline -->
        <div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white">Department Data  without Department Id</h2>
			</div>
...
<form>
	...
	<div class="col-md-1">
	<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('dept_without_deptid.php','_self')" />
	</div>	
</form>
<!-- search form END -->	
				...
				<tbody>
					 ...
					 ?>
						<tr>
							...
							<td data-title="ExtDeptId"><?php echo $row['ExtDeptId']; ?></td>
