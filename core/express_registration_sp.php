...
function checkIfUserExist($user_type, $email, $phone,$schoolID)
{
    ...
    switch ($user_type) {
        case 'student':
            ...
            $q1 = "select count(1) as exist from tbl_student where school_id='$schoolID' AND std_phone='$phone' AND (std_phone!='' OR std_phone!=NULL)"; 
            break;
        case 'teacher':
        ...
        case 'sponsor':
            ...
            $q1 = "select count(1) as exist from tbl_sponsorer where sp_phone='$phone' AND (sp_phone !='' OR sp_phone != NULL)";
            break;
    }
...
//End SMC-3592
	...
	//if($_SESSION['error_mail']==''and $_SESSION['error_mob']=='')
		if(!$isExist)
  {
	switch ($user_type) {
        case 'student':
	  ...
	  echo "<script>window.alert('Email/Mobile already exist')</script>";
  }
...
function emailUser($user_type, $email, $password,$id,$name,$member_id,$school_id,$phone)
{
	...
	if($user_type == 'student'){
		...
		$msgid = 5;
		$parameter = "email=$email&msgid=$msgid&site=$site&pass=$password&platform_source=$platform_source&studname=$name1&school_id=$school_id&member_id=$member_id&std_phone=$phone&PRN=$id";
	}
	else if($user_type == 'teacher'){
		// $msgid='welcometeacherthroughquickregitrationws';
		$msgid = 6;
		$parameter = "email=$email&msgid=$msgid&site=$site&pass=$password&platform_source=$platform_source&techname=$name1&school_id=$school_id&member_id=$member_id&t_phone=$phone&PRN=$id";	
	}
	else if($user_type == 'sponsor'){
		// $msgid='welcomesponsor';
		$msgid = 1;
		$parameter = "email=$email&msgid=$msgid&site=$site&pass=$password&platform_source=$platform_source";
	}	
	...
	$res = post_function($url,$parameter); //function defined in securityfunctions.php
	//Changes end here
...
if (isset($_POST['submit'])) {
	...
	//if(empty($_SESSION['captcha_code'] ) || strcasecmp($_SESSION['captcha_code'], $_POST['captcha_code']) != 0 && $user_type!='student'){ 
...
//validations for college added by Pranali for SMC-4676 on 16-4-20
	if (empty($user_type) or empty($name) or empty($cc)) {
	  ...
	  if($schoolID=='')
	  {
        ...
        // if ($report == "" && $_SESSION['error_mail']=='' &&  $_SESSION['error_mob']=='') {
         if ($report == "") {
			...
			// $pp = explode('\'s',$firstname);
			// $firstname1 = $pp[0];
			$firstname = str_replace("\'s","_",$firstname); 
            ...
            if ($add!=0 || $add!="") {
				...
				else if($user_type=='sponsor')
				{
					...
					//$check_email=mysql_num_row($row_sponser);
					//if($check_email>=1){
                ...
                <div class='panel-body'>
				 ...
				 <?php //print_r($a); ?>
                            ...
                            <tr>
                                ...
                                <td><input type='text' id='name' name='name' placeholder="Enter Name" style='width:200px;text-transform: capitalize;'  value='<?php if($report1!=""){ echo ""; } else { echo $name; } ?>' onfocus="resetReport()" autocomplete="off"/>&nbsp;&nbsp;&nbsp;<span class="red" id="fname" style="font-size:16px"></span></td>
                            </tr>
                            <tr>
                            	<!--error message in span added by Pranali for SMC-4689 on 5-5-20-->
                                <td>Email Address<?php if($email==''){ ?><span class='red'>*</span><?php } ?></td>
                                <td><?php if($email!=''){ ?><input autocomplete="off" type='email' id='email1' name='email' placeholder="Enter Email ID" style='width:200px'  value='<?php if($report1!=""){ echo ""; } else { echo $email; 
                                } ?>' onfocus="resetReport()" readonly />&emsp;
                                <span class='red'><?php if(isset($_SESSION['error_mail'])) echo $_SESSION['error_mail'];?></span><?php }
                                else{ ?>
                                	<input autocomplete="off" type='email' id='email' name='email' placeholder="Enter Email ID" style='width:200px'  value='<?php if($report1!=""){ echo ""; } else { echo $email; 
							...
							<!--Label changed to Mobile Number by Pranali -->
                                <td>Mobile Number <?php if($phone==''){ ?><span class='red'>*</span><?php } ?></td>
                                <td>
                                    ...
                                    <?php if($phone!=''){ ?>
                                    <input type='text' style='width:150px' id='phone1' name='phone' onKeyPress="return isNumberKey(event)" placeholder="Enter Mobile Number" maxlength="10"    value='<?php if($report1!=""){ echo ""; } else { echo $phone; } ?>' onfocus="resetReport()" autocomplete="off" readonly />
							...
							<tr id="a6"  style="display:table-row;">
                                ...
                                <td>
									<!--<input type='text' id='v_category' name='v_category' placeholder="Enter v_category" style='width:200px'  value='<?php //echo $category; ?>'/>-->
										...
										<?php
										 $cat1rowcount = mysql_num_rows($cat1);
										  for($i=1; $i<=$cat1rowcount; $i++){
											$data=mysql_fetch_array($cat1)
										  ?>
										  <option value="<?php echo $data['id']; ?>" <?php if(isset($_POST['v_category'])){ if($_POST['v_category']== $data['id']){ echo "selected"; }} ?> ><?php echo $data['category']; ?>
										</option>						
