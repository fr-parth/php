...
if(!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT semester.student_id , semester.Semesterid, semester.BranchName, semester.DeptName ,semester.SemesterName FROM StudentSemesterRecord  semester 
	...
	$sql1=" SELECT id FROM StudentSemesterRecord 
			where(SemesterName ='' or SemesterName is null ) and school_id='$school_id' and AcdemicYear='$Academic_Year' ";
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
		...
		if ($colname != ''and $colname != 'Select')
				{
			$query1=mysql_query(" SELECT semester.student_id , semester.Semesterid, semester.BranchName, semester.DeptName ,semester.SemesterName FROM StudentSemesterRecord  semester 
			JOIN tbl_student std ON std.std_PRN = semester.student_id 
	...
	and (semester.SemesterName  ='' or semester.SemesterName  is null ) 
				...
				$sql1 =mysql_query("SELECT id FROM StudentSemesterRecord 
									WHERE  (SemesterName ='' or SemesterName is null ) 
		 ...
		 WHERE semester.school_id='$school_id' and AcdemicYear='$Academic_Year'
		and std.school_id='$school_id' 
		/*and semester.`IsCurrentSemester`='1'*/ 
		  
		  ' 
		and (semester.SemesterName  ='' or semester.SemesterName  is null ) 
									AND (student_id LIKE '$searchq%' or Semesterid LIKE '$searchq%' or BranchName LIKE '$searchq%' or DeptName LIKE '%$searchq%'or SemesterName LIKE '$searchq%'  ) LIMIT $start_from, $limit") or die("could not Search!");
			
				$sql1 =mysql_query("SELECT id FROM StudentSemesterRecord 
                WHERE  (SemesterName='' or SemesterNameis null ) 
                AND school_id='$school_id' and AcdemicYear='$Academic_Year'
									AND (student_id LIKE '$searchq%' or Semesterid LIKE '$searchq%' or BranchName LIKE '$searchq%' or DeptName LIKE '%$searchq%'or SemesterName LIKE '$searchq%' )") or die("could not Search!");   
...
//$result = mysql_query("SELECT student_id , Semesterid, BranchName, DeptName ,SemesterName FROM StudentSemesterRecord where(SemesterName   ='' or SemesterName is null ) and School_ID='$school_id'");
?>
<html>
<head>
<style>
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('studsem_without_semname.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('studsem_without_semname.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white">Student Semester Data without Semester Name (<?php echo $total_records; ?>)</h2>
			</div>
...
<!--End of dropdown ----------------------- --- -->
		...
		<form>
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('studsem_without_semname.php','_self')" />
			</div>	
        </form>
		<!-- search form END -->	
