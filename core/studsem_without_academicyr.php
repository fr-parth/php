...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT semester.student_id , semester.Semesterid, semester.BranchName, semester.DeptName ,semester.SemesterName,semester.AcdemicYear FROM  StudentSemesterRecord  semester 
...
and a.Enable='1' 
and a.school_id='$school_id'  and(semester.AcdemicYear  ='' or semester.AcdemicYear is null ) 
						LIMIT $start_from, $limit") or die("could not Search!");
 
	$sql1=" SELECT id FROM StudentSemesterRecord 
			where(AcdemicYear ='' or AcdemicYear is null ) and School_ID='$school_id' ";
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
		...
		if ($colname != ''and $colname != 'Select')
				{
			$query1=mysql_query("SELECT semester.student_id , semester.Semesterid, semester.BranchName, semester.DeptName ,semester.SemesterName,semester.AcdemicYear FROM  StudentSemesterRecord  semester 
			JOIN tbl_student std ON std.std_PRN = semester.student_id 
			JOIN tbl_academic_Year a ON semester.ExtYearID=a.ExtYearID 
		WHERE semester.school_id='$school_id' 
		and std.school_id='$school_id' 
		...
		and a.school_id='$school_id'  and(semester.AcdemicYear  ='' or semester.AcdemicYear is null ) 
				...
				$sql1 =mysql_query("SELECT id FROM StudentSemesterRecord 
									WHERE  (AcdemicYear ='' or AcdemicYear is null ) 
			...
			and a.Enable='1' 
			and a.school_id='$school_id'  and(semester.AcdemicYear  ='' or semester.AcdemicYear is null ) 
									AND (student_id LIKE '$searchq%' or Semesterid LIKE '$searchq%' or BranchName LIKE '$searchq%' or DeptName LIKE '%$searchq%'or SemesterName LIKE '$searchq%'or AcdemicYear LIKE '$searchq%'  ) LIMIT $start_from, $limit") or die("could not Search!");
			
				$sql1 =mysql_query("SELECT id FROM StudentSemesterRecord 
                					WHERE  (AcdemicYear='' or AcdemicYear is null ) 
               					 	AND School_ID='$school_id' 
									AND (student_id LIKE '$searchq%' or Semesterid LIKE '$searchq%' or BranchName LIKE '$searchq%' or DeptName LIKE '%$searchq%'or SemesterName LIKE '$searchq%'or AcdemicYear LIKE '$searchq%' )") or die("could not Search!");   
...
//$result = mysql_query("SELECT student_id , Semesterid, BranchName, DeptName ,SemesterName,AcdemicYear FROM StudentSemesterRecord where(AcdemicYear  ='' or AcdemicYear is null ) and School_ID='$school_id'");
?>
<html>
<head>
<style>
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('studsem_without_academicyr.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('studsem_without_academicyr.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<!-- Headline -->
        <div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white">Student Semester Data without Academic Year (<?php echo $_SESSION["num_record"] ; ?>) </h2>
			</div>
...
<div class="row" style="padding-top:30px;"></div>
	...
	<div class="col-md-1">
	<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('studsem_without_academicyr.php','_self')" />
	</div>	
</form>
<!-- search form END -->	
				...
				<tbody>
					 ...
					 ?>
						<tr>
                            ...
                            <td data-title="AcdemicYear"><?php echo $row['AcdemicYear']; ?></td>
							<td> <center>
