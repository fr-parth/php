...
if (!($_GET['Search']))  
{
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };  
		...
		if ($colname != ''and $colname != 'Select')
				{
			$query1=mysql_query("SELECT * FROM Division 
								WHERE ( DivisionName ='' OR DivisionName is NULL)
									AND school_id = '$school_id'
									AND `$colname` LIKE '%$searchq%' 
								LIMIT $start_from, $limit") or die("could not Search!");

				$sql1 =mysql_query(" SELECT id FROM Division 
									WHERE ( DivisionName ='' OR DivisionName is NULL) 
										AND school_id = '$school_id'
...
<div class="container">
		<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white"><?php echo $designation;?> Data without <?php echo $designation;?></h2>
		</div>
            ...
            <div class="col-md-2" style="width:17%;">
                <select name="colname" class="form-control">
					...
					<?php if (($_GET['colname']) == "ExtDegreeID") {
                            echo $_GET['colname']; ?> selected="selected" <?php } ?>><?php echo $designation;?> ID</option>

					<option value="Degree_code"
					<?php if (($_GET['colname']) == "Degree_code") {
                            echo $_GET['colname']; ?> selected="selected" <?php } ?>><?php echo $designation;?> Code</option>
										...
										<div id="no-more-tables" style="padding-top:20px;">
											<table id="example" class="display" width="100%" cellspacing="0">
												<thead style="background-color:#FFFFFF;">
													...
													<th>External <?php echo $designation;?> ID</th>
													<th><?php echo $designation;?> Name</th>
												...
												<tbody>
													...
													?>
														<tr>
															...
															<td> <center>
                     											<a style="cursor: pointer;" href="edit_degree.php?degree_id=<?php echo $row['id']; ?>"><span class="glyphicon glyphicon-pencil"></span></a>
																</center>
															</td>
