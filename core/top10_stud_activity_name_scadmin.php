...
$data=xss_clean(mysql_real_escape_string($_GET['sc_id']));
            $result_explode = explode(',',$data);
            $activity_id=$result_explode[0];
...
<body bgcolor=#FFDD88>
						...
						{	
								...
								if($activity_id=="allActivity")
							
									{
										...
										$sql=mysql_query("select spl.sc_list,s.std_complete_name,s.std_img_path,s.std_school_name,sum(sc_point) as total from tbl_student s join tbl_student_point sp on s.std_PRN=sp.sc_stud_id join tbl_studentpointslist spl on spl.sc_id = sp.sc_studentpointlist_id
									...
									{
								$sql=mysql_query("select spl.sc_list,s.std_complete_name,s.std_img_path,s.std_school_name,sum(sc_point) as total from tbl_student s join tbl_student_point sp on s.std_PRN=sp.sc_stud_id join tbl_studentpointslist spl on spl.sc_id = sp.sc_studentpointlist_id
										where s.school_id='$school_id' and sp.sc_studentpointlist_id='$activity_id' and activity_type='activity'
										GROUP BY sc_stud_id order by total desc limit 10" );
									}
											...
											<thead align="center">
												<tr class="warning">
                                                    ...
                                                    <th align="center"><?php echo xecho($dynamic_student." ".$dynamic_school."");?>College Name</th>
											...
											<tbody>
												...
												<td> <?php if($row['std_img_path'] != ''){?>
													...
													<td><?php echo xecho($row['std_complete_name']);?></td>
													<td><?php echo xecho($row['std_school_name']);?></td>
													<td><?php echo xecho($row['sc_list']);?></td>
			...
			<?php 
						...
						else 
									{
										...
										$sql=mysql_query("select spl.sc_list,s.std_complete_name,s.std_img_path,s.std_school_name,sum(sc_point) as total from tbl_student s join tbl_student_point sp on s.std_PRN=sp.sc_stud_id join tbl_studentpointslist spl on spl.sc_id = sp.sc_studentpointlist_id
										where s.school_id='$school_id' and  activity_type='activity'
										GROUP BY sc_stud_id order by total desc limit 10" );
