...
if (!($_GET['Search'])){
	...
	$sql=mysql_query("Select vm.name,vm.category,vm.mobile,ac.points,ac.coupon_id
from tbl_accept_coupon ac inner join 
tbl_vol_spect_master vm on vm.mobile=ac.stud_id and vm.school_id=ac.school_id 
where ac.school_id='$school_id' LIMIT $start_from, $webpagelimit") or die("Could not Search!");

	$sql1 =mysql_query("Select vm.name,vm.category,vm.mobile,ac.points,ac.coupon_id
from tbl_accept_coupon ac inner join 
tbl_vol_spect_master vm on vm.mobile=ac.stud_id and vm.school_id=ac.school_id 
...
from tbl_accept_coupon ac inner join 
tbl_vol_spect_master vm on vm.mobile=ac.stud_id and vm.school_id=ac.school_id 
where ac.school_id='$school_id'  AND (vm.name LIKE '%$searchq%' or vm.category LIKE '%$searchq%' or vm.mobile LIKE '%$searchq%' or ac.coupon_id LIKE '%$searchq%' or ac.points LIKE '%$searchq%') LIMIT $start_from, $webpagelimit") or die("Could not Search!");
							
			$sql1 = mysql_query("Select vm.name,vm.category,vm.mobile,ac.points,ac.coupon_id
from tbl_accept_coupon ac inner join 
tbl_vol_spect_master vm on vm.mobile=ac.stud_id and vm.school_id=ac.school_id 
where ac.school_id='$school_id'  AND (vm.name LIKE '%$searchq%' or vm.category LIKE '%$searchq%' or vm.mobile LIKE '%$searchq%' or ac.coupon_id LIKE '%$searchq%' or ac.points LIKE '%$searchq%')");
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
			...
			window.location.assign('sc_analytics_details_smartcookie_coupons_used_by_spectator.php?page='+page);
        });
    });
</script>
<?php }else{
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('sc_analytics_details_smartcookie_coupons_used_by_spectator.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
        });
    });
</script>
<?php }?>
...
<div align="center">
    <div class="container" style="width:100%;">
        <div style="padding-top:30px;">
            ...
            <h2 style="padding-left:20px; margin-top:2px;color:#666"> SmartCookie Coupons Used By Spectators</h2>
		...
		<form style="margin-top:5px;">
			...
			<div class="col-md-1" >
			<input type="button" class="btn btn-info" value="Reset" onclick="window.open('sc_analytics_details_smartcookie_coupons_used_by_spectator.php','_self')" />
			</div>
			<div class="col-md-1" >
		...
		if (isset($_GET['Search']))
		{
			...
			<td data-title="Couponid"><?php echo $row['name']; ?></td>
