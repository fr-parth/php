...
*/	
    ...
    if(!empty($scName) && !empty($phone) && !empty($scheadName) && !empty($emailid)&& !empty($Country_Code))
	{
			...
			if($res>0)
			{
				 echo "<script>alert('Email Id already exist, please try with different id!!'); window.location.href='/core/cookie_edit_school.php?s_id=$s_id'; </script>";
				 EXIT;
			}
			else if($mobcount>0)
			{
				 echo "<script>alert('Mobile number already exist, please try with different number!!'); window.location.href='/core/cookie_edit_school.php?s_id=$s_id'; </script>";
				 EXIT;
			}
			else 
			{

			$sql1 = "UPDATE `tbl_school_admin` SET  school_name='$scName' ,name='$scheadName' ,email='$emailid',school_type='$school_type',group_type='$group_id',mobile='$phone',CountryCode='$Country_Code',address='" . $_POST['address'] . "'  WHERE id='" . $s_id . "'";//exit;

            mysql_query($sql1);
            if (mysql_affected_rows() > 0) {
				echo ("<script LANGUAGE='JavaScript'>
					alert('School Updated Successfully');
					window.location.href='cookie_list_school.php';
					</script>");
            } else {
						...
						<div class="row form-group" id="school_type">
                            ...
                            <div class='col-md-3'>
                                ...
                                <select name="group_id" id="group_id" class="form-control" >
                                        ...
                                        <option value="<?php echo $result['group_type']; ?>" <?php if($result['group_type']==$val['group_type']){ echo "selected"; }?>><?php echo $result['group_type']; ?></option>
                                        <?php }?>
                                        </select> 
                                </div>
                                <!--<div  id="errorcat" style="color:#FF0000" align="center">      -->
