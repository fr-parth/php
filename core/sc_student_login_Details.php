...
if (!($_GET['Search'])){
	...
	$sql=mysql_query("Select s.std_complete_name,l.EntityID,l.school_id,l.LatestLoginTime,l.LogoutTime from tbl_LoginStatus l join tbl_student s on l.EntityID=s.id where l.Entity_type='".$entity_id."' AND
l.school_id = '$school_id' order by l.RowID desc LIMIT $start_from, $webpagelimit") or die("Could not Search!");

	$sql1 =mysql_query("Select s.std_complete_name,l.EntityID,l.school_id,l.LatestLoginTime,l.LogoutTime from tbl_LoginStatus l join tbl_student s on l.EntityID=s.id where l.Entity_type='".$entity_id."' AND
...
else
{
	...
	//$colname=mysql_real_escape_string($_GET['colname']);
			...
			$sql = mysql_query("Select s.std_complete_name,l.EntityID,l.school_id,l.LatestLoginTime,l.LogoutTime from tbl_LoginStatus l join tbl_student s on l.EntityID=s.id where l.Entity_type='".$entity_id."' AND l.school_id = '$school_id' AND (l.EntityID LIKE '%$searchq%' or l.school_id LIKE '%$searchq%' or l.LatestLoginTime LIKE '%$searchq%' or l.LogoutTime LIKE '%$searchq%'  or s.std_complete_name LIKE '%$searchq%') ORDER BY l.RowID DESC LIMIT $start_from, $webpagelimit") or die("Could not Search!");
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
			...
			window.location.assign('sc_student_login_Details.php?page='+page);
        });
    });
</script>
<?php }else{
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('sc_student_login_Details.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
        });
    });
</script>
<?php }?>
...
<div align="center">
    <div class="container" style="width:100%;">
		...
		<form style="margin-top:5px;">
			...
			<div class="col-md-1" >
			<input type="button" class="btn btn-info" value="Reset" onclick="window.open('sc_student_login_Details.php','_self')" />
			</div>
			<div class="col-md-1" >
