<?php
//Added by Rutuja for SMC-4472 to solve the issue of 404 on 31/01/2020 and also updated the query to add entity_type_id for fetching only Employee records
...
p.userType,p.id,p.date
 ...
 INNER JOIN tbl_student s ON p.user_id=s.std_PRN
 INNER JOIN softreward sr ON p.reward_id=sr.softrewardId where s.entity_type_id='205'
 order by p.id
...
p.userType,p.date
 ...
 INNER JOIN softreward sr ON p.reward_id=sr.softrewardId
  ...
  where 
			...
			p.userType LIKE '%$searchq%' or
			p.date LIKE '%$searchq%' and s.entity_type_id='205'
			order by p.id 
			LIMIT $start_from, $webpagelimit") 
...
p.userType,p.date
 ...
 INNER JOIN softreward sr ON p.reward_id=sr.softrewardId 
  where 
     '$colname' LIKE '%$searchq%' and s.entity_type_id='205' order by p.id LIMIT $start_from, $webpagelimit") 
		or die("could not Search!");
					//echo $query1;
...
p.userType,p.date
 ...
 INNER JOIN softreward sr ON p.reward_id=sr.softrewardId 
  ...
  where 
     '$colname' LIKE '%$searchq%' and s.entity_type_id='205' order by p.id"; 
					$rs_result = mysql_query($sql1);  
					$row1 = mysql_fetch_row($rs_result);  
					$total_records = $row1[0];  
					$total_pages = ceil($total_records / $webpagelimit);
            ...
            <table id="example" class="col-md-12 table-bordered " align="center">
                ...
                while($result = mysql_fetch_array($query1)) {
                    ...
                    <tr>
						...
						<td data-title="Reward Image"><img style='width:40%' 
