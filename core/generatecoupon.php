...
$sql=mysql_query("SELECT * FROM tbl_giftcards  order by issue_date desc 
        
LIMIT $start_from, $webpagelimit");	

$sql1 ="SELECT count(*) FROM tbl_giftcards  order by issue_date desc";  
...
if($searchq !='')
	...
	{
		$query1=mysql_query("SELECT * FROM tbl_giftcards  
 
  where 
            card_no LIKE '%$searchq%' or 
			amount like '%$searchq%'  or 
			issue_date LIKE '%$searchq%'or 
			valid_to LIKE '%$searchq%'	 or
			status LIKE '%$searchq%'
			order by issue_date desc 
			LIMIT $start_from, $webpagelimit") 
			or die("could not Search!");
			
			$sql1 ="SELECT count(*) FROM tbl_giftcards
  ...
  where 
			...
			$query1=mysql_query("SELECT * FROM tbl_giftcards
  where 
     '$colname' LIKE '%$searchq%' order by issue_date desc LIMIT $start_from, $webpagelimit") 
		or die("could not Search!");
					//echo $query1;
		
		$sql1 ="SELECT count(*) FROM tbl_giftcards
  where 
     '$colname' LIKE '%$searchq%' order by issue_date desc"; 
					$rs_result = mysql_query($sql1);  
					$row1 = mysql_fetch_row($rs_result);  
					$total_records = $row1[0];  
					$total_pages = ceil($total_records / $webpagelimit);
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
			...
			window.location.assign('generatecoupon.php?page='+page);
        });
    });
</script>
<?php }else{
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('generatecoupon.php?Search=<?php echo $searchq; ?>&spage='+page);
        });
    });
</script>
...
<div align="center">
    <div class="container" style="width:100%;">
		...
		<div class="col-md-2" style="width:17%;">
			...
			<div class="col-md-1" >
			<input type="button" class="btn btn-info" value="Reset" onclick="window.open('generatecoupon.php','_self')" />
			</div>
