...
if (!($_GET['Search'])){
	...
	$sql=mysql_query("SELECT p.user_id,p.school_id , p.point,s.rewardType,
...
AND p.school_id='$school_id' AND v.school_id='$school_id' LIMIT $start_from, $webpagelimit") or die("Could not Search!");

	$sql1 =mysql_query("SELECT p.user_id,p.school_id , p.point,s.rewardType,
p.userType,v.name,v.mobile,v.category
FROM softreward as s
inner join purcheseSoftreward as p
on s.softrewardId=p.reward_id
...
on p.user_id=v.mobile
 where ucwords(p.userType) 
IN ('spectator','parent','volunteer','coach','player')
AND p.school_id='$school_id' AND v.school_id='$school_id'");
...
on p.user_id=v.mobile
 where ucwords(p.userType) 
IN ('spectator','parent','volunteer','coach','player')
AND p.school_id='$school_id' AND v.school_id='$school_id' AND (v.name LIKE '%$searchq%' or v.category LIKE '%$searchq%' or v.mobile LIKE '%$searchq%' or s.rewardType LIKE '%$searchq%'  or p.point LIKE '%$searchq%') LIMIT $start_from, $webpagelimit") or die("Could not Search!");
...
on p.user_id=v.mobile
 where ucwords(p.userType) 
IN ('spectator','parent','volunteer','coach','player')
AND p.school_id='$school_id' AND v.school_id='$school_id' AND (v.name LIKE '%$searchq%' or v.category LIKE '%$searchq%' or v.mobile LIKE '%$searchq%' or s.rewardType LIKE '%$searchq%'  or p.point LIKE '%$searchq%')");
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
			...
			window.location.assign('sc_soft_rewards_purchase_spectator.php?page='+page);
        });
    });
</script>
<?php }else{
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('sc_soft_rewards_purchase_spectator.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
        });
    });
</script>
<?php }?>
...
<div align="center">
    <div class="container" style="width:100%;">
		...
		<form style="margin-top:5px;">
			...
			<div class="col-md-1" >
			<input type="button" class="btn btn-info" value="Reset" onclick="window.open('sc_soft_rewards_purchase_spectator.php','_self')" />
			</div>
			<div class="col-md-1" >
