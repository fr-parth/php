...
$sql=mysql_query(" SELECT s.id,c.Stud_Member_Id,s.school_id,s.std_complete_name,c.cp_gen_date,c.status
FROM tbl_student s 
...
$sql1 =" SELECT  count(s.id),s.school_id,c.Stud_Member_Id,s.std_complete_name,c.cp_gen_date,c.status
FROM tbl_student s 
INNER JOIN tbl_coupons c ON s.id=c.Stud_Member_Id
where c.status ='yes' order by s.id desc";  
...
if($searchq !='')
	...
	{
		$query1=mysql_query("SELECT s.id,c.Stud_Member_Id,s.school_id,s.std_complete_name,c.cp_gen_date,c.status
FROM tbl_student s 
INNER JOIN tbl_coupons c ON s.id=c.Stud_Member_Id
where (c.status ='yes')
 and
			...
			c.Stud_Member_Id LIKE '%$searchq%' or
			c.cp_gen_date LIKE '%$searchq%')
			...
			$sql1 ="SELECT  count(s.id),c.Stud_Member_Id,s.school_id,s.std_complete_name,c.cp_gen_date,c.status
FROM tbl_student s 
INNER JOIN tbl_coupons c ON s.id=c.Stud_Member_Id
where (c.status ='yes')
 and 
...
where (c.status ='yes')
 and 
     ($colname LIKE '%$searchq%') order by s.id desc LIMIT $start_from, $webpagelimit") 
		or die("could not Search!");
					//echo $query1;
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
			...
			window.location.assign('analytics_details_students_generated_smartcookie.php?page='+page);
        });
    });
</script>
<?php }else{
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('analytics_details_students_generated_smartcookie.php?Search=<?php echo $searchq; ?>&spage='+page);
        });
    });
</script>
...
<div align="center">
    <div class="container" style="width:100%;">
        <div style="padding-top:30px;">
            ...
            <h2 style="padding-left:20px; margin-top:2px;color:#666">SmartCookie Coupons Generated By Student</h2>
		...
		<div class="col-md-2" style="width:17%;">
			...
			<div class="col-md-1" >
			<input type="button" class="btn btn-info" value="Reset" onclick="window.open('analytics_details_students_generated_smartcookie.php','_self')" />
			</div>
            ...
            <table id="example" class="col-md-12 table-bordered " align="center">
                ...
                while($result = mysql_fetch_array($query1)) {
                    ...
                    <tr>
					    ...
					    <td data-title="Member ID"><?php echo $result['Stud_Member_Id']; ?></td>
						<td data-title="Coupon Generate Date"><?php echo $result['cp_gen_date']; ?></td>
