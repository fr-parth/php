...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT semester.student_id , semester.Semesterid, semester.BranchName, semester.DeptName ,semester.SemesterName, semester.ExtCourseLevelID,semester.CourseLevel FROM  StudentSemesterRecord  semester 
...
and semester.`IsCurrentSemester`='1' 
  ...
  and(semester.CourseLevel  ='' or semester.CourseLevel is null ) 
	...
	$sql1=" SELECT id FROM StudentSemesterRecord 
			where(CourseLevel  ='' or CourseLevel is null ) and school_id='$school_id'  and AcdemicYear='$Academic_Year' ";
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
		...
		if ($colname != ''and $colname != 'Select')
				{
			$query1=mysql_query("SELECT semester.student_id , semester.Semesterid, semester.BranchName, semester.DeptName ,semester.SemesterName, semester.ExtCourseLevelID,semester.CourseLevel FROM  StudentSemesterRecord  semester 
			JOIN tbl_student std ON std.std_PRN = semester.student_id 
		...
		and semester.`IsCurrentSemester`='1' 
		   ...
		   and(semester.CourseLevel  ='' or semester.CourseLevel is null ) 
				...
				$sql1 =mysql_query("SELECT id FROM StudentSemesterRecord 
									WHERE  (CourseLevel ='' or CourseLevel  is null ) 
			...
			and semester.`IsCurrentSemester`='1' 
			   and AcdemicYear='$Academic_Year' 
			  and(semester.CourseLevel  ='' or semester.CourseLevel is null ) 
									AND (student_id LIKE '$searchq%' or Semesterid LIKE '$searchq%' or BranchName LIKE '$searchq%' or DeptName LIKE '%$searchq%'or SemesterName LIKE '$searchq%'or ExtCourseLevelID LIKE '$searchq%' or CourseLevel LIKE '$searchq%'  ) LIMIT $start_from, $limit") or die("could not Search!");
			
				$sql1 =mysql_query("SELECT id FROM StudentSemesterRecord 
                WHERE  (CourseLevel ='' or CourseLevel  is null ) 
                AND school_id='$school_id'  and Acdemic_Year='$Academic_Year' 
									AND (student_id LIKE '$searchq%' or Semesterid LIKE '$searchq%' or BranchName LIKE '$searchq%' or DeptName LIKE '%$searchq%'or SemesterName LIKE '$searchq%'or ExtCourseLevelID LIKE '$searchq%'or CourseLevel LIKE '$searchq%'  )") or die("could not Search!");   
...
//$result = mysql_query("SELECT student_id , Semesterid, BranchName, DeptName ,SemesterName, ExtCourseLevelID,CourseLevel FROM StudentSemesterRecord where(CourseLevel  ='' or CourseLevel is null ) and School_ID='$school_id'");
?>
<html>
<head>
<style>
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('studsem_without_courselevel.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('studsem_without_courselevel.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white"> Student Semester Data without Courselevel(<?php echo $total_records; ?>)</h2>
       </div>
...
<form>
	...
	<div class="col-md-2" style="width:17%;">
		<select name="colname" class="form-control">
			...
			<?php if (($_GET['colname']) == "SemesterName") {
               ...
               <option value="ExtCourseLevelID"
			<?php if (($_GET['colname']) == "ExtCourseLevelID") {
					echo $_GET['colname']; ?> selected="selected" <?php } ?>>CourseLeve ID</option>  
	...
	<div class="col-md-1">
	<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('studsem_without_courselevel.php','_self')" />
	</div>	
</form>
<!-- search form END -->	
				...
				<tbody>
					 ...
					 ?>
						<tr>
                            ...
                            <td data-title="student_id"><?php echo $row['student_id']; ?></td>
                            ...
                            <td data-title="ExtCourseLevelID"><?php echo $row['ExtCourseLevelID ']; ?></td>
                            <td data-title="CourseLevel"><?php echo $row['CourseLevel ']; ?></td>
							<td> <center>
