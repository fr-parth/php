...
FROM tbl_school_admin sa 
 ...
 (
select sf.schoolID
from tbl_360_activities_data sf where activity_level_id='3' and sf.Emp_type_id in (133,134,135,137) and sf.group_member_id='$group_member_id' )
group by sa.school_id
LIMIT $start_from, $webpagelimit")

		or die("could not Search!");
					//echo $query1;
		$sql1 ="SELECT  count(distinct sa.school_id) as cnt ,sa.school_id,sa.school_name FROM tbl_school_admin sa 
        join tbl_group_school gs on sa.group_member_id=gs.group_member_id and sa.school_id=gs.school_id where sa.group_member_id='$group_member_id' and sa.school_id not in 
         (
        select sf.schoolID
        from tbl_360_activities_data sf where activity_level_id='3'and sf.Emp_type_id in (133,134,135,137) and sf.group_member_id='$group_member_id')";
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
			...
			<?php if ($_GET['year']){ ?>
			window.location.assign('society_not_given_school.php?year=<?php echo $_GET['year']; ?>&group_name=<?php echo $group_name;?>&page='+page);
			<?php }else{ ?>
			window.location.assign('society_not_given_school.php?page='+page);
				<?php } ?>
        });
    });
</script>
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('society_not_given_school.php?Search=<?php echo $searchq; ?>&spage='+page);
        });
    });
</script>
<?php }?>
      ...
      <button class="btn btn-info" type="submit"><i class="glyphicon glyphicon-chevron-left"></i>Back to  360 Feedback Statistics</button>
		...
		if (isset($_GET['Search']))
		{
          ...
          if($_GET['year'] and $year !='ALL' and $year!=''){
            $cond1=" and t.t_academic_year='$year'";
                ...
                while($result = mysql_fetch_array($query1)) {
                    ...
                    <tr>
                        <td data-title="Sr.No."><?php echo $i; ?></td>
						 ...
						 <td data-title="Teacher Count"> <a href="society360_notGiven.php?sc_id=<?php echo $a;?>&year=<?php echo $year;?>&group_name=<?php echo $group_name;?> " > <?php


						 $query=" SELECT count(distinct t.t_id) as cnt
FROM tbl_teacher t
 where t.school_id='$a' and t.t_emp_type_pid in ('133','134','135','137')  and  t.t_id NOT IN
 (
select sf.tID
from tbl_360_activities_data sf where sf.schoolID='$a' and sf.activity_level_id='3' $cond2 )";
...
FROM tbl_teacher t join tbl_school_admin sa on t.school_id=sa.school_id
 where sa.school_id='$a' and t.t_emp_type_pid in (133,134,135,137)  and  t.t_id NOT IN
 (
select sf.tID
from tbl_360_activities_data sf where sf.schoolID='$a' and sf.activity_level_id='3' and sf.Emp_type_id in (133,134,135,137)  $cond2 )";
