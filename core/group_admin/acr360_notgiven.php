...
if($_GET['year'] and $_GET['year']!='ALL' and $_GET['year']!=''){
    $sqls="  SELECT distinct t.t_id,t.t_complete_name,t.t_dept ,t.school_id,t.t_academic_year FROM  tbl_teacher t join tbl_school_admin sa on t.school_id=sa.school_id where sa.school_id='$sc_id'
                          and t.t_emp_type_pid='137'  and t.t_id NOT IN
                            (select sf.tID
                          from tbl_360_activities_data sf where sf.activity_level_id='4' and  (sf.Emp_type_id ='137') and sf.schoolID='$sc_id' and sf.Academic_Year='$year')
  LIMIT $start_from, $webpagelimit";
  $sql=mysql_query($sqls);

  $sql1 =" SELECT count(distinct t.t_id),t.t_complete_name,t.t_dept ,t.school_id,t.t_academic_year FROM  tbl_teacher t join tbl_school_admin sa on t.school_id=sa.school_id where sa.school_id='$sc_id'
   and t.t_emp_type_pid='137' and t.t_id NOT IN
   (select sf.tID
 from tbl_360_activities_data sf where sf.activity_level_id='4' and  (sf.Emp_type_id ='137') and sf.schoolID='$sc_id' and sf.Academic_Year='$year')";
}
...
$sql=mysql_query("
SELECT distinct t.t_id,t.t_complete_name,t.t_dept ,t.school_id,t.t_academic_year FROM  tbl_teacher t join tbl_school_admin sa on t.school_id=sa.school_id where sa.school_id='$sc_id'
and t.t_emp_type_pid='137' and t.t_id NOT IN
 (
select sf.tID
from tbl_360_activities_data sf where sf.activity_level_id='4' and  (sf.Emp_type_id ='137') and sf.schoolID='$sc_id')
LIMIT $start_from, $webpagelimit");
...
//$colname=mysql_real_escape_string($_GET['colname']);
	...
	{
    if($_GET['year'] and $_GET['year']!='ALL' and $_GET['year']!=''){
      $query1=mysql_query("SELECT distinct t.t_id,t.t_complete_name,t.t_dept ,t.school_id,t.t_academic_year FROM  tbl_teacher t join tbl_school_admin sa on t.school_id=sa.school_id where sa.school_id='$sc_id'
   and t.t_emp_type_pid='137' and t.t_id NOT IN
       (
     select sf.tID
     from tbl_360_activities_data sf where sf.activity_level_id='4' and  (sf.Emp_type_id ='137') and sf.schoolID='$sc_id' and sf.Academic_Year='$year')
...
FROM tbl_teacher  t
 where t.school_id='$sc_id' and t.t_emp_type_pid='137' and t.t_id NOT IN
 (
select ft.tID
from tbl_360_activities_data ft where ft.activity_level_id='4' and  (sf.Emp_type_id ='137') and  ft.tID is not NULL  and  ft.schoolID='$sc_id' )
 and  $colname LIKE '%$searchq%'
LIMIT $start_from, $webpagelimit")

		or die("could not Search!");
					//echo $query1;
		$sql1 ="SELECT count(distinct t.t_id),t.t_complete_name,t.t_dept ,t.school_id,t.t_academic_year
FROM tbl_teacher  t
 where t.school_id='$sc_id' and t.t_emp_type_pid='137'  and t.t_id NOT IN
 (select ft.tID
from tbl_360_activities_data ft where ft.activity_level_id='4' and  (sf.Emp_type_id ='137') and  ft.tID is not NULL  and  ft.schoolID='$sc_id' )
...
<script type="text/javascript">
    $(function () {
      ...
      <?php if($_GET['year']){ ?>
         window.location.assign('acr360_notgiven.php?sc_id=<?php echo $sc_id;?>&year=<?php echo $year;?>&group_name=<?php echo $group_name;?>&page='+page);
      <?php } else {?>
        window.location.assign('acr360_notgiven.php?sc_id=<?php echo $sc_id;?>&page='+page);
      <?php } ?>
        });
    });
</script>
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
      <?php if($_GET['year']){ ?>
				 window.location.assign('acr360_notgiven.php?sc_id=<?php echo $sc_id;?>&year=<?php echo $year;?>&group_name=<?php echo $group_name;?>&Search=<?php echo $searchq; ?>&spage='+page);
			<?php } else {?>
				window.location.assign('acr360_notgiven.php?sc_id=<?php echo $sc_id;?>&Search=<?php echo $searchq; ?>&spage='+page);
			<?php } ?>
        });
    });
</script>
...
<div align="center">
    <div class="container" style="width:100%;">
        <div style="padding-top:30px;">
            ...
            <h2 style="padding-left:20px; margin-top:2px;color:#666"> ACR Activity-Not Given</h2>
     ...
     <?php } ?>
			...
			<div class="col-md-1" >
			<input type="button" class="btn btn-info" value="Reset" onclick="window.open('acr360_notgiven.php?sc_id=<?php echo $_GET['sc_id']; ?>&year=<?php echo $_GET['year'];?>&group_name=<?php echo $group_name;?>','_self')" />

			</div>
				</form>
						 </div>
  					<form method="post" class="pull-left" action="acr_not_given_school.php?year=<?php echo $year?>&group_name=<?php echo $group_name;?>">
...
a.Academic_Year
 ...
 t.school_id='$a'";
            ...
            <td data-title="AcademicYear"><?php echo $result['t_academic_year']; ?></td>
