...
$qr="select * from tbl_student where school_id='$college' $cond";

$rs_result=mysql_query($qr);
 unset($_SESSION['report_header']);
  unset($_SESSION['report_values']);

    $_SESSION['report_header']=array("Sr.No.","Member ID","PRN","Name","Email ID","Phone Number","Date Of Birth","School Name","School ID","Branch","std_dept","Class","Address","City","State","Country","Country Code","Gender","Permanent Address","Academic Year","Course Level","Group member ID");
  $j1=0;
  ...
  $_SESSION['report_values'][$j][9]=$row7['std_branch'];
  $_SESSION['report_values'][$j][10]=$row7['Department'];
  $_SESSION['report_values'][$j][11]=$row7['std_class'];
  $_SESSION['report_values'][$j][12]=$row7['std_address'];
  $_SESSION['report_values'][$j][13]=$row7['std_city'];
  $_SESSION['report_values'][$j][14]=$row7['std_state'];
  $_SESSION['report_values'][$j][15]=$row7['std_country'];
  $_SESSION['report_values'][$j][16]=$row7['country_code'];
  $_SESSION['report_values'][$j][17]=$row7['std_gender'];
  $_SESSION['report_values'][$j][18]=$row7['permanent_address'];
  $_SESSION['report_values'][$j][19]=$row7['Academic_Year'];
  $_SESSION['report_values'][$j][20]=$row7['Course_level'];
  $_SESSION['report_values'][$j][21]=$row7['group_member_id'];
 
    
     } ?> <div align="center"> <?php if($j1>0){ echo "<h2>".$college_name."</h2><h3>".$j1." Student Downloaded Successfully"; ?>&nbsp;&nbsp;&nbsp;&nbsp; <?php echo "<a href='student_data_report.php'>Back</a></h3>"; }else{echo "<h2>".$college_name."</h2><h3>Student data not found"; ?>&nbsp;&nbsp;&nbsp;&nbsp; <?php echo "<a href='student_data_report.php'>Back</a></h3>";} ?> </div> <?php 

echo ("<script LANGUAGE='JavaScript'>
        
          window.location.href='Export_report.php?fn=".$college."_student';
...
<select name="year" id="year" class="smartsearch form-control" required />
            ...
            <?php $row=mysql_query("select Academic_Year from tbl_student where Academic_Year !='' and school_id='".$value['school_id']."'");
      while($value1=mysql_fetch_array($row)){?>

            <option value="<?php echo $value1['Academic_Year'];?>"><?php echo $value1['Academic_Year'];?></option>
                         <?php } ?>
      </select>
