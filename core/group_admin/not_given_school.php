...
// inner join (SELECT count(distinct sf.stu_feed_student_ID,sf.stu_feed_academic_year) as cnt, FROM tbl_student_feedback sf join tbl_student st on sf.stu_feed_student_ID=st.std_PRN and sf.stu_feed_school_id=st.school_id

//  where st.group_member_id='$group_member_id' and sf.stu_feed_academic_year='$year')
...
join tbl_group_school gs on sa.group_member_id=gs.group_member_id and sa.school_id=gs.school_id 
where sa.group_member_id='$group_member_id' and sa.school_id not in 
(Select distinct stu_feed_school_id from tbl_student_feedback where stu_feed_academic_year='$year') 
LIMIT $start_from, $webpagelimit")

		or die("could not Search!");
					//echo $query1;
		$sql1 ="SELECT count(distinct sa.school_id) FROM tbl_school_admin sa 
		join tbl_group_school gs on sa.group_member_id=gs.group_member_id and sa.school_id=gs.school_id 
		where sa.group_member_id='$group_member_id' and sa.school_id not in 
		(Select distinct stu_feed_school_id from tbl_student_feedback where stu_feed_academic_year='$year')";
					$rs_result = mysql_query($sql1);
...
<script type="text/javascript">
   $(function () {
      ...
      <?php if($_GET['year'] and ($_GET['group_name'])){ ?>
         window.location.assign('not_given_school.php?year=<?php echo $year;?>&group_name=<?php echo $group_name;?>&page='+page);
      <?php } else {?>
        window.location.assign('not_given_school.php?page='+page);
      <?php } ?>
        });
    });
</script>
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('not_given_school.php?Search=<?php echo $searchq; ?>&spage='+page);
        });
    });
</script>
<?php }?>
                ...
                while($result = mysql_fetch_array($query1)) {
                    ...
                    <tr>
                        <td data-title="Sr.No."><?php echo $i; ?></td>
						 ...
						 <td data-title="Student Count"> <a href="StudentFeedback_notGiven.php?sc_id=<?php echo $a;?>&year=<?php echo $year;?>&group_name=<?php echo $group_name;?> " > <?php
						 //$a=$result['school_id'];
