...
FROM tbl_school_admin sa 
  join tbl_group_school gs on sa.group_member_id=gs.group_member_id and sa.school_id=gs.school_id where sa.group_member_id='$group_member_id' and sa.school_id not in
(select sf.schoolID
from tbl_360_activities_data sf where sf.group_member_id='$group_member_id'  and sf.activity_level_id='4' and  (sf.Emp_type_id ='137') ) 
group by sa.school_id
LIMIT $start_from, $webpagelimit")

		or die("could not Search!");
					//echo $query1;
		$sql1 ="SELECT  count(distinct sa.school_id) as cnt ,sa.school_id,sa.school_name
        FROM tbl_school_admin sa 
          join tbl_group_school gs on sa.group_member_id=gs.group_member_id and sa.school_id=gs.school_id where sa.group_member_id='$group_member_id' and sa.school_id not in
        (select sf.schoolID
        from tbl_360_activities_data sf where sf.group_member_id='$group_member_id'  and sf.activity_level_id='4' and  (sf.Emp_type_id ='137') )
...
<script type="text/javascript">
    $(function () {
      ...
      <?php if($_GET['year']){ ?>
         window.location.assign('acr_not_given_school.php?year=<?php echo $year;?>&group_name=<?php echo $group_name;?>&page='+page);
      <?php } else {?>
        window.location.assign('acr_not_given_school.php?page='+page);
      <?php } ?>
        });
    });
</script>
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('acr_not_given_school.php?Search=<?php echo $searchq; ?>&spage='+page);
        });
    });
</script>
<?php }?>
                ...
                while($result = mysql_fetch_array($query1)) {
                    ...
                    <tr>
                        <td data-title="Sr.No."><?php echo $i; ?></td>
						 ...
						 <td data-title="Teacher Count"> <a href="acr360_notgiven.php?sc_id=<?php echo $a;?>&year=<?php echo $year;?>&group_name=<?php echo $group_name;?> " > <?php
                         ...
                         and t.t_emp_type_pid='137' and t.t_id NOT IN
                          (
                        select sf.tID
                        from tbl_360_activities_data sf where sf.activity_level_id='4' and  (sf.Emp_type_id ='137') and sf.schoolID='$a' $cond2)";
