...
if(($year == 'ALL' && $group_member_id =='ALL') or ($year=='ALL' && $group_member_id=='') or ($year=='' && $group_member_id=='ALL'))
{
  ...
  join tbl_group_school gs on sa.group_member_id=gs.group_member_id and sa.school_id=gs.school_id where sa.school_id  in 
   (select sf.schoolID
  from tbl_360_activities_data sf join tbl_teacher t on t.t_id= sf.tID  where activity_level_id='3' and sf.Emp_type_id in (133,134,135,137))
  ...
  from tbl_360_activities_data sf join tbl_teacher t on t.t_id= sf.tID where activity_level_id='3'and sf.Emp_type_id in (133,134,135,137))";
    ...
    if(($year!='' and $year!='ALL') and ($group_member_id!='' and $group_member_id!='ALL')){
        ...
        join tbl_group_school gs on sa.group_member_id=gs.group_member_id and sa.school_id=gs.school_id where sa.group_member_id='$group_member_id' and sa.school_id  in 
         (select sf.schoolID
        from tbl_360_activities_data sf join tbl_teacher t on t.t_id= sf.tID  where activity_level_id='3' and sf.Emp_type_id in (133,134,135,137) and sf.group_member_id='$group_member_id' and sf.Academic_Year='$year')
        LIMIT $start_from, $webpagelimit");
    ...
    elseif (($year!='' and $year!='ALL') and ($group_member_id=='' or $group_member_id=='ALL')){
        ...
        join tbl_group_school gs on sa.group_member_id=gs.group_member_id and sa.school_id=gs.school_id where  sa.school_id  in 
         (select sf.schoolID
        from tbl_360_activities_data sf join tbl_teacher t on t.t_id= sf.tID  where activity_level_id='3' and sf.Emp_type_id in (133,134,135,137)  and sf.Academic_Year='$year')
        LIMIT $start_from, $webpagelimit");
        ...
        join tbl_group_school gs on sa.group_member_id=gs.group_member_id and sa.school_id=gs.school_id where sa.group_member_id='$group_member_id' and sa.school_id  in 
         (select sf.schoolID
        from tbl_360_activities_data sf join tbl_teacher t on t.t_id= sf.tID  where activity_level_id='3' and sf.Emp_type_id in (133,134,135,137) and sf.group_member_id='$group_member_id' )
        LIMIT $start_from, $webpagelimit");
...
$query1=mysql_query("SELECT count(distinct sf.tID) as cnt ,sc.school_name,sc.school_id FROM
tbl_school_admin sc  join tbl_360_activities_data sf on sc.school_id=sf.schoolID
where sf.activity_level_id='3' and (sf.Emp_type_id in (133,134,135,137)) and $cond
group by sc.school_id
...
LIMIT $start_from, $webpagelimit")
		...
		or die("could not Search!");
					//echo $query1;
		$sql1 ="SELECT count(distinct sc.school_id) as cnt ,sc.school_name,sc.school_id FROM
tbl_school_admin sc  join tbl_360_activities_data sf on sc.school_id=sf.schoolID
where sf.activity_level_id='3' and (sf.Emp_type_id in (133,134,135,137)) and $cond
";
					$rs_result = mysql_query($sql1);
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
			...
			<?php if($_GET['year']){ ?>
				 window.location.assign('society_given_school.php?year=<?php echo $year;?>&group_name=<?php echo $group_name;?>&page='+page);
			<?php } else {?>
				window.location.assign('society_given_school.php?page='+page);
			<?php } ?>
        });
    });
</script>
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('society_given_school.php?Search=<?php echo $searchq; ?>&spage='+page);
        });
    });
</script>
<?php }?>
                ...
                while($result = mysql_fetch_array($query1)) {
                    ...
                    <tr>
                        <td data-title="Sr.No."><?php echo $i; ?></td>
						 ...
						 <td data-title="Teacher Count"> <a href="society360_given.php?sc_id=<?php echo $a;?>&year=<?php echo $year;?>&group_name=<?php echo $group_name;?> " > <?php
						 if ($_GET['year'] and $_GET['year']!='ALL' and $_GET['year']!=''){
							$cond1="and Academic_Year='$year'";
						} else { $cond1=""; }
						 $query=" SELECT count(distinct tID,Academic_Year,deptID) as cnt  FROM tbl_360_activities_data where schoolID='$a' and activity_level_id='3' $cond1";
                    ...
                    <tr>
                        ...
                        <td data-title="School Name"><?php echo $result['school_name']; ?></td>
						 ...
						 if ($_GET['year'] and $_GET['year']!='ALL' and $_GET['year']!=''){
						    ...
						    $query="SELECT count(distinct t.t_id) as cnt
							FROM tbl_teacher t join tbl_school_admin sa on t.school_id=sa.school_id
							 where sa.school_id='$a' and t.t_emp_type_pid in (133,134,135,137)  and  t.t_id  IN
							 (
							select sf.tID
							from tbl_360_activities_data sf where sf.schoolID='$a' and sf.activity_level_id='3' and sf.Emp_type_id in (133,134,135,137)  $cond1 )";
							$sql=mysql_query($query);
							$result=mysql_fetch_array($sql);
						    echo $result['cnt'];?></td>
                        </tr>
