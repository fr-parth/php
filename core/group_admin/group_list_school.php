...
{  
	...
	$row=mysql_query("SELECT tsa.id,tsa.school_id,tsa.school_name,tsa.name,tsa.school_name,tsa.email,tsa.group_status,tsa.school_type,tsa.reg_date,tsa.lat,tsa.lon FROM tbl_school_admin tsa inner join tbl_group_school tgs on tsa.school_id=tgs.school_id WHERE tgs.school_id!='' and tgs.group_member_id='$group_member_id' ORDER BY tsa.school_id  LIMIT $start_from, $limit") or die("Query Failed!");
	$sql1="SELECT tsa.school_id FROM `tbl_school_admin` tsa inner join tbl_group_school tgs on tsa.school_id=tgs.school_id where tgs.school_id!='' AND tgs.group_member_id='$group_member_id' ";
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
		...
		if ($colname != ''and $colname != 'Select')
				{ 
					$colname1="tsa.".$colname;
					
			$query1=mysql_query("SELECT tsa.id,tsa.school_id,tsa.school_name,tsa.name,tsa.school_name,tsa.email,tsa.group_status,tsa.school_type,tsa.reg_date,tsa.lat,tsa.lon FROM tbl_school_admin tsa inner join tbl_group_school tgs on tsa.school_id=tgs.school_id WHERE tgs.school_id!='' AND 
			tgs.group_member_id='$group_member_id' AND $colname1 LIKE '%$searchq%' ORDER BY tsa.school_id  LIMIT $start_from, $limit") or die("could not Search!");
									

				$sql1 =mysql_query("SELECT tsa.id,tsa.school_id,tsa.school_name,tsa.name,tsa.school_name,tsa.email,tsa.group_status,tsa.school_type,tsa.reg_date,tsa.lat,tsa.lon FROM tbl_school_admin tsa inner join tbl_group_school tgs on tsa.school_id=tgs.school_id WHERE tgs.school_id!='' AND tgs.group_member_id='$group_member_id' AND $colname1 LIKE '%$searchq%' ORDER BY school_id ") or die("could not Search!");
									
				$total_records = mysql_num_rows($sql1);   
				$total_pages = ceil($total_records / $limit);
		}else{	
				$query1=mysql_query("SELECT tsa.id,tsa.school_id,tsa.school_name,tsa.name,tsa.school_name,tsa.email,tsa.group_status,tsa.school_type,tsa.reg_date,tsa.lat,tsa.lon FROM tbl_school_admin tsa inner join tbl_group_school tgs on tsa.school_id=tgs.school_id WHERE tgs.school_id!='' AND tgs.group_member_id='$group_member_id' AND ( id LIKE '%$searchq%' or
				school_id LIKE '%$searchq%' or
				school_name LIKE '%$searchq%' or
				name LIKE '%$searchq%' or
				school_name LIKE '%$searchq%' or
				email LIKE '%$searchq%' or
				group_status LIKE '%$searchq%' or
				...
				$sql1 =mysql_query("SELECT tsa.id,tsa.school_id,tsa.school_name,tsa.name,tsa.school_name,tsa.email,tsa.group_status,tsa.school_type,tsa.reg_date FROM tbl_school_admin tsa inner join tbl_group_school tgs on tsa.school_id=tgs.school_id 
									WHERE tgs.school_id!='' AND tgs.group_member_id='$group_member_id' AND ( id LIKE '%$searchq%' or
									school_id LIKE '%$searchq%' or
									school_name LIKE '%$searchq%' or
									name LIKE '%$searchq%' or
									school_name LIKE '%$searchq%' or
...
//if (isset($_GET["page"])) { $page  = $_GET["page"]; } else { $page=1; };  
//$start_from = ($page-1) * $limit;
//$sql="SELECT id,school_id,school_name,name,school_name,email,group_status,school_type,reg_date FROM `tbl_school_admin` where `school_id`!='' LIMIT $start_from, $limit order by id desc ";
//$row = mysql_query($sql);
?>
<html>
	...
	*/
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('group_list_school.php?page='+page);
    		});
  		});
  		</script>
		<?php }else{ ?>
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('group_list_school.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   		</script>
	<?php }?>
    ...
    function confirmation(xxx)
    	...
    	if (answer){
        	...
        	window.location = "group_delete_school.php?sc_id="+xxx;
		...
		<form>
            ...
            <div class="col-md-2" style="width:17%;">
                <select name="colname" class="form-control">
					...
					<option value="group_status"
					<?php if (($_GET['colname']) == "group_status") {
                            echo $_GET['colname']; ?> selected="selected" <?php } ?>>Group Status</option> 
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('group_list_school.php','_self')" />
			</div>	
        </form>
					...
					<?php
						if (isset($_GET['Search']))
						{
							...
							if($count == 0){
								...
								/* $searchq=$_GET['Search'];
								$query1=mysql_query(" SELECT id,school_id,school_name,name,school_name,email,group_status,school_type,reg_date FROM `tbl_school_admin` where 
								`school_id`!='' AND
								id LIKE '%$searchq%' or
								school_id LIKE '%$searchq%' or
								school_name LIKE '%$searchq%' or
								...
								?>	
									<table id="example" class="table-bordered table-striped ">
										...
										<?php $i=($start_from +1);
												...
												<td><?php echo $result['group_status'];?></td>
												<td><?php echo $result['school_type'];?></td>
												<td><?php echo $result['lat'];?></td>
												<td><?php echo $result['lon'];?></td>
												<td><?php echo $result['reg_date'];?></td>
												<td><a href="group_edit_school.php?s_id=<?php echo $result['id']; ?>"><span class="glyphicon glyphicon-pencil"></span></a></td>
