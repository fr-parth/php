...
if($year !='ALL' && $year!='')
{
    $cond .= "and ft.feed360_academic_year_ID='$year'";
}
else
{
    $cond=" ";
...
and  t.t_emp_type_pid in (133,134,135,137)
                         and t.t_id in (select ft.feed360_teacher_id
from tbl_360feedback_template ft where ft.feed360_school_id='$sc_id' $cond )
LIMIT $start_from, $webpagelimit");

/*removed part from $sql = sf.stu_feed_school_id=st.school_id AND  . after 'on' condition*/

$sql1 ="SELECT count(distinct t.t_id) FROM tbl_teacher t join tbl_school_admin sa on t.school_id=sa.school_id where sa.school_id='$sc_id' 
and t.t_emp_type_pid in (133,134,135,137)
                         and t.t_id in (select ft.feed360_teacher_id
from tbl_360feedback_template ft where ft.feed360_school_id='$sc_id' $cond )";
...
//$colname=mysql_real_escape_string($_GET['colname']);
    ...
    $query1=mysql_query("SELECT t.t_id,t.t_complete_name,t.t_dept ,t.school_id,t.t_academic_year
    FROM tbl_teacher t join tbl_school_admin sa on t.school_id=sa.school_id where sa.school_id='$sc_id' 
    and  t.t_emp_type_pid in (133,134,135,137)
                               and t.t_id in (select ft.feed360_teacher_id
    from tbl_360feedback_template ft where ft.feed360_school_id='$sc_id' $cond )
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            <?php if($_GET['year']){ ?>
               window.location.assign('teachingProcess_Given.php?sc_id=<?php echo $sc_id;?>&year=<?php echo $year;?>&group_name=<?php echo $group_name;?>&page='+page);
            <?php } else {?>
 			        window.location.assign('teachingProcess_Given.php?sc_id=<?php echo $sc_id;?>&page='+page);
            <?php } ?>
        });
    });
</script>
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
            <?php if($_GET['year']){ ?>
               window.location.assign('teachingProcess_Given.php?sc_id=<?php echo $sc_id;?>&year=<?php echo $year;?>&group_name=<?php echo $group_name;?>&Search=<?php echo $searchq; ?>&spage='+page);
            <?php } else {?>
              window.location.assign('teachingProcess_Given.php?sc_id=<?php echo $sc_id;?>&Search=<?php echo $searchq; ?>&spage='+page);
            <?php } ?>
        });
    });
</script>
...
<div align="center">
    <div class="container" style="width:100%;">
        <div style="padding-top:30px;">
            ...
            <h2 style="padding-left:20px; margin-top:2px;color:#666"> Teaching Process- Given</h2>
     ...
     <?php } ?>
			...
			<div class="col-md-1" >
			<input type="button" class="btn btn-info" value="Reset" onclick="window.open('teachingProcess_Given.php?sc_id=<?php echo $_GET['sc_id']; ?>&year=<?php echo $_GET['year'];?>&group_name=<?php echo $group_name;?>','_self')" />
			</div>
				</form>
						 </div>
  					<!-- <form method="post" class="pull-left" action="tnot_given_school_list.php?year=<//?php echo $year?>">
      <input type="text" name="year" value="<//?= $year;?>" /> -->

    <!-- </form> -->
	<div class=container style="margin-right: 86%;">
	<a href="tschool_list.php?year=<?php echo $year;?>&group_name=<?php echo $group_name;?>"><button class="btn btn-info" ><i class="glyphicon glyphicon-chevron-left"></i>Back to  School List</button></a>
	</div>
...
a.Academic_Year
 ...
 FROM tbl_teacher_subject_master sm join tbl_teacher s on sm.school_id=s.school_id and
 sm.teacher_id=s.t_id
 ...
 s.school_id='$a'";
           ...
           <td data-title="AcademicYear"><?php echo $result['feed360_academic_year_ID']; ?></td>
