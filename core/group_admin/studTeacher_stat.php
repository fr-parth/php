...
if (isset($_POST['submit']))
	{
		...
		//teacher
		$sqlLoginTeacher = "SELECT distinct(ls.EntityID), ls.school_id, ls.LatestLoginTime, ls.LogoutTime, 
t.group_member_id,t.t_complete_name from tbl_LoginStatus  ls join tbl_teacher t on 
ls.EntityID=t.id where
 $where and  Entity_type='103' 
 and t.group_member_id='$group_member_id'  GROUP BY ls.EntityID ";
		
		$resultLoginTeacher = mysql_query($sqlLoginTeacher);
		$countLoginTeacher = mysql_num_rows($resultLoginTeacher); 
		
		$sqlTeacher = " SELECT * FROM tbl_teacher where group_member_id='$group_member_id' and  t_emp_type_pid 
		in (133,134,135,137)";
		...
		//student
		$sqlLoginStudent = "Select distinct (l.EntityID),s.group_member_id,s.std_complete_name,s.std_PRN,l.school_id,l.LatestLoginTime,l.LogoutTime 
from tbl_LoginStatus l join tbl_student s on l.EntityID=s.id where $where and l.Entity_type='105'
and s.group_member_id='$group_member_id' group by l.EntityID";
		
		$resultLoginStudent = mysql_query($sqlLoginStudent);
		$countLoginStudent = mysql_num_rows($resultLoginStudent); 
		
		$sqlStudent = " SELECT * FROM tbl_student where group_member_id='$group_member_id'";
