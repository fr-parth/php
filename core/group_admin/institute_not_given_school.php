...
if(($year == 'ALL' && $group_member_id =='ALL') or ($year=='ALL' && $group_member_id=='') or ($year=='' && $group_member_id=='ALL'))
{
    ...
    (select sf.schoolID from tbl_360_activities_data sf join tbl_teacher t on t.t_id= sf.tID where  sf.activity_level_id='2' and 
    (sf.Emp_type_id='137'))
  ...
  (sf.Emp_type_id ='137')) ";
	...
	if(($year!='' and $year!='ALL') and ($group_member_id!='' and $group_member_id!='ALL')){
		...
		(select sf.schoolID from tbl_360_activities_data sf join tbl_teacher t on t.t_id= sf.tID where  sf.activity_level_id='2' and 
		(sf.Emp_type_id='137')  and  sf.group_member_id='$group_member_id' and sf.Academic_Year='$year'  )
		 LIMIT $start_from, $webpagelimit");
	...
	elseif (($year!='' and $year!='ALL') and ($group_member_id=='' or $group_member_id=='ALL')){
			...
			where sa.school_id not in 
			(select sf.schoolID from tbl_360_activities_data sf join tbl_teacher t on t.t_id= sf.tID where  sf.activity_level_id='2' and 
			(sf.Emp_type_id='137')  and  sf.Academic_Year='$year'  )
			 LIMIT $start_from, $webpagelimit");
	...
	$query1=mysql_query("SELECT  distinct sa.school_id,sa.school_name FROM tbl_school_admin sa 
			...
			where sa.group_member_id='$group_member_id' and sa.school_id not in 
			(select sf.schoolID from tbl_360_activities_data sf join tbl_teacher t on t.t_id= sf.tID where  sf.activity_level_id='2' and 
			(sf.Emp_type_id='137')  and  sf.group_member_id='$group_member_id' )
			 LIMIT $start_from, $webpagelimit");
...
// group by sc.school_id";
	...
	// }else{
		...
		FROM tbl_school_admin sa 
		  join tbl_group_school gs on sa.group_member_id=gs.group_member_id and sa.school_id=gs.school_id 
		  where sa.group_member_id='$group_member_id' and sa.school_id not in 
		(select sf.schoolID from tbl_360_activities_data sf where  sf.activity_level_id='2' and 
		 (sf.Emp_type_id='137' ) and  sf.group_member_id='$group_member_id' $cond ) group by sa.school_id
		...
		LIMIT $start_from, $webpagelimit")
				...
				where sa.group_member_id='$group_member_id' and sa.school_id not in 
			  (select sf.schoolID from tbl_360_activities_data sf where  sf.activity_level_id='2' and 
			   (sf.Emp_type_id='137' ) and  sf.group_member_id='$group_member_id' $cond )";
							$rs_result = mysql_query($sql1);
							$row1 = mysql_fetch_row($rs_result);
							$total_records = $row1[0];
							$total_pages = ceil($total_records / $webpagelimit);
...
<script type="text/javascript">
    $(function () {
      ...
      <?php if($_GET['year']){ ?>
         window.location.assign('institute_not_given_school.php?year=<?php echo $year;?>&group_name=<?php echo $group_name;?>&page='+page);
      <?php } else {?>
        window.location.assign('institute_not_given_school.php?page='+page);
      <?php } ?>
        });
    });
</script>
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('institute_not_given_school.php?Search=<?php echo $searchq; ?>&spage='+page);
        });
    });
</script>
<?php }?>
                ...
                while($result = mysql_fetch_array($query1)) {
                    ...
                    <tr>
                        <td data-title="Sr.No."><?php echo $i; ?></td>
						 ...
						 <td data-title="Teacher Count"> <a href="institute360_notGiven.php?sc_id=<?php echo $a;?>&year=<?php echo $year;?>&group_name=<?php echo $group_name;?> " > <?php
             if($_GET['year'] and $year !='ALL' and $year!=''){
               $cond2=" and Academic_Year='$year'";
             }else {
             $cond2=""; }
						 ...
						 and  t.t_emp_type_pid='137'
						  and t.t_id not in (select tID
						 from tbl_360_activities_data where schoolID='$a' and activity_level_id='2' $cond2)";
