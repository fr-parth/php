...
if($month < 08){
  ...
  $sqln1="SELECT count(id) as cnt,school_id,school_name,scadmin_state,ROUND((SUM(IFNULL(teaching_process,0))/count(id)), 2) as teaching_process,ROUND((SUM(IFNULL(student_feedback,0))/count(id)),2) as student_feedback,ROUND((SUM(IFNULL(dept_activity,0))/count(id)),2) as dept_activity,ROUND((SUM(IFNULL(inst_activity,0))/count(id)),2) as inst_activity,ROUND((SUM(IFNULL(acr,0))/count(id)),2) as acr,ROUND((SUM(IFNULL(cont_society,0))/count(id)),2) as cont_society, (ROUND((SUM(IFNULL(teaching_process,0))/count(id)), 2)+ROUND((SUM(IFNULL(student_feedback,0))/count(id)),2)+ROUND((SUM(IFNULL(dept_activity,0))/count(id)),2)+ROUND((SUM(IFNULL(inst_activity,0))/count(id)),2)+ROUND((SUM(IFNULL(acr,0))/count(id)),2)+ROUND((SUM(IFNULL(cont_society,0))/count(id)),2)) as total_points FROM aicte_ind_feedback_summary_report where $where group by school_id order by total_points DESC LIMIT $Lmt";
    // echo $sqln1; exit;
...
<div class="container">
  <div class="row">
        ...
        <div class="radius" style="height:50px; background-color:#694489;" align="center">
          <h2 style="padding-left:20px;padding-top:10px;color:white">360 Feedback Top Institute</h2>
        </div>
  </div>
        ...
        <div class="col-md-4">
          ...
          <div class="col-sm-8" id="typeedit">
            <select class="form-control searchselect" name="institute" id="institute" required="required">
              <option <?php if(isset($_POST['institute'])){ if($_POST['institute']=="5"){ echo "selected"; } }?> value="5"> 5 Institute</option>
              <option <?php if(isset($_POST['institute'])){ if($_POST['institute']=="10"){ echo "selected"; } }?> value="10">10 Institute</option>
              <option <?php if(isset($_POST['institute'])){ if($_POST['institute']=="15"){ echo "selected"; } }?> value="15">15 Institute</option>
            </select>
          </div>
        </div>
        <div class="col-md-6">
