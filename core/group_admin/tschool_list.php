...
and sa.school_id=gs.school_id where sa.school_id  in (SELECT distinct ft.feed360_school_id
...
FROM tbl_360feedback_template  ft join tbl_teacher t on t.t_id=ft.feed360_teacher_id)";
	...
	if(($year!='' and $year!='ALL') and ($group_member_id!='' and $group_member_id!='ALL')){
		$query1=mysql_query("SELECT distinct(sa.school_id),sa.school_name  FROM tbl_school_admin sa join tbl_group_school gs on sa.group_member_id=gs.group_member_id
and sa.school_id=gs.school_id where sa.group_member_id='$group_member_id' and sa.school_id  in (SELECT distinct ft.feed360_school_id
...
$query1=mysql_query("SELECT count(distinct ft.feed360_teacher_id) as cnt ,sc.school_name,sc.school_id FROM tbl_school_admin sc  join tbl_360feedback_template ft on sc.school_id=ft.feed360_school_id where $cond
group by sc.school_id
...
LIMIT $start_from, $webpagelimit")
		...
		or die("could not Search!");
					//echo $query1;
		$sql1 ="SELECT count(distinct sc.school_id) as cnt  FROM tbl_school_admin sc  join tbl_360feedback_template ft on sc.school_id=ft.feed360_school_id where $cond";
					$rs_result = mysql_query($sql1);
					$row1 = mysql_fetch_row($rs_result);
					$total_records = $row1[0];
					$total_pages = ceil($total_records / $webpagelimit);
...
<script type="text/javascript">
    $(function () {
      ...
      <?php if ($_GET['year']){ ?>
window.location.assign('tschool_list.php?year<?php echo $_GET['year']; ?>&group_name=<?php echo $group_name;?>&page='+page);
<?php }else{ ?>
window.location.assign('tschool_list.php?page='+page);
	<?php } ?>
        });
    });
</script>
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('tschool_list.php?Search=<?php echo $searchq; ?>&group_name=<?php echo $group_name;?>&spage='+page);
        });
    });
</script>
<?php }?>
...
<div align="center">
	...
	{
		$cond1 = "and ft.feed360_academic_year_ID='$year'";
	}
	else
	{
		$cond1 ="";
                ...
                while($result = mysql_fetch_array($query1)) {
                    ...
                    <tr>
                        <td data-title="Sr.No."><?php echo $i; ?></td>
						 ...
						 <td data-title="Teacher Count"> <a href="teachingProcess_Given.php?sc_id=<?php echo $a;?>&year=<?php echo $year;?>&group_name=<?php echo $group_name;?>" > <?php
...
from tbl_360feedback_template ft where ft.feed360_school_id='$a' $cond1 )";
