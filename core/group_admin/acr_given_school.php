...
from tbl_360_activities_data sf join tbl_teacher t on t.t_id= sf.tID where sf.activity_level_id='4' and  (sf.Emp_type_id ='137') )";
	...
	elseif (($year!='' and $year!='ALL') and ($group_member_id=='' or $group_member_id=='ALL')){
		...
		from tbl_360_activities_data sf join tbl_teacher t on t.t_id= sf.tID where 
		 sf.Academic_Year='$year' and sf.activity_level_id='4' and  (sf.Emp_type_id ='137') )
		LIMIT $start_from, $webpagelimit");
...
where sf.activity_level_id='4' and (sf.Emp_type_id ='137') and $cond
group by sc.school_id
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
			...
			<?php if($_GET['year']){ ?>
				 window.location.assign('acr_given_school.php?year=<?php echo $year;?>&group_name=<?php echo $group_name;?>&page='+page);
			<?php } else {?>
				window.location.assign('acr_given_school.php?page='+page);
			<?php } ?>
        });
    });
</script>
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('acr_given_school.php?Search=<?php echo $searchq; ?>&spage='+page);
        });
    });
</script>
<?php }?>
                ...
                while($result = mysql_fetch_array($query1)) {
                    ...
                    <tr>
                        <td data-title="Sr.No."><?php echo $i; ?></td>
						 ...
						 <td data-title="Teacher Count"> <a href="acr360_given.php?sc_id=<?php echo $a;?>&year=<?php echo $year;?>&group_name=<?php echo $group_name;?>" > <?php
						 if ($_GET['year'] and $_GET['year']!='ALL' and $_GET['year']!=''){
							$cond1="and Academic_Year='$year'";
						} else { $cond1=""; }
						$query=" SELECT count(distinct t.t_id) as cnt FROM tbl_teacher t
						where t.school_id='$a' and t.t_emp_type_pid ='137' and  t.t_id  IN
						(select sf.tID
					   from tbl_360_activities_data sf where sf.schoolID='$a' and sf.activity_level_id='4' $cond1 )";
                    ...
                    <tr>
                        ...
                        <td data-title="School Name"><?php echo $result['school_name']; ?></td>
						 ...
						 if ($_GET['year'] and $_GET['year']!='ALL' and $_GET['year']!=''){
							...
							where t.school_id='$a' and t.t_emp_type_pid ='137' and  t.t_id  IN
							(select sf.tID
						   from tbl_360_activities_data sf where sf.schoolID='$a' and sf.activity_level_id='4' $cond2 )";
							$sql=mysql_query($query);
							$result=mysql_fetch_array($sql);
						    echo $result['cnt'];?></td>
                        </tr>
