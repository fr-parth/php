...
$start_from = ($page-1) * $webpagelimit;
	...
	SELECT distinct t.t_id,t.t_complete_name,t.t_dept ,t.school_id,t.t_academic_year
	  ...
	  and t.t_emp_type_pid = '137' and t.t_id  IN
	   (select sf.tID
	  from tbl_360_activities_data sf where sf.activity_level_id='4' and  (sf.Emp_type_id ='137') and sf.schoolID='$sc_id' $cond)
...
and
 ...
 t.t_academic_year  LIKE '%$searchq%'
	...
	and t.t_emp_type_pid = '137' and t.t_id  IN
	 (select sf.tID
	from tbl_360_activities_data sf where sf.activity_level_id='1' and  (sf.Emp_type_id  = '137') and sf.schoolID='$sc_id' $cond)
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
			...
			<?php if($_GET['year']){ ?>
				 window.location.assign('acr360_given.php?sc_id=<?php echo $sc_id;?>&year=<?php echo $year;?>&group_name=<?php echo $group_name;?>&page='+page);
			<?php } else {?>
				window.location.assign('acr360_given.php?sc_id=<?php echo $sc_id;?>&page='+page);
			<?php } ?>
        });
    });
</script>
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			<?php if($_GET['year']){ ?>
				 window.location.assign('acr360_given.php?sc_id=<?php echo $sc_id;?>&year=<?php echo $year;?>&group_name=<?php echo $group_name;?>&Search=<?php echo $searchq; ?>&spage='+page);
			<?php } else {?>
				window.location.assign('acr360_given.php?sc_id=<?php echo $sc_id;?>&Search=<?php echo $searchq; ?>&spage='+page);
			<?php } ?>
        });
    });
</script>
...
<div align="center">
    <div class="container" style="width:100%;">
        <div style="padding-top:30px;">
            ...
            <h2 style="padding-left:20px; margin-top:2px;color:#666"> ACR Activity- Given</h2>
     ...
     <?php } ?>
			...
			<input type="button" class="btn btn-info" value="Reset" onclick="window.open('acr360_given.php?sc_id=<?php echo $_GET['sc_id'];?>&year=<?php echo $_GET['year'];?>&group_name=<?php echo $group_name;?>','_self')" />
			</div>
				</form>
						 </div>
  					<form method="post" class="pull-left" action="acr_given_school.php?year=<?php echo $year?>&group_name=<?php echo $group_name;?>">
