...
$sql1 ="select COUNT(t.id) from tbl_teacher t inner join tbl_group_school gs on t.school_id= gs.school_id where gs.group_member_id = '$group_member_id' AND t_emp_type_pid IN (133,134,135,137) order by t.school_id";  
...
$colname=$_GET['colname'];
	if ($colname !=''and $colname != 'Select')
	{
		...
		t_name,t_complete_name,t_middlename,t_lastname,t.school_id,t_current_school_name,t_email,t_phone,t_address from tbl_teacher t inner join tbl_group_school gs on t.school_id= gs.school_id where gs.group_member_id = '$group_member_id' AND t_emp_type_pid IN (133,134,135,137) AND $colname LIKE '%$searchq%' order by t.school_id LIMIT $start_from, $webpagelimit") or die("could not Search!");
		}else{
			$query1=mysql_query("SELECT t_id,
		t_name,t_complete_name,t_middlename,t_lastname,t.school_id,t_current_school_name,t_email,t_phone,t_address from tbl_teacher t inner join tbl_group_school gs on t.school_id= gs.school_id where gs.group_member_id = '$group_member_id' AND t_emp_type_pid IN (133,134,135,137) AND $colname LIKE '%$searchq%' order by t.school_id LIMIT $start_from, $webpagelimit") or die("could not Search!");
		}			
		$sql1 ="select COUNT(t.id) from tbl_teacher t inner join tbl_group_school gs on t.school_id= gs.school_id where gs.group_member_id = '$group_member_id' AND t_emp_type_pid IN (133,134,135,137) AND $colname LIKE '%$searchq%' order by t.school_id "; 
					$rs_result = mysql_query($sql1);  
					$row1 = mysql_fetch_row($rs_result);  
					$total_records = $row1[0];  
					$total_pages = ceil($total_records / $webpagelimit);
...
<div class="container" style="width:100%;">
    <div class="row" style="padding-top:10px;height:30px;" align="center">
        <h2 style="margin-top:2px;color:#666"> List of <?php echo $dynamic_teacher;?></h2>
    </div>
