		 ...
		 if(isset($_POST['submit']))
				...
				if($coupon_id!="")
			{
			 ...
			 // mysql_query("update tbl_giftof_rewardpoint set parent_id='$parent_id' where coupon_id='$coupon_id'");
			  ...
			  $point=$arr['amount'];
			  	$rows=mysql_query("select * from tbl_parent where id='$parent_id'");
			  $arrs=mysql_fetch_array($rows);
			  $balance_point=$arrs['balance_blue_points'];
			  $balance_point=$balance_point+$point;
			$status='Used';
			//SMC-3576 modify by Pravin 2018-10-11
        		        	//$date1=date('d/m/Y');
        		        	$date1=CURRENT_TIMESTAMP;
			   mysql_query("update tbl_parent set  balance_blue_points='$balance_point' where id='$parent_id' ");
			   mysql_query("update  tbl_giftcards  set  amount='0' ,status='$status' where card_no='$coupon_id' ");
			   			   mysql_query("insert into tbl_giftof_bluepoint(coupon_id,points,issue_date,entities_id,user_id)values('$coupon_id','$point','$date1','106','$parent_id')");
			  if(mysql_affected_rows()>0)
			  {
			  	$report="You  got ".$point. " Blue Points";
			  }
			  else
			  {
    ...
    <div class="row">
            ...
            <div class="container" style="padding:10px;background-color:#FFFFFF;border:1px solid #CCCCCC;box-shadow:0px 1px 3px 1px #C3C3C4;" align="center">
                <div class="row"  align="center">
                    ...
                    <div class="row" style="padding:10px;color:#0066FF;font-weight:bold;font-size:32px;">
                    <?php $rows=mysql_query("select * from tbl_parent  where id='$parent_id'");
 ...
 if(!isset($id))
	{
		header('location:login.php');
		 ...
		 if(isset($_POST['submit']))
			...
			$redirect=$GLOBALS['URLNAME'].'/core/scadmin_bluepoint_coupon.php';
			...
			if($responseStatus==200)
				{														
					echo "<script>alert('$points Blue points purchased successfully');location.assign('$redirect');</script>";	
				}
				else						
				{														
		...
		/*		if($coupon_id!="")
			{
			 ...
			 // mysql_query("update tbl_giftof_rewardpoint set parent_id='$parent_id' where coupon_id='$coupon_id'");
			  ...
			  $point=$arr['amount'];
				...
				{
			  	$rows=mysql_query("select * from tbl_school_admin where id='$parent_id'");
			  $arrs=mysql_fetch_array($rows);
			  $balance_point=$arrs['balance_blue_points'];
			  $balance_point=$balance_point+$point;
			$status='Used';
			//SMC-3576 modify by Pravin 2018-10-11
        		        	//$date1=date('d/m/Y');
        		        	$date1=CURRENT_TIMESTAMP;
			   mysql_query("update tbl_school_admin set  balance_blue_points='$balance_point' where id='$parent_id' ");
			   mysql_query("update  tbl_giftcards  set  amount='0' ,status='$status' where card_no='$coupon_id' ");
			   			   mysql_query("insert into tbl_giftof_bluepoint(coupon_id,points,issue_date,entities_id,user_id)values('$coupon_id','$point','$date1','102','$parent_id')");
			  if(mysql_affected_rows()>0)
			  {
			  	$successreport="You  got ".$point. " Blue Points";
			  }
			  else
			  {
			  	$report="Coupon is Invalid";
                      ...
                      <div class="col-md-10 col-md-offset-1">
						...
						{ echo  $values['issue_date'];  } ?></div>																	                                       
                                      ...
                                      <div class="col-md-6 col-md-offset-3"  align="center" >
											...
											<div style="color:#006400;"><?php echo $successreport;?></div>
