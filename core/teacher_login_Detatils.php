...
$start_from = ($fpage-1) * $webpagelimit;
   ...
   if ($info == "1") {
		...
		$sq = mysql_query("Select vm.t_complete_name,ls.EntityID,ls.school_id,ls.LatestLoginTime,ls.LogoutTime from tbl_LoginStatus
 ls left join 
tbl_teacher vm on vm.id=ls.EntityID and vm.school_id=ls.school_id  
 where ls.Entity_type=103 order by RowID desc
LIMIT $start_from, $webpagelimit");

$sq1 ="Select count(ls.Entity_type),vm.t_complete_name,ls.EntityID,ls.school_id,ls.LatestLoginTime,ls.LogoutTime from tbl_LoginStatus
 ls left join 
tbl_teacher vm on vm.id=ls.EntityID and vm.school_id=ls.school_id  
 where ls.Entity_type=103 order by RowID desc";
...
$sq = mysql_query("Select vm.t_complete_name,ls.EntityID,ls.school_id,ls.LatestLoginTime,ls.LogoutTime from tbl_LoginStatus
 ls left join 
tbl_teacher vm on vm.id=ls.EntityID and vm.school_id=ls.school_id  
 where ls.Entity_type=103  AND ls.LatestLoginTime like '%$date%' order by RowID desc
LIMIT $start_from, $webpagelimit");
...
tbl_teacher vm on vm.id=ls.EntityID and vm.school_id=ls.school_id  
 ...
 ls left join 
tbl_teacher vm on vm.id=ls.EntityID and vm.school_id=ls.school_id  
 where ls.Entity_type=103 AND (ls.LatestLoginTime BETWEEN  '$week' AND '$date1') 
 order by RowID desc LIMIT $start_from, $webpagelimit");
...
tbl_teacher vm on vm.id=ls.EntityID and vm.school_id=ls.school_id  
 ...
 ls left join 
tbl_teacher vm on vm.id=ls.EntityID and vm.school_id=ls.school_id  
 where ls.Entity_type=103 AND (LatestLoginTime BETWEEN  '$first_day_this_month' AND 
 '$last_day_this_month') order by RowID desc";  
 $count=mysql_num_rows($sq); 
$rs_result = mysql_query($sq1);  
					$row1 = mysql_fetch_row($rs_result);  
					$total_records = $row1[0];  
 ...
 ls left join 
tbl_teacher vm on vm.id=ls.EntityID and vm.school_id=ls.school_id  
 where ls.Entity_type=103 AND (LatestLoginTime BETWEEN  '$from' AND '$to') 
 order by RowID desc LIMIT $start_from, $webpagelimit");
...
$sql=mysql_query("Select vm.t_complete_name,ls.EntityID,ls.school_id,ls.LatestLoginTime,ls.LogoutTime from tbl_LoginStatus
 ls left join 
tbl_teacher vm on vm.id=ls.EntityID and vm.school_id=ls.school_id  
 where ls.Entity_type=103 order by RowID desc
LIMIT $start_from, $webpagelimit");	

$sql1 ="Select count(ls.Entity_type),vm.t_complete_name,ls.EntityID,ls.school_id,ls.LatestLoginTime,ls.LogoutTime from tbl_LoginStatus
 ls left join 
tbl_teacher vm on vm.id=ls.EntityID and vm.school_id=ls.school_id  
 where ls.Entity_type=103 order by RowID desc"; 
...
//$colname=mysql_real_escape_string($_GET['colname']);
	if ($searchq != '')
	{ 
		$query1=mysql_query("Select vm.t_complete_name,ls.EntityID,ls.school_id,
		ls.LatestLoginTime,ls.LogoutTime from tbl_LoginStatus
 ls inner join 
tbl_teacher vm on vm.id=ls.EntityID and vm.school_id=ls.school_id  
 where ls.Entity_type=103  and
 
  vm.t_complete_name LIKE '%$searchq%' or ls.EntityID LIKE '%$searchq%'
  ...
  LIMIT $start_from, $webpagelimit") or die("could not Search!");
			...
			$sql1 ="Select count(vm.t_complete_name),ls.EntityID,ls.school_id,
		ls.LatestLoginTime,ls.LogoutTime from tbl_LoginStatus
 ls inner join 
tbl_teacher vm on vm.id=ls.EntityID and vm.school_id=ls.school_id  
 where ls.Entity_type=103  and
...
$query1=mysql_query("Select vm.t_complete_name,ls.EntityID,ls.school_id,
 ...
 ls inner join 
tbl_teacher vm on vm.id=ls.EntityID and vm.school_id=ls.school_id  
 where ls.Entity_type=103  and  $colname LIKE '%$searchq%' order by RowID desc
...
<?php if ($_GET['find']){?>
    ...
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
			...
			window.location.assign('teacher_login_Detatils.php?info=<?php echo $info;?>&find=<?php echo Find;?>&from=<?php echo $from;?>&to=<?php echo $to;?>&fpage='+page);
        });
    });
</script>
<?php }
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('teacher_login_Detatils.php?page='+page);
        });
    });
</script>
<?php }else{
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('teacher_login_Detatils.php?Search=<?php echo $searchq; ?>&spage='+page);
        });
    });
</script>
<?php }?>
...
<div align="center">
    <div class="container" style="width:100%;">
        <div style="padding-top:30px;">
            ...
            <h2 style="padding-left:20px; margin-top:2px;color:#666"> Teacher Login Logout Details</h2>
...
<!-- From and To date controls added by Pranali for task SMC-2235 on 05-07-2018-->
		  ...
		  <div class='row' style="margin-left:270px;">
			...
			<div class="col-md-1" >
			<input type="button" class="btn btn-info" value="Reset" onclick="window.open('teacher_login_Detatils.php','_self')" />
			</div>
