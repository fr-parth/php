<?php
/*Modified by Rutuja to add 'address' column instead of 'add' in table 'tbl_cookie_adminstaff' to solve the issue of Cookie Admin details not getting updated for SMC-4598 on 27/03/2020*/
...
if (isset($_POST['update'])) {
    ...
    $updatestaff = "UPDATE tbl_cookie_adminstaff SET `stf_name`='$name',`exprience` ='$experience',`designation`='$designation',`address`='$address',`country`='$country',`city`='$city',`statue`='$state',`dob`='$date',`age`='$age',`gender`='$gender',`email`='$email',`phone`='$phone',`qualification`='$education' WHERE  `id`='$staffid'";
...
*/		
        function valid() {
			...
			{
				document.getElementById('errorexperience').innerHTML = 'Please Enter Experience between 0 to 99';
...
*/			
				...
				if(id_phone==null||id_phone=="")
            {
				...
				regx2=/^[123456789]\d{9}$/;
                //validation for name
                if(!regx2.test(id_phone) || !regx2.test(id_phone))
                {
                document.getElementById('errorphone').innerHTML='Please Enter valid Phone Number';
                    return false;
                }
