...
if (!($_GET['Search'])) {
    ...
    $sql=mysql_query("SELECT  t.t_complete_name,p.user_id,p.date,p.school_id ,s.rewardType FROM purcheseSoftreward 
    as p 
    ...
    $sql1 ="SELECT count(t.t_complete_name),p.user_id,p.date,p.school_id ,s.rewardType FROM purcheseSoftreward 
    as p 
    inner join softreward as s on p.reward_id=s.softrewardId 
     inner join tbl_teacher t on t.id=p.user_id and p.school_id=t.school_id
    where (p.date BETWEEN '$from' AND '$to') and userType='Teacher'  and  t.school_id='$school_id' ";  
		...
		<script type="text/javascript">
			$(function() {
				...
				window.pagObj = $('#pagination').twbsPagination({
					...
					window.location.assign('soft_reward_teacher_stat.php?from=<?php echo $from; ?>&to=<?php echo $to; ?>&page='+page);
				});
			});
		</script>
	<?php } else {
		...
		<script type="text/javascript">
			$(function() {
				...
				window.pagObj = $('#pagination').twbsPagination({
					...
					console.info(page + '(from event listening)');
					window.location.assign('soft_reward_teacher_stat.php?Search=<?php echo $searchq; ?>&from=<?php echo $from; ?>&to=<?php echo $to; ?>&spage='+page);
				});
			});
		</script>
	<?php } ?>
	...
	<div align="center">
		<div class="container" style="width:100%;">
			<div style="padding-top:30px;">
				...
				<!-- $dynamic_school_admin added by Sayali Balkawade for SMC-4254 on 11/12/2019 -->
                <h2 style="padding-left:20px; margin-top:2px;color:#666">Soft Reward Purchased By <?php echo $dynamic_teacher ?></h2>
