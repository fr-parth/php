...
$AcademicYear=mysql_query("SELECT * FROM tbl_academic_Year where school_id='$school_id' and Enable='1' ");
    $year=mysql_fetch_array($AcademicYear);
if(isset($_POST['submit'])){
    $Academic_Year= $_POST['Academic_Year'];
} 
else if($_SESSION['AcademicYear']!='')
{
...
//echo $selection_value;
   ...
   if($selection_value=="All"){
       $query="select t_emp_type_pid,id,t_designation, t_id, t_complete_name, t_name, t_middlename, t_lastname, t_email,t_phone, t_dept,t_DeptCode ,t_academic_year,t_date_of_appointment, t_pc,school_id from tbl_teacher where school_id='$sc_id' $where order by t_complete_name ASC";
       $sql=mysql_query($query) or die("Could not Search!"); 
   } 
   else if($selection_value=="1"){
    $query="select t_emp_type_pid,id,t_designation, t_id, t_complete_name, t_name, t_middlename, t_lastname, t_email,t_phone, t_dept,t_DeptCode ,t_academic_year,t_date_of_appointment,t_pc,school_id from tbl_teacher where school_id='$sc_id' and is_active='1' $where order by t_complete_name ASC";
     
     $sql=mysql_query($query);
   }
   else if($selection_value=="0"){
       
       $query ="select t_emp_type_pid,id,t_designation, t_id, t_complete_name, t_name, t_middlename, t_lastname, t_email,t_phone, t_dept,t_DeptCode ,t_academic_year,t_date_of_appointment,t_pc,school_id from tbl_teacher where school_id='$sc_id' and is_active='0' $where order by t_complete_name ASC";  
       $sql=mysql_query($query);
   }
   else{
     $query="select t_emp_type_pid,id,t_designation, t_id, t_complete_name, t_name, t_middlename, t_lastname, t_email,t_phone, t_dept,t_DeptCode ,t_academic_year,t_date_of_appointment, t_pc,school_id, t_academic_year from tbl_teacher where school_id='$sc_id' AND t_academic_year<='$Academic_Year' $where order by t_complete_name ASC";
  $sql=mysql_query($query);
   }
}
else{   
                ...
                <div style="background-color:#F8F8F8 ;">
                    <div class="row">
                        ...
                        <select name="Academic_Year" class=" form-control" style="text-indent:30%">
                            <?php $select_option_value= $_POST['Academic_Year'] ?>
                   ...
                   <?php  
                                                ...
                                                <?php  }else{ ?>
                                                 ...
                                                 <?php  }  ?> 
                                                    <?php 
                                                $sql1="SELECT ExtYearID,Academic_Year, Enable FROM tbl_academic_Year where school_id='$school_id' and ExtYearID != '' group by Academic_Year";
                   ...
                   <option value="1" <?php if($select_option_value == "1") echo 'selected="selected"'; ?>>Active</option>
                 
                 <option value="0" <?php if($select_option_value == "0") echo 'selected="selected"'; ?>>Inactive</option>
                </select>
                            ...
                            <table id="example" class="table table-bordered">
                                <thead>
                                    <tr style="background-color: #428BCA; color: #FFF;">
                                        ...
                                        <th>Email/ Phone</th>
                                        <th>Department /</br>Department Code</th>
                                        <th>Academic Year / </br>Date of Appointment</th>
                                        <th>Designation                                       
                                ...
                                <tbody>
                                    ...
                                    <?php
                                     ...
                                     $k=1; while($row=mysql_fetch_array($sql)){
                                        //print_r($row);exit;
                                        $teacher_id = $row['id'];
                                        $t_id = $row['t_id'];
                                        $DeptID=$row['t_DeptID'];
                                    ...
                                    <tr>
                                       ...
                                       <td>
                                          <?php echo $row['t_academic_year']; ?>/
                                          <?php echo $row['t_date_of_appointment']; ?>
                                            </td>
                                        <td>
