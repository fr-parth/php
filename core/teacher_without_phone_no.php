...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT  t_id,t_complete_name, t_email, t_phone FROM tbl_teacher 
						where (t_phone is null or t_phone = '') and (t_emp_type_pid =133 OR t_emp_type_pid=134  or t_emp_type_pid=135 or t_emp_type_pid=137)  and school_id='$school_id'  and t_academic_year='$Academic_Year' 
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
		...
		if ($colname != ''and $colname != 'Select')
				{
			$query1=mysql_query(" SELECT  t_id,t_complete_name, t_email, t_phone FROM tbl_teacher 
									WHERE (t_phone is null or t_phone = '')
				...
				$sql1 =mysql_query("SELECT id FROM tbl_teacher 
									WHERE  (t_phone ='' or t_phone is null ) 
                                    AND (t_emp_type_pid = 133 OR t_emp_type_pid = 134  or t_emp_type_pid=135 or t_emp_type_pid=137)
									AND school_id='$school_id' and t_academic_year='$Academic_Year' 
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('teacher_without_phone_no.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('teacher_without_phone_no.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<!-- Headline -->
	<div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white"><?php echo $dynamic_teacher;?> Data without Phone No. (<?php echo $total_records; ?>)</h2>
			</div>
...
<!--End of dropdown ----------------------- --- -->
		...
		<form>
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('teacher_without_phone_no.php','_self')" />
			</div>	
        </form>
		<!-- search form END -->	
