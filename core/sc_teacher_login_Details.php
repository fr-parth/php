...
//$entity_id added and checked for Entity_type in all queries by Pranali for SMC-4629 on 10-4-20
...
ls.EntityID=t.id where Entity_type='".$entity_id."' and ls.school_id='$school_id' ORDER BY ls.RowID DESC LIMIT $start_from, $webpagelimit") or die("Could not Search!");
...
ls.EntityID=t.id where Entity_type='".$entity_id."' and ls.school_id='$school_id' ORDER BY ls.RowID DESC");
...
else
{
	...
	//$colname=mysql_real_escape_string($_GET['colname']);
			...
			$sql = mysql_query("SELECT ls.EntityID,ls.school_id,ls.LatestLoginTime,ls.LogoutTime,t.t_complete_name from tbl_LoginStatus  ls join tbl_teacher t on
			ls.EntityID=t.id where Entity_type='".$entity_id."' and ls.school_id='$school_id' AND (ls.EntityID LIKE '%$searchq%' or ls.school_id LIKE '%$searchq%' or ls.LatestLoginTime LIKE '%$searchq%' or ls.LogoutTime LIKE '%$searchq%'  or t.t_complete_name LIKE '%$searchq%') ORDER BY ls.RowID DESC LIMIT $start_from, $webpagelimit") or die("Could not Search!");
							
			$sql1 = mysql_query("SELECT ls.EntityID,ls.school_id,ls.LatestLoginTime,ls.LogoutTime,t.t_complete_name from tbl_LoginStatus  ls join tbl_teacher t on
			ls.EntityID=t.id where Entity_type='".$entity_id."' and ls.school_id='$school_id' AND (ls.EntityID LIKE '%$searchq%' or ls.school_id LIKE '%$searchq%' or ls.LatestLoginTime LIKE '%$searchq%' or  ls.LogoutTime LIKE '%$searchq%' or t.t_complete_name LIKE '%$searchq%') ORDER BY ls.RowID DESC  ");
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
			...
			window.location.assign('sc_teacher_login_Details.php?page='+page);
        });
    });
</script>
<?php }else{
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('sc_teacher_login_Details.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
        });
    });
</script>
<?php }?>
...
<div align="center">
    <div class="container" style="width:100%;">
		...
		<form style="margin-top:5px;">
			...
			<div class="col-md-1" >
			<input type="button" class="btn btn-info" value="Reset" onclick="window.open('sc_teacher_login_Details.php','_self')" />
			</div>
			<div class="col-md-1" >
