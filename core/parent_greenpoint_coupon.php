	...
	//$cp_id="";
		 ...
		 if(isset($_POST['submit']))
			 ...
			 // mysql_query("update tbl_giftof_rewardpoint set parent_id='$parent_id' where coupon_id='$coupon_id'");
                  ...
                  {
					  $row1=mysql_query("SELECT * FROM tbl_giftof_waterpoint where coupon_id='$cp_id'");
					//  echo "SELECT * FROM tbl_giftof_waterpoint where coupon_id='$cp_id'";
					//  echo mysql_num_rows($row1);
					  if(mysql_num_rows($row1)>0)
                                {
								//	echo "if";
							    	$report="coupon is already used";
								}
					  else
					  {
						 // echo "else";
        			      ...
        			      $balance_point=$arrs['balance_points'];
        			      $balance_point=$balance_point+$point;
        		        	$status='Used';
						//SMC-3576 modify by Pravin 2018-10-11
        		        	//$date1=date('d/m/Y');
        		        	$date1=CURRENT_TIMESTAMP;
        			    mysql_query("update tbl_parent set  balance_points='$balance_point' where Id='$parent_id' ");
        			    ...
        			    mysql_query("insert into tbl_giftof_waterpoint(coupon_id,points,issue_date,entities_id,user_id)
								values('$coupon_id','$point','$date1','106','$parent_id')");

			        	$report1="You  got ".$point. " Water Points";
					  }
                }
			  else
			  {
    ...
    <div class="row">
            ...
            <div class="container" style="padding:10px;background-color:#FFFFFF;border:1px solid #CCCCCC;box-shadow:0px 1px 3px 1px #C3C3C4;" align="center">
                <div class="row"  align="center">
                    ...
                    <div class="row" style="padding:10px;color:#1ca3ec;font-weight:bold;font-size:32px;">
                    <?php $rows=mysql_query("select * from tbl_parent  where Id='$parent_id'");
					
					       $value=mysql_fetch_array($rows);
						   
					?>
                            <?php echo $value['balance_points']; ?>
                    </div>
           ...
           <div class="container" style="padding:10px;background-color:#FFFFFF;box-shadow:0px 1px 3px 1px #C3C3C4;" align="center ">
            ...
            <?php
			            if(isset($_POST['Search']))
						{
								...
								$row1=mysql_query("SELECT * FROM tbl_giftcards where card_no='$cp_id'");
                                if(mysql_num_rows($row1)>0)
                                {
							    	$values=mysql_fetch_array($row1);
								
                               } else{
                                                $cp_id="";
                                                $report="No coupon is available for this coupon ID. Please Enter valid Coupon Id." ;
                                       }
                      ...
                      <div class="col-md-10 col-md-offset-1">
						...
						{ echo  $values['issue_date'];  } ?></div>																	                                       
                                     ...
                                     <?php
										...
										?></div> <?php }   ?>
										<div class="col-md-6 col-md-offset-3"  align="center" style="color:#006400;">
