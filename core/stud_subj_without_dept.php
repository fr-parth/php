...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT Stud_Member_Id,ExtSemesterId,subjectName,student_id,subjcet_code,Department_id 
	...
	$sql1=" SELECT id,Stud_Member_Id,ExtSemesterId,subjectName,student_id,subjcet_code,Department_id 
			FROM tbl_student_subject_master 
			WHERE (Department_id = '' OR Department_id is NULL)
			AND school_id = '$school_id' and AcademicYear='$Academic_Year' ";
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };  
		...
		if ($colname != ''and $colname != 'Select')
				{
			$query1=mysql_query("SELECT Stud_Member_Id,ExtSemesterId,subjectName,student_id,subjcet_code,Department_id
								FROM tbl_student_subject_master 
								WHERE (Department_id = '' OR Department_id is NULL)
									AND school_id = '$school_id' and AcademicYear='$Academic_Year'
									AND `$colname` LIKE '%$searchq%' LIMIT $start_from, $limit") or die("could not Search!");

				$sql1 =mysql_query("SELECT id,Stud_Member_Id,ExtSemesterId,subjectName,student_id,subjcet_code,Department_id 
				                	FROM tbl_student_subject_master 
									WHERE (Department_id = '' OR Department_id is NULL)
										AND school_id = '$school_id' and AcademicYear='$Academic_Year'
				...
				$query1=mysql_query("SELECT Stud_Member_Id,ExtSemesterId,subjectName,student_id,subjcet_code,Department_id
									...
									WHERE (Department_id = '' OR Department_id is NULL)
										AND school_id = '$school_id' and AcademicYear='$Academic_Year'
										AND (Department_id LIKE '$searchq%' or Stud_Member_Id LIKE '$searchq%' or ExtSemesterId LIKE '$searchq%' or subjectName LIKE '$searchq%' or subjcet_code LIKE '$searchq%' or subjectName LIKE '$searchq%'  or student_id LIKE '$searchq%' ) LIMIT $start_from, $limit") or die("could not Search!");
			
				$sql1 =mysql_query("SELECT id,Stud_Member_Id,ExtSemesterId,subjectName,student_id,subjcet_code,Department_id 
									FROM tbl_student_subject_master 
									WHERE (Department_id = '' OR Department_id is NULL)
										AND school_id = '$school_id' and AcademicYear='$Academic_Year'
										AND (Department_id LIKE '$searchq%' or Stud_Member_Id LIKE '$searchq%' or subjectName LIKE '$searchq%' ExtSemesterId LIKE '$searchq%' or or subjcet_code LIKE '$searchq%' or subjectName LIKE '$searchq%'  or student_id LIKE '$searchq%' ) ") or die("could not Search!");   
  	...
  	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('stud_subj_without_dept.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('stud_subj_without_dept.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
	});
	});
</script>
<?php }?>
...
<body>
	...
	<div class="container">
		<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white">Student Subject Data without Department (<?php echo $total_records; ?>)</h2>
		</div>
...
<!--End of dropdown ----------------------- --- -->
	...
	<!-- search form -->
		...
		<form>
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('stud_subj_without_dept.php','_self')" />
			</div>
						...
						<tbody>
							...
							?>
								<tr>
									...
									<td> <center>
                     						<a style="cursor: pointer;" href="edit_student_subject_data_quality.php?id=<?php echo $row['id']; ?>&FLAG=Department" ><span class="glyphicon glyphicon-pencil"></span></a>
										</center>
									</td>
								</tr>
							<?php $i++;} ?>     
