...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT Semester_Id,  Semester_Name, class , Department_Name 
						...
						WHERE (Semester_Name  ='' or Semester_Name is null ) and School_ID='$school_id'
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
		...
		if ($colname != ''and $colname != 'Select')
			...
			$sql1=mysql_query("SELECT Semester_Id,  Semester_Name, class , Department_Name 
								FROM tbl_semester_master 
								WHERE (Semester_Name  ='' or Semester_Name is null ) 
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('sem_without_semname.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('sem_without_semname.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<form>
	...
	<div class="col-md-2" style="width:17%;">
		<select name="colname" class="form-control">
			...
			<?php if (($_GET['colname']) == "Department_Name ") {
	...
	<div class="col-md-1">
	<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('sem_without_semname.php','_self')" />
	</div>	
</form>
<!-- search form END -->	
