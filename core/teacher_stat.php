...
if (!($_GET['Search'])) {
	...
	$sql = mysql_query("SELECT ls.EntityID,ls.school_id,ls.LatestLoginTime,ls.LogoutTime,t.t_complete_name from tbl_LoginStatus  ls join tbl_teacher t on
	ls.EntityID=t.id where (ls.LatestLoginTime BETWEEN '$from' AND '$to')  and  Entity_type='103' and ls.school_id='$school_id' ORDER BY ls.RowID DESC LIMIT $start_from, $webpagelimit") or die("Could not Search!");

	$sql1 = mysql_query("SELECT ls.EntityID,ls.school_id,ls.LatestLoginTime,ls.LogoutTime,t.t_complete_name from tbl_LoginStatus  ls join tbl_teacher t on
	ls.EntityID=t.id where (ls.LatestLoginTime BETWEEN '$from' AND '$to')  and  Entity_type='103' and ls.school_id='$school_id' ORDER BY ls.RowID DESC");
	...
	//$dynamic_school_admin added by Sayali Balkawade for SMC-4254 on 11/12/2019
	$sql = mysql_query("SELECT ls.EntityID,ls.school_id,ls.LatestLoginTime,ls.LogoutTime,t.t_complete_name from tbl_LoginStatus  ls join tbl_teacher t on
	ls.EntityID=t.id where (ls.LatestLoginTime BETWEEN '$from' AND '$to')  and  Entity_type='103' and ls.school_id='$school_id' AND (ls.EntityID LIKE '%$searchq%' or ls.school_id LIKE '%$searchq%' or ls.LatestLoginTime LIKE '%$searchq%' or ls.LogoutTime LIKE '%$searchq%'  or t.t_complete_name LIKE '%$searchq%') ORDER BY ls.RowID LIMIT $start_from, $webpagelimit") or die("Could not Search!");
	$sql1 = mysql_query("SELECT ls.EntityID,ls.school_id,ls.LatestLoginTime,ls.LogoutTime,t.t_complete_name from tbl_LoginStatus  ls join tbl_teacher t on
	ls.EntityID=t.id where (ls.LatestLoginTime BETWEEN '$from' AND '$to')  and  Entity_type='103' and ls.school_id='$school_id' AND (ls.EntityID LIKE '%$searchq%' or ls.school_id LIKE '%$searchq%' or ls.LatestLoginTime LIKE '%$searchq%' or  ls.LogoutTime LIKE '%$searchq%' or t.t_complete_name LIKE '%$searchq%') ORDER BY ls.RowID DESC  ");
	$total_records = mysql_num_rows($sql1);
	$total_pages = ceil($total_records / $webpagelimit);
		...
		<script type="text/javascript">
			$(function() {
				...
				window.pagObj = $('#pagination').twbsPagination({
					...
					window.location.assign('teacher_stat.php?from=<?php echo $from; ?>&to=<?php echo $to; ?>&page=' + page);
				});
			});
		</script>
	<?php } else {
		...
		<script type="text/javascript">
			$(function() {
				...
				window.pagObj = $('#pagination').twbsPagination({
					...
					console.info(page + '(from event listening)');
					window.location.assign('teacher_stat.php?Search=<?php echo $searchq; ?>&from=<?php echo $from; ?>&to=<?php echo $to; ?>&spage=' + page);
				});
			});
		</script>
	<?php } ?>
	...
	<div align="center">
		<div class="container" style="width:100%;">
			<div style="padding-top:30px;">
				...
				<!-- $dynamic_school_admin added by Sayali Balkawade for SMC-4254 on 11/12/2019 -->
				<h2 style="padding-left:20px; margin-top:2px;color:#666"> <?php echo $dynamic_teacher; ?> Login Logout Details</h2>
			...
			<input type="button" class="btn btn-info" value="Reset" onclick="window.open('green_point_distributed_by_cookieadmin.php','_self')" />
					 ...
					 <div style="margin-left: 800px;">
						...
						<input type="button" value="Reset" onclick="window.open('teacher_stat.php?from=<?php echo $from; ?>&to=<?php echo $to; ?>','_self')" />
					</div> -->
				</form>
			</div>
