...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT id,Dept_code,Dept_Name,Email_Id
	FROM tbl_department_master 
	where(Email_Id ='' or Email_Id is null or Email_Id NOT REGEXP  '^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,3}$') 
	and School_ID='$school_id' 
						LIMIT $start_from, $limit") 
						or die("could not Search!");
 
	$sql1=" SELECT id FROM tbl_department_master
			where(Email_Id ='' or Email_Id is null ) and School_ID='$school_id' ";
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
		...
		if ($colname != ''and $colname != 'Select')
				{
			$query1=mysql_query("SELECT id,Dept_code,Dept_Name,Email_Id FROM tbl_department_master where(Email_Id ='' or Email_Id is null ) and School_ID='$school_id'
		
									AND `$colname` LIKE '%$searchq%' LIMIT $start_from, $limit") or die("could not Search!");

				$sql1 =mysql_query("SELECT id FROM tbl_department_master
									WHERE  (Email_Id='' or Email_Id is null ) 
									AND School_ID='$school_id'
				...
				$query1=mysql_query("SELECT id,Dept_code,Dept_Name,Email_Id FROM tbl_department_master where(Email_Id ='' or Email_Id is null ) and School_ID='$school_id'
									...
									AND (id LIKE '$searchq%' or Dept_code LIKE '$searchq%' or Dept_Name LIKE '$searchq%'  or Email_Id LIKE '$searchq%') LIMIT $start_from, $limit") or die("could not Search!");
			
				$sql1 =mysql_query("SELECT id FROM tbl_department_master
                					WHERE  (Email_Id='' or Email_Id is null ) 
               					 	AND School_ID='$school_id' 
									AND (id LIKE '$searchq%' or Dept_code LIKE '$searchq%' or Dept_Name LIKE '$searchq%' or Email_Id LIKE '$searchq%' )") or die("could not Search!");   
...
//$result = mysql_query("SELECT id,Dept_code,Dept_Name,Email_Id FROM tbl_department_master where(Email_Id ='' or Email_Id is null ) and School_ID='$school_id'");
?>
<html>
<head>
<style>
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('dept_without_emailid.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('dept_without_emailid.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<!-- Headline -->
        <div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white">Department Data  without Email Id</h2>
			</div>
...
<form>
	...
	<div class="col-md-2" style="width:17%;">
		<select name="colname" class="form-control">
			...
			<option value="id"
			<?php if (($_GET['colname']) == "id") {
					echo $_GET['colname']; ?> selected="selected" <?php } ?>> Id</option>

			<option value="Dept_code"
			<?php if (($_GET['colname']) == "Dept_code") {
					echo $_GET['colname']; ?> selected="selected" <?php } ?>>Department Code</option>  
	...
	<div class="col-md-1">
	<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('dept_without_emailid.php','_self')" />
	</div>	
</form>
<!-- search form END -->	
				...
				<tbody>
					 ...
					 ?>
						<tr>
							...
							<td data-title="Dept_code"><?php echo $row['Dept_code']; ?></td>
							<td data-title="Dept_Name"><?php echo $row['Dept_Name']; ?></td>
							<td data-title="Email_Id"><?php echo $row['Email_Id']; ?></td>
							<td> <center>
