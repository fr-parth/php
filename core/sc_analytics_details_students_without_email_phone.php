...
<head>
    ...
    <title>Students Without Email ID And Phone No.</title>
...
$sql=mysql_query("SELECT * FROM tbl_student where (std_phone ='') and (std_email='') and (school_id='$school_id')
LIMIT $start_from, $webpagelimit");	

$sql1 ="SELECT count(*) FROM tbl_student where (std_phone ='') and (std_email='') and (school_id='$school_id')"; 
...
//$colname=mysql_real_escape_string($_GET['colname']);
	if ($searchq != '')
	{ 
		$query1=mysql_query("SELECT * FROM tbl_student where (std_phone ='') and (std_email='') and (school_id='$school_id') and
 ...
 (std_complete_name LIKE '%$searchq%' or id LIKE '%$searchq%'
  ...
  LIMIT $start_from, $webpagelimit") or die("could not Search!");
			...
			$sql1 ="SELECT count(*) FROM tbl_student where (std_phone ='') and (std_email='') and (school_id='$school_id') and
...
$query1=mysql_query("SELECT * FROM tbl_student where (std_phone ='') and (std_email='') and (school_id='$school_id')
...
LIMIT $start_from, $webpagelimit")
		...
		or die("could not Search!");
					//echo $query1;
		$sql1 ="SELECT count(*) FROM tbl_student where (std_phone ='') and (std_email='') and (school_id='$school_id') and $colname LIKE '%$searchq%'"; 
					$rs_result = mysql_query($sql1);  
					$row1 = mysql_fetch_row($rs_result);  
					$total_records = $row1[0];  
					$total_pages = ceil($total_records / $webpagelimit);
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
			...
			window.location.assign('sc_analytics_details_students_without_email_phone.php?page='+page);
        });
    });
</script>
<?php }else{
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('sc_analytics_details_students_without_email_phone.php?Search=<?php echo $searchq; ?>&spage='+page);
        });
    });
</script>
<?php }?>
...
<div align="center">
    <div class="container" style="width:100%;">
        <div style="padding-top:30px;">
            ...
            <h2 style="padding-left:20px; margin-top:2px;color:#666"> <?php echo $dynamic_student;?> Without Email ID And Phone No.</h2>
		...
		<form style="margin-top:5px;">
			...
			<div class="col-md-1" >
			<input type="button" class="btn btn-info" value="Reset" onclick="window.open('sc_analytics_details_students_without_email_phone.php','_self')" />
			</div>
			<div class="col-md-1" >
