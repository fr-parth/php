...
$uploadedStatus = 0;
	...
	$query2="select batch_id from `tbl_raw_parent` WHERE school_id='$school_id' ORDER BY `parent_Id` DESC LIMIT 1";  
...
else
{
	...
	$file_type1=$file_t1[1]." ".$file_t1[2];
			...
			if(isset($_POST["submit"])) 
			{
				//Changes done by Pranali on 18-06-2018 for bug SMC-2552
	...
	if($limit>0)
	{
		...
		if($flag==1)
		{
			...
			try
			{
				...
				echo "<script type=text/javascript>alert('File has been scanned successfully...'); window.location=''</script>";
				fclose($file);
			}
			catch (Exception $e) 
			{ 
	    ...
	    else
		{
			for($i=2;$i<=$min;$i++)
			{
				...
				{ 
						...
						$arr[$i]["I"]=str_replace("'","", trim($allDataInSheet[$i]["I"]));   //Country
						$arr[$i]["J"]=str_replace("'","", trim($allDataInSheet[$i]["J"]));    // Family Income
						$arr[$i]["K"]=str_replace($replace," ", trim($allDataInSheet[$i]["K"]));    // Qualification
						$arr[$i]["L"]=str_replace($replace," ", trim($allDataInSheet[$i]["L"]));    // Occupation
						$arr[$i]["M"]=str_replace("'","", trim($allDataInSheet[$i]["M"]));    // Parent_Image_Path
						$arr[$i]["N"]=str_replace("'","", trim($allDataInSheet[$i]["N"]));    // State
						$arr[$i]["O"]=str_replace("'","", trim($allDataInSheet[$i]["O"]));    // City
						$arr[$i]["P"]=str_replace("'","", trim($allDataInSheet[$i]["P"]));    // Mother Name
				...
				if($sprn_lenght>0)
				{
					$row=mysql_query("select std_PRN from tbl_raw_parent where std_PRN='$s_reg_no' AND school_id='$school_id'");
					//$check_parent=mysql_query("select std_PRN from tbl_parent where std_PRN='$s_reg_no'");
					if(mysql_num_rows($row)==0)
					{
				          ...
				          if($s_f>0 || $s_l>0)
						  {
						  ...
						  }else
							   {
									...
									$reports="Inserted data from Excel Sheet is not valid data";
										 ...
										 {
										      ++$insertc;
									          $check_parent=mysql_query("select std_PRN from `tbl_parent` where std_PRN='$s_reg_no' and school_id='$school_id'");
											 ...
											 }else
											 {
                                						...
                                						$err_flag="Duplicate";
                                						$check_duplicate=mysql_query("select std_PRN from `tbl_raw_parent` where std_PRN='$s_reg_no' AND school_id='$school_id'");
                                						if(mysql_num_rows($check_duplicate)==0)
                                						{
                                							...
                                							if($parent_schid==$value){}
                                								   else{if($value!=""){
                                											$update_schid="UPDATE `tbl_parent` SET school_id='$value' where std_PRN='$s_reg_no' AND school_id='$school_id'";
                                											$update16= mysql_query($update_schid) or die('Could not update data: ' . mysql_error());
                                										}
                                									    }
                                							...
                                							if($parent_phone==$m_no){}
                                								         ...
                                								         if($m_val && $m_lenght==10){
                                											...
                                											$update2= mysql_query($update_ph) or die('Could not update data: ' . mysql_error());
                                										}
                                									    }
                                							if($parent_schid==$school_id){}
                                								         ...
                                								         if($sch_val || $sch_val!=""){
                                											...
                                											$update3= mysql_query($update_scid) or die('Could not update data: ' . mysql_error());
                                										}
                                									    }
                                							if($parent_emialid==$email_id){}
                                								...
                                								if($parent_Add==$parnt_ADDRESS){}
                                											...
                                											$add_val=preg_match('/^\s*[a-z0-9\s]+$/i',$parnt_ADDRESS);
                                											if($add_val || $parnt_ADDRESS!=""){
                                											   ...
                                											   $update7= mysql_query($update_add) or die('Could not update data: ' . mysql_error());
                                											}
                                										}
                                								...
                                								if($parent_Ctry==$parnt_COUNTRY){}
                                											...
                                											if($country_val){
                                											   ...
                                											   $update9= mysql_query($update_country) or die('Could not update data: ' . mysql_error());
                                											}
                                									}
                                								if($parent_Fincome==$parnt_FINCOME){}
                                											...
                                											if($income_val){
                                											   ...
                                											   $update10= mysql_query($update_fincome) or die('Could not update data: ' . mysql_error());
                                											}
                                										}
                                								if($parent_Quali==$parnt_QUALIFICATION){}
                                											...
                                											if($que_val){
                                											   ...
                                											   $update11= mysql_query($update_qua) or die('Could not update data: ' . mysql_error());
                                											}
                                										}
                                								if($parent_Occu==$parnt_OCCUPATION){}
                                											...
                                											if($occ_val){
                                											   ...
                                											   $update12= mysql_query($update_occu) or die('Could not update data: ' . mysql_error());
                                											}
                                										}
                                								if($parent_imgpath==$parnt_IMAGE_PATH){}
                                											...
                                											if($img_val){
                                											   ...
                                											   $update13= mysql_query($update_imgpath) or die('Could not update data: ' . mysql_error());
                                											}
                                										}
                                								if($parent_state==$parnt_STATE){}
                                											...
                                											if($state_val){
                                											   ...
                                											   $update14= mysql_query($update_state) or die('Could not update data: ' . mysql_error());
                                											}
                                										}
                                								if($parent_city==$parnt_CITY){}
                                											...
                                											if($city_val){
                                											   ...
                                											   $update15= mysql_query($update_city) or die('Could not update data: ' . mysql_error());
                                											}
                                										}
                                								if($parent_Mname==$parnt_MOTHER_NAME){}
                                											...
                                											if($mom_val){
                                											   ...
                                											   $update16= mysql_query($update_mname) or die('Could not update data: ' . mysql_error());
                                											}
										                                }
											 }
...
//------------------------------------ Code Of batch master-------------------------------------
			   /*	$query4="select count(case when `error_status`= 'Err-Phone/Email' then 1 else null end) as PHONE,
				...
				$error_count=$insertc+$count_dup+$count_of_updates+$sc_id+$bad_tid+$invalid_nm;
...
{
	echo "<script type=text/javascript>alert('Please select Upload Limit'); window.location=''</script>";
}
...
<div class='container' style="padding-top:30px;padding-left:30px;">
    <div class='panel panel-primary dialog-panel'>
   <div style="color:red;font-size:15px;font-weight:bold;margin-top:10px;"> <?php if(isset($_GET['report'])){ echo $_GET['report']; };?></div>
          <div class='panel-heading' align="center" style="
    width: 967px;margin-left:60px;">
   		...
   		<form name='frm' method='post' enctype='multipart/form-data' id='frm'>
		    ...
		    <div class="row" style="margin-left: 405px;">
				  ...
				  <div class="assignlimit">
                                  <div class="assign-limit">
                                        <form method="post" action="#">
                                        	<select name="limit" class="limitofuploadingrecords" id="limit" style="width:20%; height:30px; border-radius:2px;margin-left:405px;">
                                              <option value="20" disabled selected>Set upload Records Limit</option>
											  <option value="1">1</option>
											  <option value="4">4</option>
                    ...
                    <div class='col-md-4'>
                        <div class='col-md-4 indent-small'>
                            <input type='file' name='file'  id='file' size='30' onChange="ValidateSingleInput(this);" style="margin-left:378px;margin-top:20px"  />                          </div> 
                    </div>
					<br><br>
                  </div>
                  <div style="height:50px;"></div>
                  <div class='form-group'>
                                <div class='col-md-offset-3 col-md-3' style="padding-left:135px;margin-top:-35px">
                                  <input class='btn-lg btn-primary' type='submit' value="Submit" name="submit" />
                                </div>
                                <div class='col-md-3' style="margin-top:-35px">
								<!--
								comment cancel button-->
                                  <!--<button class='btn-lg btn-danger'  type='submit'>Cancel</button>-->
								  <input class='btn-lg btn-danger' type="reset" value="Reset" name="Reset" style="margin-left: -40px;">
                                </div>
                                </div>
                                <div class="row">
                                    <div class='col-md-6 col-md-offset-2' align="center" style="color:black;margin-top:15px;padding-left:201px;">
