...
$sql=mysql_query("select id,error_type from tbl_error_log where (error_type !='') and (error_type !='A-login') AND (error_type !='A-log') and (datetime between '".$_SESSION['from']."' and '".$_SESSION['to']."') group by error_type LIMIT $start_from, $webpagelimit");	

$sql1 ="select id, error_type from tbl_error_log where (error_type !='') and (error_type !='A-login')AND (error_type !='A-log') and (datetime between '".$_SESSION['from']."' and '".$_SESSION['to']."') group by error_type ";  
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
			...
			window.location.assign('list_error_type_log_org.php?page='+page);
        });
    });
</script>
<?php }else{
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('list_error_type_log_org.php?Search=<?php echo $searchq; ?>&spage='+page);
        });
    });
</script>
