...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT DISTINCT std_PRN,std_complete_name,std_Father_name,std_dept FROM tbl_student 
	WHERE (std_dept is Null or std_dept = ' ')  
		 AND school_id=$school_id and Academic_Year='$Academic_Year'
		 AND std_PRN 
					 NOT IN ( SELECT DISTINCT s.std_PRN FROM tbl_student s JOIN tbl_department_master d ON s.std_dept = d.Dept_Name WHERE s.school_id ='$school_id and Academic_Year='$Academic_Year') 

						LIMIT $start_from, $limit") or die("die could not Search!");
 
	$sql1=" SELECT DISTINCT std_PRN,std_complete_name,std_Father_name,std_dept FROM tbl_student 
	WHERE (std_dept is Null or std_dept = ' ')  
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
		...
		if ($colname != ''and $colname != 'Select')
				{
			$query1=mysql_query(" SELECT std_PRN, std_complete_name, std_semester, std_class, std_div, parent_id, std_email FROM tbl_student 
									WHERE (std_email ='' or std_email is null ) 
									AND  school_id='$school_id' and Academic_Year='$Academic_Year'
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('students_without_department.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('students_without_department.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<!-- Headline -->
	<div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white"><?php echo $dynamic_student;?> Data without Department (<?php echo $total_records; ?>)</h2>
			</div>
...
<?php
$acyear2=mysql_query("SELECT * FROM tbl_academic_Year where school_id='$school_id' and Enable='1' ");
$result=mysql_fetch_array($acyear2);
?>
...
<?php 
$acyear=mysql_query("SELECT * FROM tbl_academic_Year where school_id='$school_id' ");
while($result1 = mysql_fetch_array($acyear)){ ?>
...
<!--End of dropdown ----------------------- --- -->
		...
		<form>
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('students_without_department.php','_self')" />
			</div>	
        </form>
		<!-- search form END -->	
			...
			<table id="example" class="display" width="100%" cellspacing="0">
					...
					<tbody>
						...
						<tr>
							...
							<td>
                     			<a style="cursor: pointer;" href="edit_student_data_quality.php?std_prn=<?php echo $row['std_PRN']; ?>&flag=department#std_dept"><span class="glyphicon glyphicon-pencil"></span></a>
                        	</td>
						</tr>
						<?php $i++;} ?>     
					</tbody>
