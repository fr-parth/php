...
//$dynamic_student used and distinct removed in all below queries by Pranali to take dynamic usertype for SMC-4649 on 9-4-20	
if (!($_GET['Search'])){

if (isset($_GET["page"])){ $page  = mysql_real_escape_string($_GET["page"]); } else { $page=1; };  
$start_from = ($page-1) * $webpagelimit;
	
$sql=mysql_query("SELECT st.std_PRN,st.school_id,st.std_complete_name,p.user_id,p.school_id ,s.rewardType FROM purcheseSoftreward 
as p 
...
where (userType='".$dynamic_student."') and  (st.school_id='$school_id')
        
LIMIT $start_from, $webpagelimit");	


//Change the count query and added mysql_num_rows by Sayali Balkawade for SMC-4532 on 2/3/2020


$sql1 ="SELECT st.std_PRN,st.school_id,st.std_complete_name,p.user_id,p.school_id ,s.rewardType FROM purcheseSoftreward 
as p 
inner join softreward as s on p.reward_id=s.softrewardId 
 inner join tbl_student st on (st.std_PRN=p.user_id and p.school_id=st.school_id)
where (userType='".$dynamic_student."') and  (st.school_id='$school_id')";
...
if($searchq !='')
	...
	//if ($colname != ''and $colname != 'Select')
	{
		...
		$query1=mysql_query("SELECT st.std_PRN,st.school_id,st.std_complete_name,p.user_id,p.school_id ,s.rewardType FROM purcheseSoftreward 
as p 
inner join softreward as s on p.reward_id=s.softrewardId 
 inner join tbl_student st on (st.std_PRN=p.user_id and p.school_id=st.school_id)
where (userType='".$dynamic_student."') and  (st.school_id='$school_id')
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
			...
			window.location.assign('sc_soft_reward_purchase_student.php?page='+page);
        });
    });
</script>
<?php }else{
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('sc_soft_reward_purchase_student.php?Search=<?php echo $searchq; ?>&spage='+page);
        });
    });
</script>
...
<div align="center">
    <div class="container" style="width:100%;">
		...
		<div class="col-md-2" style="width:17%;">
			...
			<div class="col-md-1" >
			<input type="button" class="btn btn-info" value="Reset" onclick="window.open('sc_soft_reward_purchase_student.php','_self')" />
			</div>
			<div class="col-md-1" >
		...
		<th><?php echo $organization;?> ID</th>
                ...
                while($result = mysql_fetch_array($query1)) {
                    ...
                    <tr>
                        ...
                        <td data-title="<?php echo $organization;?> ID"><?php echo Strtoupper($result['school_id']); ?></td>
