...
if (!($_GET['Search'])){
	...
	$sql=mysql_query("Select vm.name,vm.category,vm.mobile,ls.EntityID,ls.school_id,ls.LatestLoginTime,ls.LogoutTime from tbl_LoginStatus
 ls inner join 
tbl_vol_spect_master vm on vm.id=ls.EntityID and vm.school_id=ls.school_id 
 where (l.LatestLoginTime BETWEEN '$from' AND '$to') and  ls.Entity_type='119' and ls.school_id='$school_id' order by RowID desc LIMIT $start_from, $webpagelimit") or die("Could not Search!");

	$sql1 =mysql_query("Select vm.name,vm.category,vm.mobile,ls.EntityID,ls.school_id,ls.LatestLoginTime,ls.LogoutTime from tbl_LoginStatus
 ls inner join 
tbl_vol_spect_master vm on vm.id=ls.EntityID and vm.school_id=ls.school_id 
 where (l.LatestLoginTime BETWEEN '$from' AND '$to') and  ls.Entity_type='119' and ls.school_id='$school_id' order by RowID desc");
...
else
{
 ...
 ls inner join 
tbl_vol_spect_master vm on vm.id=ls.EntityID and vm.school_id=ls.school_id 
 where (l.LatestLoginTime BETWEEN '$from' AND '$to') and  ls.Entity_type='119' and ls.school_id='$school_id' AND (vm.name LIKE '%$searchq%' or vm.category LIKE '%$searchq%' or vm.mobile LIKE '%$searchq%' or ls.EntityID LIKE '%$searchq%'  or ls.school_id LIKE '%$searchq%'
  or ls.LatestLoginTime LIKE '%$searchq%'  or ls.LogoutTime LIKE '%$searchq%'
 ) order by RowID desc LIMIT $start_from, $webpagelimit") or die("Could not Search!");
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
			...
			window.location.assign('spectator_stat.php?from=<?php echo $from; ?>&to=<?php echo $to; ?>&page='+page);
        });
    });
</script>
<?php }else{
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('spectator_stat.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&from=<?php echo $from; ?>&to=<?php echo $to; ?>&spage='+page);
        });
    });
</script>
<?php }?>
...
<div align="center">
    <div class="container" style="width:100%;">
        <div style="padding-top:30px;">
            ...
            <h2 style="padding-left:20px; margin-top:2px;color:#666"> Spectator Login Logout Details</h2>
		...
		<form style="margin-top:5px;">
			...
			<div class="col-md-1" >
			<input type="button" class="btn btn-info" value="Reset" onclick="window.open('spectator_stat.php','_self')" />
			</div>
		...
		<th>Logout Time</th>
			...
			<td><?php echo $result['EntityID']; ?></td>
			<td><?php echo $result['category']; ?></td>
			<td><?php echo $result['mobile']; ?></td>
			<td><?php echo $result['LatestLoginTime']; ?></td>
