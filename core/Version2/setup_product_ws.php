...
if($product != "" && $points != "" && $sponsor_id != "" && $product_type != "" && $product_price!="")
{
...
$arr = mysql_query("select id from tbl_sponsored where Sponser_product = '".$product."' and sponsor_id='".$sponsor_id."'");
  ...
  if($count == 0)
  {
	...
	$test1 = mysql_query("INSERT INTO `tbl_sponsored` (Sponser_type,Sponser_product,points_per_product,sponsor_id,total_coupons,valid_until,sponsered_date,daily_limit,daily_counter,reset_date,category, discount,product_image,product_price) VALUES('".$product_type."', '".$product."', '".$points."', '".$sponsor_id."','unlimited','$valid_until','$sponsored','unlimited','unlimited','$sponsored','$category','".$discount."','".$img_name."','".$product_price."')");//SMC-3490
		
		}
		else
		{
	
$test1 = mysql_query("INSERT INTO `tbl_sponsored` (Sponser_type, Sponser_product, points_per_product, sponsor_id, total_coupons, valid_until, sponsered_date, daily_limit, daily_counter, reset_date, category, discount, product_price) VALUES('".$product_type."', '".$product."', '".$points."', '".$sponsor_id."','unlimited','$valid_until','$sponsored','unlimited','unlimited','$sponsored','$category','".$discount."','".$product_price."')");//End SMC-3490
		}	
