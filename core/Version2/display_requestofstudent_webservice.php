...
//input from user
    ...
    if($stud_id1!="" && $school_id!='')//SMC-3450
	{
		    ...
		    //retrive info from tbl_master  for each activity
					$arr1=mysql_query("select stud_id1 as stud_id,requestdate,points,reason,id from tbl_request where stud_id2='$stud_id1' and school_id= '$school_id' and flag='N' and entitity_id='105'");//SMC-3450
  			...
  			if(mysql_num_rows($arr1)>=1) {
				...
				$points=$post['points'];
				$id=$post['id'];
					$sql=mysql_query("select std_complete_name from tbl_student where std_PRN ='$st_id' and school_id = '$school_id'");//SMC-3450
