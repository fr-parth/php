							...
							if ($phonenumber != "" && $emailid != "" && $firstname != "" && $lastname != "" && filter_var($emailid, FILTER_VALIDATE_EMAIL) && strlen($phonenumber) == 10)
								...
								if ($type == "student")
									...
									if (mysql_num_rows($row) <= 0)
										{
										mysql_query("insert into tbl_student(std_complete_name,std_name,std_lastname,std_Father_name,std_phone,school_id,std_email,std_password,country_code,std_country) values ('$std_complete_name','$firstname','$lastname','$middlename','$phonenumber','OPEN','$emailid','$password','$countrycode','$country')");
										...
										if (mysql_num_rows($row_student) > 0)
											...
											while ($post = mysql_fetch_assoc($row_student))
												...
												if ($countrycode == 91)
													...
													$url = "http://www.smswave.in/panel/sendsms.php?user=$dynamic_user&password=$dynamic_pass&sender=$dynamic_sender&PhoneNumber=$phonenumber&Text=$Text";
								...
								if ($type == "teacher")
									...
									if (mysql_num_rows($row) <= 0)
										...
										//// Start SMC-3450 Modify By Pravin 2018-09-21 08:27 PM 
											//$date=date('Y-m-d');
											//End SMC-3450
												mysql_query("insert into tbl_teacher(t_complete_name,t_name,t_lastname,t_middlename,t_phone,t_email,t_password,school_id,CountryCode,t_country) values ('$teacher_complete_name','$firstname','$lastname','$middlename','$phonenumber','$emailid','$password','OPEN',$countrycode,'$country')");
												$next_id = mysql_insert_id();
