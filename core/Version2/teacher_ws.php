...
function teacherMySubjects($school_id,$teacher_id,$Division_id,$Semester_id,$Branches_id,$Department_id,$CourseLevel,$limit,$offset){
...
{
$arr=mysql_query("SELECT ss.id as subjcet_code,sm.subjectName,sm.Semester_id,sm.Branches_id,teacher_ID FROM tbl_student_subject_master sm JOIN tbl_school_subject ss ON sm.subjcet_code=ss.Subject_Code JOIN tbl_academic_Year a ON sm.AcademicYear = a.Year WHERE sm.student_id ='$std_PRN' AND sm.school_id='$school_id' AND Enable='1' AND sm.teacher_ID='$t_id' group by sm.subjectName limit $limit OFFSET $offset");
