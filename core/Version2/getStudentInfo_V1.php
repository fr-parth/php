...
include 'conn.php';
 ...
 //SMC-3451 modify Pravin 2018-10-05 1:19 PM 
...
else
{
	...
	$sql=mysql_query("SELECT DISTINCT ss.student_id, ss.school_id, ss.subjectName, ss.Branches_id, ss.AcademicYear, ss.Division_id, s.std_complete_name,s.std_address,s.std_age, s.std_name,s.std_city, s.std_lastname, s.std_Father_name,s.std_class,s.std_country, s.std_phone,s.std_img_path,s.std_PRN,s.std_school_name,s.std_email,s.std_gender,s.std_dob,s.std_date,s.std_hobbies,s.std_semester,ss.Semester_id 
FROM tbl_student_subject_master ss JOIN tbl_academic_Year Y ON ss.AcademicYear = Y.Year and Y.Enable='1' JOIN tbl_student s ON s.std_PRN = ss.student_id and s.school_id =  '$school_id' WHERE ss.`teacher_id` ='$t_id' AND ss.school_id =  '$school_id' group by ss.student_id ORDER BY s.std_class, std_name,s.std_PRN LIMIT $limit OFFSET $offset ");
	...
	$thanqu_flag=$result1['status'];
		 ...
		 else
		 {
			...
			*/
			...
			'total_count'=>clean_string($count),
