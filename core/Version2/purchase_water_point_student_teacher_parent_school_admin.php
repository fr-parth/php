...
if(($card_no == '' || $user_id == '' || $entity_id == '' || $school_id == '') && $entity_id != '106')
{
	...
	$postvalue['responseMessage']="Enter school_id";
...
else
{
		if (strtotime($date1) <= strtotime($valid_to))
		{
			switch($entity_id)
			{
				...
				case  '102':
				  ...
				  $count=mysql_num_rows($result);
							if($count == '0')
							{
								...
								$admin_name=$arrs['name'];
								$username=$arrs['school_name'];
								$balance_blue_points=$balance_green_points+$amount;
								$query1="update tbl_school_admin set balance_blue_points='$balance_green_points' where id='$user_id'";
								$result = mysql_query($query1) or die('Errant query:  '.$query1);
								$update="update  tbl_giftcards  set  amount='0' ,status='Used' where card_no='$card_no'";
							$result = mysql_query($update) or die('Errant query:  '.$update);
				...
				$selectteacher="select id,t_id,t_complete_name,t_name,t_middlename,t_lastname,water_point from tbl_teacher where id='$user_id'";
				...
				if($count == '0')
							...
							$test=("insert into tbl_waterpoint(coupon_id,points,issue_date,entities_id,stud_id,school_id,Teacher_Member_Id)values('$card_no','$amount','$date1','$entity_id','$t_id','$school_id','$user_id')");
				...
				case  '105':
							...
							$selectstudinfo="select id,std_PRN,std_complete_name,std_name,std_lastname,std_Father_name,balance_water_points from tbl_student where id='$user_id'";
							...
							$test=("insert into tbl_waterpoint(coupon_id,points,issue_date,entities_id,stud_id,school_id,Stud_Member_Id)values('$card_no','$amount','$date1','$entity_id','$std_PRN','$school_id','$user_id')");
							$result = mysql_query($test) or die('Errant query:  '.$test);
				...
				case  '106':
						...
						$selectparent="select Name,Mother_name,balance_points from tbl_parent where Id='$user_id'";
							...
							$test=("insert into tbl_giftof_waterpoint(coupon_id,points,issue_date,entities_id,user_id)values('$card_no','$amount','$date1','$entity_id','$user_id')");
							$result = mysql_query($test) or die('Errant query:  '.$test);
