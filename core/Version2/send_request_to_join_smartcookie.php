...
if((($receiver_country_code!='' && $receiver_mobile_number!='') || $receiver_email_id!='') && $sender_member_id!='' && $sender_entity_id !='' && $receiver_entity_id!='' && $firstname!='' && $middlename!='' && $lastname!='' && $platform_source!='' && $request_status!='')
{
	...
	$viral_link1 = $server_name."/core/Version2/accept_request_to_join_smartcookie.php?id=".base64_encode($sender_member_id)."&senderentity=".base64_encode($sender_entity_id)."&receiverentity=".base64_encode($receiver_entity_id)."&referral_id=".base64_encode($referral_id);


	if($count == 0 && $count1 == 0)
	{ 
			$referral_activity_log_query = mysql_query("insert into referral_activity_log (sender_member_id,sender_user_type,receiver_country_code,receiver_mobile_number,receiver_email_id,invitation_sent_datestamp,acceptance_flag,method,receiver_user_type,firstname,middlename,lastname,referral_id) values ('$sender_member_id','$sender_type','$receiver_country_code','$receiver_mobile_number','$receiver_email_id','$invitation_sent_datestamp','$acceptance_flag','$platform_source','$receiver_type','$firstname','$middlename','$lastname','$referral_id')");
			...
			if($referral_activity_log_query)
			{
				...
				$url="http://www.smswave.in/panel/sendsms.php?user=$dynamic_user&password=$dynamic_pass&sender=$dynamic_sender&PhoneNumber=$receiver_mobile_number&Text=$Text";
				file_get_contents($url);

				$msgid='promoting_smartcookie';
				$res = file_get_contents($GLOBALS['URLNAME']."/core/clickmail/sendmail.php?email=$receiver_email_id&msgid=$msgid&invitation_sender_name=$invitation_sender_name&viral_link=$viral_link&platform_source=$platform_source&site=$server_name");
				...
				$point_insert_query = mysql_query("INSERT INTO $tbl_point ($point_insert_column) VALUES ('$id_prn','$receiver_entity_id','','$request_status','$points','$issue_date','$school_id','$referral_id')");
