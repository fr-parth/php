		...
		if ($colname != ''and $colname != 'Select')
			...
			$query1=mysql_query(" SELECT  ts.ExtSchoolSubjectId, ts.subjectName, ts.teacher_id,ts.ExtBranchId,ts.Branches_id, t.t_complete_name
				...
				$query1=mysql_query("SELECT  ts.ExtSchoolSubjectId, ts.subjectName, ts.teacher_id,ts.ExtBranchId,ts.Branches_id, t.t_complete_name
									...
									WHERE (ts.Branches_id ='' or ts.Branches_id is null ) 
									and ts.school_id = '$school_id' and ts.AcademicYear='$Academic_Year' 
									AND (ts.Branches_id  LIKE '$searchq%' or ts.ExtSchoolSubjectId LIKE '$searchq%' or ts.subjectName  LIKE '%$searchq%' or ts.teacher_id  LIKE '%$searchq%' or ts.ExtBranchId LIKE '%$searchq%'or t.t_complete_name LIKE '%$searchq%') LIMIT $start_from, $limit") or die("could not Search 4!");
				...
				WHERE (ts.Branches_id ='' or ts.Branches_id is null ) 
				and ts.school_id = '$school_id' and ts.AcademicYear='$Academic_Year' 
									AND (ts.Branches_id  LIKE '$searchq%' or ts.ExtSchoolSubjectId LIKE '$searchq%' or ts.subjectName LIKE '%$searchq%' or ts.teacher_id  LIKE '%$searchq%' or ts.ExtBranchId LIKE '%$searchq%'or t.t_complete_name  LIKE '%$searchq%' )") or die("could not Search 5!");   
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('teacher_subject_without_branchid.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('teacher_subject_without_branchid.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<!-- Headline -->
	<div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white"><?php echo $dynamic_teacher;?> <?php echo $dynamic_subject;?> Data without <?php echo $dynamic_branch; ?> ID (<?php echo $total_records; ?>) </h2>
			</div>
		...
		<form>
            ...
            <div class="col-md-2" style="width:17%;">
                <select name="colname" class="form-control">
                    ...
                    <option value="ExtBranchId"
					<?php if (($_GET['colname']) == "ExtBranchId") {
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('teacher_subject_without_branchid.php','_self')" />
			</div>	
        </form>
		<!-- search form END -->	
