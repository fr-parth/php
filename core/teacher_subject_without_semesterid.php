...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT  ts.ExtSchoolSubjectId, ts.subjectName, ts.teacher_id, t.t_complete_name ,ts.ExtSemesterId, ts.Semester_id 
						FROM tbl_teacher t 
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
		...
		if ($colname != ''and $colname != 'Select')
				{
			$query1=mysql_query(" SELECT  ts.ExtSchoolSubjectId, ts.subjectName, ts.teacher_id, t.t_complete_name ,ts.ExtSemesterId, ts.Semester_id 
									FROM tbl_teacher t 
										JOIN tbl_teacher_subject_master ts ON ts.school_id =t.school_id AND ts.teacher_id=t.t_id 
									WHERE (ts.Semester_id ='' OR ts.Semester_id is null ) 
										AND ts.school_id = '$school_id' and ts.AcademicYear='$Academic_Year' 
			...
			WHERE (ts.Semester_id ='' OR ts.Semester_id is null ) 
				...
				$query1=mysql_query("SELECT  ts.ExtSchoolSubjectId, ts.subjectName, ts.teacher_id, t.t_complete_name ,ts.ExtSemesterId, ts.Semester_id 
										...
										WHERE (ts.Semester_id ='' OR ts.Semester_id is null ) 
											AND ts.school_id = '$school_id' and ts.AcademicYear='$Academic_Year' 
									AND (ts.Semester_id LIKE '$searchq%' or ts.ExtSchoolSubjectIdail_Id LIKE '$searchq%' or ts.subjectName  LIKE '%$searchq%' or ts.teacher_id  LIKE '%$searchq%' or ts.ExtSemesterId  LIKE '%$searchq%'or t.t_complete_name  LIKE '%$searchq%') LIMIT $start_from, $limit") or die("could not Search!");
			...
			WHERE (ts.Semester_id ='' OR ts.Semester_id is null ) 
				AND ts.school_id = '$school_id' and ts.AcademicYear='$Academic_Year' 
									AND (ts.Semester_id LIKE '$searchq%' or ts.ExtSchoolSubjectId LIKE '$searchq%' or ts.subjectName LIKE '%$searchq%' or ts.teacher_id  LIKE '%$searchq%' or ts.ExtSemesterId  LIKE '%$searchq%' or t.t_complete_name  LIKE '%$searchq%' ) LIMIT $start_from, $limit") or die("could not Search!");   
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('teacher_subject_without_semesterid.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('teacher_subject_without_semesterid.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<!-- Headline -->
	<div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white"><?php echo $dynamic_teacher;?> <?php echo $dynamic_subject;?> Data without <?php echo $dynamic_semester;?> ID (<?php echo $total_records; ?>)</h2>
			</div>
...
<!--End of dropdown ----------------------- --- -->
		...
		<form>
            ...
            <div class="col-md-2" style="width:17%;">
                <select name="colname" class="form-control">
					...
					<?php if (($_GET['colname']) == "ExtSemesterId") {
                            echo $_GET['colname']; ?> selected="selected" <?php } ?>>Semester</option>
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('teacher_subject_without_semesterid.php','_self')" />
			</div>	
        </form>
		<br>
		<br>
			...
			<table id="example" class="display" width="100%" cellspacing="0">
					...
					<tbody>
						...
						<td data-title="id"><center><?php echo $i; ?></center></td>
                            ...
                            <td data-title="ExtSemesterId" ><?php echo $row['ExtSemesterId']; ?></td>
                            <td data-title="Semester_id" ><?php echo $row['Semester_id']; ?></td>
							<td> <center>
                     				<a style="cursor: pointer;" href="edit_TeacherSubject_DataQuality.php?subjectName=<?php echo $row['subjectName']; ?>"><span class="glyphicon glyphicon-pencil"></span></a>
