...
$std_PRN=xss_clean(mysql_real_escape_string($_GET['id']));
$report=" "; 
...
{
	if(xss_clean(mysql_real_escape_string(isset($_POST['activity']))))
		{			
		
				$method=xss_clean(mysql_real_escape_string($_POST['method_type']));
			...
			{
				$grade=xss_clean(mysql_real_escape_string($_POST['grade']));
			...
			{	
				if($method=="3")
				{
				$marks=xss_clean(mysql_real_escape_string($_POST['grade']));
				
				}
				
				elseif($method=="4")
				{
				$marks=xss_clean(mysql_real_escape_string($_POST['percentile']."%"));
				}
				else
				{
				$marks=xss_clean(mysql_real_escape_string($_POST['marks']));
				
				
				}
				
				$point = xss_clean(mysql_real_escape_string($_POST['points']));
						...
						if($point>0 && ereg('^[0-9]', $point))
							 {
								  ...
								  $teacher_id = $value['t_id'];
								  $entites_id = '103';
								  $studentpointlist_id = xss_clean(mysql_real_escape_string($_POST['activity']));
								
								  $pointreason = xss_clean(mysql_real_escape_string($_POST['pointreason']));
								...
								$point_type=xss_clean(mysql_real_escape_string($_POST['point_type']));
		...
		mysql_query("insert into tbl_student_point(sc_stud_id,sc_entites_id,sc_teacher_id,sc_studentpointlist_id,sc_point,marks,point_date,sc_status,method,activity_type,school_id,comment) values('$std_PRN','$entites_id','$teacher_id','$reason_id','$point','$marks','$point_date','N','$method','$activity_type','$sc_id','$pointreason')");
					            $q = "select sc_stud_id from tbl_student_reward where sc_stud_id = '$std_PRN' and school_id ='$sc_id'";
								$arr_count = mysql_query($q);
...
<body style="background: none repeat scroll 0% 0% #E5E5E5;text-shadow: none;">
      ...
      <div  class="col-md-9" style="border:1px solid #CCCCCC; background-color:#FFFFFF;">
            ...
            <div class="row">
                   <?php
				   $school_id=xss_clean(mysql_real_escape_string($_GET['school_id']));
                   ...
                   Assign Points to <?php if($fullName=="")
				   {
					 $name=ucwords(strtolower($row['std_name']))." ".ucwords(strtolower($row['std_Father_name']))." ".ucwords(strtolower($row['std_lastname']));
				         ...
				         echo xecho($name);
									 ...
									 {
					    			   echo xecho(ucwords(strtolower($fullName)));
										}
											 ?>
                    </div>
                    ...
                    <div>
                        <div style="padding:2px 2px 2px 2px;">
                           ...
                           <table style="font-size:14px;">
                           	...
                           	<tr>
                           		<td rowspan="4" align="center" style="padding-right:50px;"><img src="<?php if($row['std_img_path'] !=''){echo xecho($row['std_img_path']);}else{ echo xecho("Images/avatar_2x.png");}?>" width="80" height="80" /></td><td></td><td></td><td></td><td></td>
                            </tr>
                            <tr style="background-color:#FFFFFF">
								<td rowspan="4" align="center"></td><td style="font-size:16px;"><strong>Department:&nbsp;</strong></td><td><?php echo xecho($row['std_dept']);?></td><td>&nbsp;&nbsp;&nbsp;</td>
                            </tr>
                            <tr >
                           		<td ><strong>Branch&nbsp;:</strong></td><td ><?php echo xecho($row['std_branch']);?></td>
                            </tr>
	...
	$test=mysql_fetch_array($arra);?>
		...
		echo xecho($test['sc_total_point']);
		}
		?>
                            ...
                            <div style=" padding:2px 2px 2px 2px;">
                                ...
                                <div align="left"><a href="https://twitter.com/share" class="twitter-share-button" data-url=<?php echo $GLOBALS['URLNAME'];?> data-text="Good Performance<?php echo $row['std_complete_name'];?>" data-size="large" data-hashtags="smartcookieprogram"><b style="margin-top:20px;">Tweet<b></a>
                                  
                                   <div align="right" class="fb-share-button" data-href=<?php echo $GLOBALS['URLNAME'];?>  data-layout="button_count"></div></div></div>
...
WHERE sm.student_id = '$std_PRN' AND sm.school_id='$school_id' AND Enable='1' AND sm.teacher_ID='$teacher_id'");
                            ...
                            <input type="radio" value="<?php echo xecho('subject-'.$sub_id);?>" name="activity" id="<?php echo xecho('subject-'.$sub_id);?>" onclick = "MyAlert(this.value)"/><span style="font-size:12px;">
                            &nbsp;&nbsp;<?php echo xecho(ucfirst(strtolower($row['subjectName'])));?><br/></span>
                             <?php 
							   } 
							 ?>
                      ...
                      <div class="col-md-3"  style="padding-top:5px;" >
                           <h4 align="left">Art</h4>
                            ...
                            <input type="radio"  value="<?php echo xecho($row['sc_id']);?>" name="activity" id="activity" onclick = "MyAlert(this.value)"/><span style="font-size:12px;"> &nbsp;&nbsp;<?php echo xecho(ucfirst(strtolower($row['sc_list'])));?><br/></span>
                             <?php
							 	}
							 ?>
                      </div>
...
<input type="radio" value="<?php echo xecho($row['sc_id']);?>" name="activity" id="activity" onClick="MyAlert(this.value)"/><span style="font-size:12px;"> &nbsp;&nbsp;<?php echo  xecho(ucfirst(strtolower($row['sc_list'])));?><br/></span>
                       ...
                       <div class="container">
						...
						<div class="col-md-3" >      
								...
								<input type="text" name="pointreason" id="activity"  onClick="MyAlert(this.value)"class="form-control"/>
								</div>
								 	</div>
							<div class="col-md-3" >      
                                 <h5>Select point:</h5>
			  ...
			  <option value='Greenpoint' <?php if(xss_clean(mysql_real_escape_string(isset($_POST['point_type'])))){ if($_POST['point_type']=="Greenpoint")?> selected <?php } ?>>Green points</option> 
			  <option value='Waterpoint' <?php if(xss_clean(mysql_real_escape_string(isset($_POST['point_type'])))){ if($_POST['point_type']=="Waterpoint")?> selected <?php } ?>>Water Points</option> 
			</select></div>
			</div>
...
<th>Activity / Subject</th><th>Comment</th><th>Method</th><th style="width:20%">Marks / Grade / Percentile</th><th style="width:10%">Points Assigned</th><th style="width:10%">Date</th></tr>
                             <?php
							 
		$query5 = mysql_query("select * from tbl_student_point where sc_stud_id = ".xss_clean(mysql_real_escape_string($_GET['id']))." order by id desc limit 0, 5");
				...
				$sc_list=$rows6['subjectName'];
							...
							{  
                             ...
                             <tr style="background-color:#FFFFFF" >
                             	...
                             	<td><?php echo xecho($sc_list);?></td>
								 <td><?php echo xecho($rows5['comment']);?></td>
                                 
                             <td><?php $method_id= $rows5['method'];
							 $sql=@mysql_query("select method_name from tbl_method where id='$method_id'");
							 $res=@mysql_fetch_array($sql);
							 echo xecho($res['method_name']);
							 ?></td>
							 
                                	<td><?php echo xecho($rows5['marks']);?></td>
                                <td><?php echo xecho($rows5['sc_point']);?></td>
                                <td><?php echo xecho($rows5['point_date']);?></td>
