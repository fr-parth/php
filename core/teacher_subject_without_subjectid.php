...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("  SELECT ts.tch_sub_id,ts.ExtSchoolSubjectId, ts.subjectName, ts.teacher_id, t.t_complete_name , ts.subjcet_code 
							FROM tbl_teacher t 
						...
						LIMIT $start_from, $limit") or die("could not Search1!");;
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
		...
		if ($colname != ''and $colname != 'Select')
				{
				$query1= mysql_query("SELECT ts.tch_sub_id,ts.ExtSchoolSubjectId, ts.subjectName, ts.teacher_id, t.t_complete_name , ts.subjcet_code 
									FROM tbl_teacher t 
									JOIN tbl_teacher_subject_master ts ON ts.school_id =t.school_id AND ts.teacher_id=t.t_id
									WHERE (ts.ExtSchoolSubjectId ='' or ts.ExtSchoolSubjectId is null ) 
									AND  ts.school_id = '$school_id' and ts.AcademicYear='$Academic_Year' 
									AND ts.$colname LIKE '%$searchq%' LIMIT $start_from, $limit") or die("could not Search2!");
				...
				WHERE (ts.ExtSchoolSubjectId = '' or ts.ExtSchoolSubjectId is null ) 
				AND  ts.school_id = '$school_id' and ts.AcademicYear='$Academic_Year' 
									AND ts.$colname LIKE '%$searchq%' ") or die("could not Search3!");
				...
				$query1=mysql_query(" SELECT  ts.tch_sub_id,ts.ExtSchoolSubjectId, ts.subjectName, ts.teacher_id, t.t_complete_name , ts.subjcet_code 
										...
										WHERE (ts.ExtSchoolSubjectId ='' or ts.ExtSchoolSubjectId is null ) 
										AND  ts.school_id = '$school_id' and ts.AcademicYear='$Academic_Year'  
										AND (ts.ExtSchoolSubjectId LIKE '$searchq%' or ts.subjectName LIKE '$searchq%' or ts.teacher_id LIKE '%$searchq%' or t.t_complete_name  LIKE '%$searchq%' ) LIMIT $start_from, $limit") or die("could not Search4!");
				...
				$sql1 =mysql_query("SELECT ts.tch_sub_id 
										...
										WHERE (ts.ExtSchoolSubjectId ='' or ts.ExtSchoolSubjectId is null ) 
										AND  ts.school_id = '$school_id' and ts.AcademicYear='$Academic_Year' 
										AND (ts.ExtSchoolSubjectId LIKE '$searchq%' or ts.subjectName LIKE '$searchq%' or ts.teacher_id LIKE '%$searchq%' or t.t_complete_name  LIKE '%$searchq%' )") or die("could not Search5!");   
...
//$result = mysql_query("SELECT * FROM tbl_teacher_subject_master where(ExtSchoolSubjectId ='' or ExtSchoolSubjectId is null ) and School_ID='$school_id'");
?>
<html>
<head>
<style> 
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('teacher_subject_without_subjectid.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('teacher_subject_without_subjectid.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<!-- Headline -->
	<div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white"><?php echo $dynamic_teacher;?> <?php echo $dynamic_subject;?> Data without <?php echo $dynamic_subject;?> ID (<?php echo $total_records; ?>)</h2>
			</div>
...
<!--End of dropdown ----------------------- --- -->
		...
		<form>
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('teacher_subject_without_subjectid.php','_self')" />
			</div>	
        </form>
		<!-- search form END -->	
			...
			<table id="example" class="display" width="100%" cellspacing="0">
					...
					<tbody>
						...
						<td data-title="id"><center><?php echo $i; ?></center></td>
							...
							<td> <center>
                     				<a style="cursor: pointer;" href="edit_TeacherSubject_DataQuality.php?tch_sub_id=<?php echo $row['tch_sub_id']; ?>" ><span class="glyphicon glyphicon-pencil"></span></a>
								</center>
							</td>
						</tr>
						<?php $i++;} ?>     
