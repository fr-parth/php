...
$query = mysql_query("SELECT Id,Parent_Member_id,country,std_PRN,Name,Father_name,Mother_name FROM  tbl_parent where (country ='' or country is null) AND school_id = '$school_id' LIMIT $start_from, $limit") or die("could not Search!");

$sql1 = "SELECT Id FROM  tbl_parent where (country ='' or country is null) AND school_id = '$school_id' ";
...
if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
        ...
        if ($colname != ''and $colname != 'Select')
				{
            		$query1=mysql_query(" SELECT Id,Parent_Member_id,country,std_PRN,Name,Father_name,Mother_name FROM  tbl_parent where (country='' or country is null) AND school_id = '$school_id'
									AND `$colname` LIKE '%$searchq%' LIMIT $start_from, $limit") or die("could not Search!");

				$sql1 =mysql_query("SELECT Id FROM  tbl_parent where (country ='' or country is null) AND school_id = '$school_id'AND `$colname` LIKE '%$searchq%' ") or die("could not Search!");
				$total_records = mysql_num_rows($sql1);   
				$total_pages = ceil($total_records / $limit);
		}else{	
				$query1=mysql_query("SELECT Id,Parent_Member_id,country,std_PRN,Name,Father_name,Mother_name FROM  tbl_parent where (country ='' or country is null) AND school_id = '$school_id' 
									AND (country LIKE '$searchq%' or Parent_Member_id LIKE '$searchq%' or std_PRN LIKE '%$searchq%' 
				...
				$sql1 =mysql_query("SELECT Id FROM  tbl_parent where (country ='' or country is null) AND school_id = '$school_id' 
									AND (country LIKE '$searchq%' or Parent_Member_id LIKE '$searchq%' or std_PRN LIKE '%$searchq%' 
								or Name LIKE '%$searchq%'
							or Father_name LIKE '%$searchq%'
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('parent_without_country.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('parent_without_country.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<!-- Headline -->
	<div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white">Parent Data without Country </h2>
			</div>
        ...
        <div class="col-md-2" style="width:17%;">
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('parent_without_country.php','_self')" />
			</div>	
        </form>
		<!-- search form END -->	
