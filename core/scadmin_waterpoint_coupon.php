 ...
 if(!isset($id))
	{
		header('location:login.php');
		 ...
		 if(isset($_POST['submit']))
			...
			$redirect=$GLOBALS['URLNAME'].'/core/scadmin_waterpoint_coupon.php';
		...
		//	print_r($responseStatus);exit;
			if($responseStatus==200)
				{														
					echo "<script>alert('$points Water points purchased successfully');location.assign('$redirect');</script>";	
				}
				else						
				{														
		...
		/*	if($coupon_id!="")
			{
			 ...
			 // mysql_query("update tbl_giftof_rewardpoint set parent_id='$parent_id' where coupon_id='$coupon_id'");
			  ...
			  $balance_water_point=$balance_water_point+$point;
			$status='Used';

		$date1 = CURRENT_TIMESTAMP; // define in core/securityfunctions.php
							
			   mysql_query("update tbl_school_admin set  balance_water_point='$balance_water_point' where id='$id' ");
			   mysql_query("update  tbl_giftcards  set  amount='0',status='$status' where card_no='$coupon_id' ");
			   
			   mysql_query("insert into tbl_waterpoint(coupon_id,points,issue_date,entities_id,School_Member_Id,school_id)values('$coupon_id','$point','$date1','102','$id','$school_id')");
    ...
    <div class="row">
            ...
            <div class="container" style="padding:10px;background-color:#FFFFFF;border:1px solid #CCCCCC;box-shadow:0px 1px 3px 1px #C3C3C4;" align="center">
                <div class="row"  align="center">
					...
					?>
                            <?php echo $value['balance_water_point']; ?>
                    </div>
