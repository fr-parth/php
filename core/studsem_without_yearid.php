...
and (semester.ExtYearID  ='' or semester.ExtYearID  is null ) 
	...
	$sql1=" SELECT id FROM StudentSemesterRecord 
			where(ExtYearID  ='' or ExtYearID  is null ) and school_id='$school_id' and AcdemicYear='$Academic_Year' ";
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
		...
		and (semester.ExtYearID  ='' or semester.ExtYearID  is null ) 
				...
				$sql1 =mysql_query("SELECT id FROM StudentSemesterRecord 
									WHERE  (ExtYearID ='' or ExtYearID is null ) 
			...
			and semester.`IsCurrentSemester`='1' 
			 ...
			 and AcdemicYear='$Academic_Year'
			and (semester.ExtYearID  ='' or semester.ExtYearID  is null )  
									AND (student_id LIKE '$searchq%' or Semesterid LIKE '$searchq%' or BranchName LIKE '$searchq%' or DeptName LIKE '%$searchq%'or SemesterName LIKE '$searchq%'or ExtYearID LIKE '$searchq%'  ) LIMIT $start_from, $limit") or die("could not Search!");
			
				$sql1 =mysql_query("SELECT id FROM StudentSemesterRecord 
                WHERE  (ExtYearID='' or ExtYearID is null ) 
                AND school_id='$school_id' and AcdemicYear='$Academic_Year'
									AND (student_id LIKE '$searchq%' or Semesterid LIKE '$searchq%' or BranchName LIKE '$searchq%' or DeptName LIKE '%$searchq%'or SemesterName LIKE '$searchq%'or ExtYearID LIKE '$searchq%' )") or die("could not Search!");   
...
//$result = mysql_query("SELECT student_id , Semesterid, BranchName, DeptName ,SemesterName,ExtYearID FROM StudentSemesterRecord where(ExtYearID   ='' or ExtYearID is null ) and School_ID='$school_id'");
?>
<html>
<head>
<style>
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('studsem_without_yearid.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('studsem_without_yearid.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<!-- Headline -->
	<div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white">Student Semester Data without Year Id  (<?php echo $total_records; ?>) </h2>
			</div>
...
<!--End of dropdown ----------------------- --- -->
		...
		<form>
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('studsem_without_yearid.php','_self')" />
			</div>	
        </form>
		<!-- search form END -->	
				...
				<tbody>
					 ...
					 ?>
						<tr>
                            ...
                            <td data-title="ExtYearID"><?php echo $row['ExtYearID']; ?></td>
							<td> <center>
