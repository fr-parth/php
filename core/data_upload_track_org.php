...
if (isset($_POST['search']))
{
	$sql = "SELECT bm.school_id,bm.tbl_display_name,bm.tbl_name,bm.inserted_date,ay.Academic_Year,ay.group_member_id,s.school_id,s.school_type
		...
		LEFT JOIN tbl_school_admin s ON bm.school_id=s.school_id  where  s.school_type='Organization' ";
 ...
 $sql = $sql . $qry . " group by bm.school_id";
    ...
    $sql = "SELECT bm.school_id,bm.tbl_display_name,bm.tbl_name,bm.inserted_date,ay.Academic_Year,ay.group_member_id,s.school_id,s.school_type
        FROM tbl_school_datacount bm 
        LEFT JOIN tbl_academic_Year ay ON bm.school_id=ay.school_id 
		LEFT JOIN tbl_school_admin s ON bm.school_id=s.school_id  where bm.school_id!='' and s.school_type='Organization' group by bm.school_id ";
}
//echo $sql;
$row = mysql_query($sql);
...
<div align="center">
     <div class="row" style="padding-top:30px;">
        ...
        <br>
            <div class="col-md-2" style="font-weight:bold;font-size:120%;padding-top:5px;margin-right:-52px;margin-left:220px"> Financial Year</div>
            ...
            <table id="example" class="col-md-12 table-bordered">
              ...
              <tr style="background-color:#0073BD; color:#ffffff;font-family:Times New Roman, Times, serif;font-size:17px">
                ...
                <tr style="font-family:Times New Roman, Times, serif;font-size:17px; color:#333;">
                    ...
                    <td data-title="Organization ID"><?php echo $result['school_id'];?></td>
					<td data-title="Organization Name"><?php echo $res['school_name'];?></td>
					<td data-title="Upload Preview">
						<input type="submit" value="Upload Preview" name="upload_preview" onClick="window.location.href='data_upload_track_sa_org.php?school_id=<?php echo $res['school_id'] ?>&school_type=<?php echo $res['school_type'] ?>&group_id=<?php echo $result['group_member_id'] ?>&Academic_Year=<?php echo $result['Academic_Year'] ?>&school_name=<?php echo $res['school_name'] ?>'" class="btn btn-primary" />
					</td>
				</tr>	
                    <?php $i++;
                    }
