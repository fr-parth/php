...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT id,ExtDegreeID,Degee_name,Degree_code,course_level FROM tbl_degree_master 
						WHERE ( course_level ='' OR course_level is NULL) 
		...
		if ($colname != ''and $colname != 'Select')
			...
			$query1=mysql_query("SELECT id,ExtDegreeID,Degee_name,Degree_code,course_level FROM tbl_degree_master 
				...
				$query1=mysql_query("SELECT id,ExtDegreeID,Degee_name,Degree_code,course_level FROM tbl_degree_master 
									WHERE ( course_level ='' OR course_level is NULL) 
										AND school_id = '$school_id'
										AND (course_level LIKE '$searchq%' or Degee_name LIKE '$searchq%' or Degree_code LIKE '$searchq%' or ExtDegreeID LIKE '$searchq%'  ) LIMIT $start_from, $limit") or die("could not Search!");
			
				$sql1 =mysql_query("SELECT id FROM  tbl_degree_master
									WHERE ( Degee_name ='' OR Degee_name is NULL )  
										AND school_id = '$school_id'
										AND (course_level LIKE '$searchq%' or Degee_name LIKE '$searchq%' or Degree_code LIKE '$searchq%' or ExtDegreeID LIKE '$searchq%' )") or die("could not Search!");   
   ...
   <?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('degree_without_courselevel.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('degree_without_courselevel.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
		});
	});
</script>
<?php }?>
...
<div class="container">
		<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white"><?php echo $dynamic_degree;?> Data without <?php echo $dynamic_course_level;?></h2>
		</div>
		...
		<form>
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('degree_without_courselevel.php','_self')" />
			</div>
