...
$fr_date = date("d-m-Y H:i",mktime(00,00));//hour, minute, second
 $to_date = date("d-m-Y H:i",mktime(23,59));//hour, minute, second
  ...
  // print_r($_POST); exit;
     ...
     $student_sql = "SELECT ls.EntityID, s.std_PRN,s.std_dept, s.std_complete_name,count(s.std_complete_name) as cnt, ls.FirstLoginTime, ls.LatestLoginTime, ls.LogoutTime, ls.school_id, sa.school_name FROM tbl_LoginStatus as ls JOIN tbl_student as s ON ls.EntityID=s.id and ls.school_id=s.school_id JOIN tbl_school_admin as sa ON ls.school_id=sa.school_id where ls.school_id='$sc_id' AND ls.LatestMethod='$type' AND ls.LatestLoginTime between '$from_dt' AND '$to_dt'
  AND (ls.Entity_type='105' OR ls.Entity_type='205') group by ls.EntityID order by ls.LatestLoginTime DESC";
      // echo $student_sql; exit;
    $arr=mysql_query($student_sql);
  
    $res_count = mysql_num_rows($arr);

    $query1=mysql_query("SELECT school_type,school_name FROM tbl_school_admin where school_id='$sc_id'");

        $row=mysql_fetch_array($query1);
        $school_type=$row['school_type'];
      $school_name=$row['school_name'];
      $result = $school_name . '(' . $sc_id .')';
      $a= "Date Of " .$result .date('d-m-Y H:i',strtotime($f_dt))." - ".date('d-m-Y H:i',strtotime($t_dt));
...
<div class="container">
  ...
  <div class="row" style="padding-top:10px;">
    ...
    <div class="col-md-12"><h3 class="text-center"><?= $a; ?></h3></div>
    <br>
    <div class="clearfix"></div>
    <div class="col-md-12">
      <table id="example" class="table-bordered table-condensed cf display" width="100%;">
                                <thead>
                                ...
                                <!-- Camel casing done for Sr. No. by Pranali -->
                                    ...
                                    <th style="width:21%;">Student ID</th>
                                    ...
                                    <th style="width:15%;">Last Logout</th>
                                     <!-- <th style="width:21%;">Student Name</th> -->
                                      <th style="width:21%;">Student Name count</th>
                                </tr>
                                </thead>
                                ...
                                <?php
                                    ...
                                    ?>
                                      <tr style="color:#808080;" class="active s_web detail_color">
                                        ...
                                        <td data-title="student_id"><b><?php echo $row['std_PRN']; ?></b></td>
                                        <td data-title="student_dept"><b><?php echo $row['std_dept']; ?></b></td>
