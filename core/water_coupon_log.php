   ...
   <table id="example" class="display" width="100%" cellspacing="0">
        ...
        <tbody>
         <?php
          ...
          $sql = "SELECT gp.*, sa.Name, sa.school_id as saschoolid, s.std_complete_name, s.school_id as stschoolid,t.t_complete_name,t.school_id as tschoolid FROM `tbl_waterpoint` gp left join `tbl_parent` sa on gp.`Parent_Member_Id`=sa.`Id` left JOIN `tbl_student` s on gp.`Stud_Member_Id`=s.`id` left JOIN tbl_teacher t on gp.`Teacher_Member_Id`=t.`id` where gp.`entities_id`='106' or gp.`entities_id`='105' or gp.`entities_id`='103' order by gp.`id` desc" ;
