...
and AcdemicYear='$Academic_Year'
	...
	$sql1="SELECT id FROM StudentSemesterRecord 
			where(Semesterid ='' or Semesterid is null ) and school_id='$school_id' and AcdemicYear='$Academic_Year' ";
		  ...
		  $query1=mysql_query(" SELECT  semester.student_id ,  semester.Semesterid,  semester.BranchName,  semester.DeptName , semester.SemesterName 
			...
			and (semester.Semesterid ='' or semester.Semesterid is null ) 
			 ...
			 $sql1 =mysql_query("SELECT id FROM StudentSemesterRecord 
									WHERE  (Semesterid ='' or Semesterid is null ) 
			...
			$query1=mysql_query("SELECT  semester.student_id ,  semester.Semesterid,  semester.BranchName,  semester.DeptName , semester.SemesterName 
			 ...
			 $sql1 =mysql_query("SELECT id FROM StudentSemesterRecord 
				...
				echo $total_records = mysql_num_rows($sql1);    
				$total_pages = ceil($total_records / $limit);
			}
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('studsem_without_semid.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('studsem_without_semid.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white"> student semester without semester Id (<?php echo $total_records; ?>)</h2>
			</div>
...
<!--End of dropdown ----------------------- --- -->
		...
		<form>
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('studsem_without_semid.php','_self')" />
			</div>	
        </form>
		<!-- search form END -->	
