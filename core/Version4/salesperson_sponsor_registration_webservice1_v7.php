...
// revenue_percent & revenue_per_visit added by Pranali for SMC-3678 on 5-12-18
...
//new Google API Key, key=AIzaSyCbCGGc7SEjm-6G_v1KYy-Szb0Srsd-Dz0 added by Pranali for SMC-3678
			...
			CURLOPT_URL => "https://maps.googleapis.com/maps/api/geocode/json?address=$string&key=AIzaSyCbCGGc7SEjm-6G_v1KYy-Szb0Srsd-Dz0",
	...
	if($test)
		{
		   ...
		   //$date = date('mdY');
		  $date = CURRENT_TIMESTAMP;
// revenue_percent & revenue_per_visit inserted into tbl_sponsored by Pranali for SMC-3678 on 5-12-18	   
           $test2="INSERT INTO `tbl_sponsored`(Sponser_type, Sponser_product, points_per_product, sponsered_date, validity, sponsor_id, valid_until, category,discount, daily_limit,daily_counter, reset_date,revenue_percent,revenue_per_visit)
		   VALUES('discount', '$discount_setup', '$points', '$date', 'valid', '$memberid','$date1', '$sp_product_category','$discount_setup', 'unlimited', 'unlimited', '$date1','$revenue_percent','$revenue_per_visit')";
			...
			$msgid=16; //msgid=16 taken from table tbl_email_sms_templates
			$ccmail='accounts@blueplanetsolutions.com';
//Mail functionality modified by Pranali for SMC-3678 on 27-12-2018
			$url = SEND_MAIL_PATH; //defined in securityfunctions.php
	
			$myvars = 'email=' . $sp_email . '&msgid=' . $msgid. '&site=' . $site. '&ReceiptNo=' .$ReceiptNo . '&Name=' . $company_name . '&Amount=' . $amount . '&Validity=' .$date1. '&paymentmethod=' .$payment_method. '&ccmail=' .$ccmail;
...
//preg_replace & file_put_contents added by Pranali for Image upload for SMC-3678 
	 $sponsor_shop_imageData = base64_decode(preg_replace('#^data:image/\w+;base64,#i', '',$sponsor_shop_imageDataEncoded));
	
	 $sponsor_shop_imageSave = file_put_contents($sponsor_shop_imageName, $sponsor_shop_imageDataEncoded);
...
//preg_replace & file_put_contents added by Pranali for Image upload for SMC-3678
	  $imageData = base64_decode(preg_replace('#^data:image/\w+;base64,#i', '',$imageDataEncoded));
	  $imageSave = file_put_contents($imageName, $imageDataEncoded);
	...
	$msgid=1; //msgid=1 taken from table tbl_email_sms_templates
//Mail functionality modified by Pranali for SMC-3678 on 27-12-2018
	$url = SEND_MAIL_PATH; //defined in securityfunctions.php
	
	$myvars = 'email=' . $sp_email . '&msgid=' . $msgid. '&site=' . $site. '&pass=' . $pass;
...
curl_setopt_array($curl, array(
    CURLOPT_RETURNTRANSFER => 1,
    CURLOPT_URL => "$site/core/clickmail/
sendmail_new.php?email=$sp_email&msgid=$msgid&site=$site&pass=$pass&platform_source=$platform_source",
    CURLOPT_USERAGENT => 'Codular Sample cURL Request'
));
// Send the request & save response to $resp
$res = curl_exec($curl);*/
					...
					//	$res = file_get_contents("http://$site/core/clickmail/sendmail.php?email=$sp_email&msgid=$msgid&site=$site&pass=$pass&platform_source=$platform_source");
						...
						// last_programmer_name added by Rutuja for SMC-4915 for  on 21-10-2020
							$ch = curl_init("$site/core/Version3/error_log_ws.php");
...
$ch = curl_init("$site/core/Version3/error_log_ws.php");
 ...
 elseif($sponsor_shop_imageSave && $test)
 {
		...
		if($imageSave){
			...
			$img="Image uploaded sucessfully";
			$responseStatus="200";
			$responseMessage="OK";
		}
		elseif(!$imageSave)
