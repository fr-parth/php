...
$site=$GLOBALS['URLNAME']; //from core/securityfunctions.php
	...
	function getdetails($entity_name,$member_id,$condition)
	{
	   if($entity_name=='Teacher')
		 ...
		 else if($entity_name=='Employee')
		 {
		    $query="SELECT * FROM `tbl_student` where entity_type_id='205' and $condition "; 
			$result = mysql_query($query) or die('Errant query:  Query error');
			$post = mysql_fetch_assoc($result);
			if($post['std_img_path']=='')
			{
		 ...
		 else if($entity_name=='Manager')
		 {
			$query="select * from tbl_teacher where `entity_type_id`=203 and $condition  ";
			$result = mysql_query($query) or die('Errant query:  Query error');
			$post = mysql_fetch_assoc($result);
			if($post['t_pc']=='')
			{
