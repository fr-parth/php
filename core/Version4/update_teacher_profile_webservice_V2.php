...
//new school id parameter added by Pranali on 29-1-19 for SMC-3762
	 ...
	 $t_dept = xss_clean(mysql_real_escape_string($obj->{'t_dept'}));
	 
	 
$row=mysql_query("select Dept_code from tbl_department_master where Dept_Name='$t_dept'");
        $arr = mysql_fetch_array($row);
        $t_DeptCode= $arr['Dept_code'];
//Author: Pranali 
...
//below if condition added by Pranali for SMC-3762
	if($new_t_id!='' && $t_id!='')
	{
		...
		{
			$test=mysql_query("UPDATE tbl_teacher SET t_id='$new_t_id',t_dept='$t_dept',t_DeptCode='$t_DeptCode' where id='$User_Meid'");
 ...
 if($imageDataEncoded=$obj->{'User_imagebase64'} != '')
	...
	if($t_dept != '')
	{
	  ...
	  $test = mysql_query("update tbl_teacher set t_dept='$t_dept',t_DeptCode='$t_DeptCode' where id = '$User_Meid' and (school_id = '$school_id' or school_id ='$new_sc_id')");
	...
	if($t_complete_name != '' && $gender != '')
	{
	  ...
	  $test = mysql_query("update `tbl_teacher` set t_complete_name = '$t_complete_name', t_name ='$fname' , t_middlename='$mname' ,t_lastname ='$lname',t_email='$email', t_dob = '$dob', t_address = '$address', t_city = '$city', t_country = '$country',state = '$state', t_gender = '$gender',t_phone = '$phone',  CountryCode = '$country_code',t_dept='$t_dept',t_DeptCode='$t_DeptCode' where id = '$User_Meid' and (school_id = '$school_id' or school_id ='$new_sc_id')");
