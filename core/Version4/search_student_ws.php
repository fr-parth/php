<?php
/*
$query taken common and filter for email and phone given by Pranali for SMC-4569 on 25-2-20
*/
...
{	
	$query="SELECT DISTINCT s.school_id,s.id as member_id,s.std_PRN ,s.std_name,s.std_lastname,s.std_Father_name,s.std_complete_name,s.std_dept,s.std_class,s.std_branch,s.std_dob,s.std_age,s.std_address,s.std_city,s.std_country,s.std_gender,s.std_div,s.std_hobbies,s.std_img_path,s.std_email,s.std_phone,s.std_state,s.Iscoordinator,s.Academic_Year,sa.school_name as std_school_name 
		FROM tbl_student s
		JOIN tbl_school_admin sa
		ON s.school_id=sa.school_id
		WHERE
		(s.std_PRN LIKE '%$student_key%' OR s.std_complete_name LIKE '%$student_key%' 
		OR s.std_lastname LIKE '%$student_key%' OR s.std_dept LIKE '$student_key%'	
		OR s.std_Father_name LIKE '%$student_key%' OR s.school_id LIKE '%$student_key%'
		OR s.std_name LIKE '%$student_key%' OR s.std_class LIKE '$student_key%' OR s.std_branch LIKE '%$student_key%' OR s.std_email LIKE '%$student_key%' OR s.std_phone LIKE '%$student_key%')";

	if(!empty($school_id))
	{			
		$query.=" AND s.school_id = '$school_id' LIMIT 100";
