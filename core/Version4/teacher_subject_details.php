...
//////////// *****Validation for Empty****** ////////
	...
	else if(!empty($school_id) && !empty($subjectCode) && !empty($department) && !empty($courseLevel) && !empty($semester) && !empty($year)  && !empty($branch) && $entity_id == '105')
	{
		$teacher = mysql_query("select t.id,t.t_id,ucwords(t.t_complete_name) as t_complete_name,ucwords(t.t_name) as t_name,ucwords(t.t_middlename) as t_middlename,ucwords(t.t_lastname) as t_lastname from tbl_teacher_subject_master ts join tbl_teacher t 
		on ts.school_id =t.school_id and t.t_id = ts.teacher_id where ts.school_id = '$school_id' AND ts.subjcet_code='$subjectCode' AND ts.Department_id='$department' AND ts.CourseLevel='$courseLevel' AND ts.Semester_id='$semester' AND ts.AcademicYear='$year' AND ts.Branches_id='$branch'");
	...
	else
	{
		...
		$sub = mysql_query("select id,subject,Subject_Code,Subject_type,Course_Level_PID from tbl_school_subject where school_id = '$school_id' and subject!=''");
//distinct added in below queries by Pranali for SMC-3778 on 19-2-19
		while($row = mysql_fetch_assoc($sub))
		{
			$id=$row['id'];
			 ...
			 $Course_Level_PID=$row['Course_Level_PID'];
			$subjects[] =array('id'=>$id,'subject'=>$subject,'Subject_Code'=>$Subject_Code,'Subject_type'=>$Subject_type,'Course_Level_PID'=>$Course_Level_PID);
		...
		$sem = mysql_query("select distinct(Semester_Name) from tbl_semester_master where school_id = '$school_id'");
		...
		$year = mysql_query("select distinct(Year),Academic_Year,ExtYearID from tbl_academic_Year where school_id = '$school_id' group by Academic_Year AND Enable='1'"); /*AND Enable='1'*/
		...
		$branch = mysql_query("select distinct(branch_Name) from tbl_branch_master where school_id = '$school_id' ");
