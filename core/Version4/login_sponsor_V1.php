...
//join query done by Pranali for SMC-3678 to display revenue_per_visit,points_per_product,discount,points_per_product on 7-12-18
	
	//below query added by Pranali to check if sponsor has setup product or not
	$sql=mysql_query("SELECT sponsor_id FROM tbl_sponsored where sponsor_id='$User_Id'");
	
	
	if(mysql_num_rows($sql) >= 1) 
	{
//if sponsor has setup some discount / product / revenue_per_visit / revenue_percent 
		$getdata = "SELECT sp.*,spd.revenue_percent,spd.revenue_per_visit,spd.discount,spd.points_per_product 
		FROM tbl_sponsorer sp 
		join tbl_sponsored spd on sp.id = spd.sponsor_id
		where (sp.sp_email = '$User_Name' or  sp.sp_phone = '$User_Name') and binary sp.sp_password = '$User_Pass' and sp.id = '$User_Id' order by spd.id desc limit 1";
