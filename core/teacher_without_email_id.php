...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT t_id, t_complete_name, t_email, t_phone, t_internal_email FROM tbl_teacher 
						where (t_email ='' or t_email is null or t_email NOT REGEXP  '^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,3}$') and (t_emp_type_pid =133 OR t_emp_type_pid=134  or t_emp_type_pid=135 or t_emp_type_pid=137)  and school_id='$school_id'
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
		...
		if ($colname != ''and $colname != 'Select')
			...
			$query1=mysql_query(" SELECT  t_id,t_complete_name, t_email, t_phone, t_internal_email FROM tbl_teacher 
									WHERE (t_email ='' or t_email is null or t_email NOT REGEXP  '^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,3}$')
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('teacher_without_email_id.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('teacher_without_email_id.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<!-- Headline -->
	<div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white">Teachers Data without Email id</h2>
			</div>
		...
		<form>
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('teacher_without_email_id.php','_self')" />
			</div>	
        </form>
		<!-- search form END -->	
			...
			<table id="example" class="display" width="100%" cellspacing="0">
					...
					<tbody>
						...
						<tr>
                            ...
                            <td data-title="t_phone" ><?php echo $row['t_internal_email']; ?></td>
