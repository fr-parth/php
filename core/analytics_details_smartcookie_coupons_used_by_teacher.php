...
<head>
    ...
    <title>SmartCookie Coupons Used By Teacher </title>
...
$sql=mysql_query("SELECT t.t_complete_name,t.school_id,tc.amount,tc.coupon_id
 FROM tbl_teacher_coupon tc inner join tbl_teacher t
 on t.id=tc.user_id
 where status='no' or 
 status='p' 
LIMIT $start_from, $webpagelimit");	

$sql1 ="SELECT count(t.t_complete_name),t.school_id,tc.amount,tc.coupon_id
 FROM tbl_teacher_coupon tc inner join tbl_teacher t
 on t.id=tc.user_id
 where status='no' or 
 status='p' "; 
...
//$colname=mysql_real_escape_string($_GET['colname']);
	if ($searchq != '')
	{ 
		$query1=mysql_query("SELECT t.t_complete_name,t.school_id,tc.amount,tc.coupon_id
 FROM tbl_teacher_coupon tc inner join tbl_teacher t
 on t.id=tc.user_id
 where (status='no' or 
 status='p')   and
 
 t.t_complete_name LIKE '%$searchq%' or t.school_id LIKE '%$searchq%'
  or tc.amount LIKE '%$searchq%' or 
  tc.coupon_id LIKE '%$searchq%' 
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
			...
			window.location.assign('analytics_details_smartcookie_coupons_used_by_teacher.php?page='+page);
        });
    });
</script>
<?php }else{
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('analytics_details_smartcookie_coupons_used_by_teacher.php?Search=<?php echo $searchq; ?>&spage='+page);
        });
    });
</script>
<?php }?>
...
<div align="center">
    <div class="container" style="width:100%;">
        <div style="padding-top:30px;">
            ...
            <h2 style="padding-left:20px; margin-top:2px;color:#666"> SmartCookie Coupons Used By Teacher</h2>
		...
		<form style="margin-top:5px;">
			...
			<div class="col-md-1" >
			<input type="button" class="btn btn-info" value="Reset" onclick="window.open('analytics_details_smartcookie_coupons_used_by_teacher.php','_self')" />
			</div>
