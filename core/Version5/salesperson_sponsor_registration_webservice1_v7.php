...
if ($obj->{'User_Name'} != '' && ($obj->{'User_email'} != '' || $obj->{'sp_phone'} != '') && $obj->{'User_pass'} != ''  && $obj->{'Lattitude'} && $obj->{'Longitude'} != '' && $obj->{'country_code'} != '' && $obj->{'sales_p_lon'} != ''  && $obj->{'sales_p_lat'} != '' && $obj->{'User_Image'}!='' && $obj->{'User_imagebase64'} && $obj->{'sponsor_shop_Image'}!='' && $obj->{'sponsor_shop_imagebase64'})
	...
	$school_id=xss_clean(mysql_escape_string($obj->{'school_id'}));
...
$duplicate=mysql_query("select * from tbl_sponsorer where sp_email='$sp_email' or sp_phone='$sp_phone'");
//echo $duplicate;exit;
if (mysql_num_rows($duplicate)>0)
{
echo "PhoneNumber or Email id already exists.";
}
else{

$test1 = "INSERT INTO `tbl_sponsorer`(sp_name,sp_company,sp_email,CountryCode,v_status,v_category, sp_password,sp_address,sp_phone,sp_city,sp_state,sp_country,sales_person_id,lat,lon,sp_date,expiry_date,amount,pin,comment,calback_date_time,v_responce_status,sales_p_lat,sales_p_lon,school_id,platform_source,app_version,payment_method,calculated_lat,calculated_lon,current_marketing,current_market_val,discount,discount_val,digital_marketing,digital_market_val)VALUES('$sponsor_name','$company_name','$sp_email','$country_code','$sp_status','$sp_product_category', '$pass','$sp_address','$sp_phone','$sp_city','$sp_state','$sp_country','$sp_sales_person_id','$lat','$lon','$dates','$date1','$amount','$pin','$comment','$callback_date_time','$v_responce_status','$sales_p_lat','$sales_p_lon','$school_id','$platform_source','$app_version','$payment_method','$calculated_lat','$calculated_lon','$current_marketing','$current_market_val','$discount','$discount_val','$digital_marketing','$digital_market_val')";
