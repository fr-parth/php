...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT DISTINCT std_PRN,std_complete_name,std_class, std_semester, std_div, parent_id, std_email FROM tbl_student 
	...
	$sql1=" SELECT DISTINCT std_PRN FROM tbl_student 
    WHERE (std_class is Null or std_class = ' ')  
         AND std_PRN NOT IN ( SELECT DISTINCT s.std_PRN FROM tbl_student s JOIN Class c ON c.class = s.std_class WHERE s.school_id ='COEP')
         AND school_id='COEP' and Academic_Year='$Academic_Year'; ";
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('students_without_class.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('students_without_class.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<!-- Headline -->
	<div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white"><th><?php echo $dynamic_student;?> Data without <th><?php echo $dynamic_class;?> (<?php echo $total_records; ?>)</h2>
			</div>
...
<!--End of dropdown ----------------------- --- -->
		...
		<form>
            ...
            <div class="col-md-2" style="width:17%;">
                <select name="colname" class="form-control">
					...
					<?php if (($_GET['colname']) == "std_complete_name") {
                            echo $_GET['colname']; ?> selected="selected" <?php } ?>><th><?php echo $dynamic_student;?> Name</option>  
                </select>
			</div>
			<div class="col-md-2" style="width:17%;">
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('students_without_class.php','_self')" />
			</div>	
        </form>
		<br>
		<br>
			...
			<table id="example" class="display" width="100%" cellspacing="0">
					...
					<tbody>
						...
						<tr>
							...
							<td>
                     			<a style="cursor: pointer;" href="edit_student_data_quality.php?std_prn=<?php echo $row['std_PRN']; ?>&flag=Class_Division#id_class"><span class="glyphicon glyphicon-pencil"></span></a>
                        	</td>
						</tr>
						<?php $i++;} ?>     
					</tbody>
