...
if(isset($_POST['submit']))
{
  ...
  // $dynamic_school_admin added by Sayali Balkawade for SMC-4254 on 11/12/2019
		 ...
		 $result_insert11 = mysql_query($insert_stud_rewards) or die(mysql_error());
         $log = "insert into `tbl_student_point` (sc_stud_id,sc_entites_id,sc_studentpointlist_id,sc_point,point_date,school_id,reason,type_points,Stud_Member_Id,sc_teacher_id) values('$stud_id','102','$reason','$points','$date','$school_id',
        ...
        $que= mysql_query($log);
		 ...
		 $updatecount=mysql_affected_rows();
          $log = "insert into `tbl_student_point` (sc_stud_id,sc_entites_id,sc_studentpointlist_id,sc_point,point_date,school_id,activity_type,reason,type_points,Stud_Member_Id,sc_teacher_id) values('$stud_id','102','$reason','$points','$date','$school_id','activity','assigned by $dynamic_school_admin','Greenpoint','$Stud_Member_Id','$id')" ;
        $que= mysql_query($log);
			 if($updatecount>0)
			{		 
			   //$report1="$points Points are given successfully";
			    echo "<script type=text/javascript>alert('$points Points assigned successfully'); window.location='assigngreenpointsstud.php'</script>";
			 }
			 else
			 {
				  echo "<script type=text/javascript>alert('Points are not assigned successfully'); window.location='search_student_reward_points.php'</script>";
			 }
