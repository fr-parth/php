...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT id,Dept_code,Dept_Name,PhoneNo  FROM tbl_department_master where( PhoneNo =''OR PhoneNo IS NULL) AND School_ID='$school_id' 
						LIMIT $start_from, $limit") or die("could not Search!");
 
	$sql1=" SELECT id FROM tbl_department_master
			where( PhoneNo ='' or PhoneNo is null ) and School_ID='$school_id' ";
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
		...
		if ($colname != ''and $colname != 'Select')
				{
			$query1=mysql_query("SELECT id,Dept_code,Dept_Name,PhoneNo  FROM tbl_department_master where( PhoneNo =''OR PhoneNo IS NULL) AND School_ID='$school_id' 
				...
				$sql1 =mysql_query("SELECT id FROM tbl_department_master
									WHERE  (PhoneNo='' or PhoneNo is null ) 
				...
				$query1=mysql_query("SELECT id,Dept_code,Dept_Name,PhoneNo  FROM tbl_department_master where( PhoneNo =''OR PhoneNo IS NULL) AND School_ID='$school_id' 
									...
									AND (id LIKE '$searchq%' or Dept_code LIKE '$searchq%' or Dept_Name LIKE '$searchq%'  or PhoneNo  LIKE '$searchq%') LIMIT $start_from, $limit") or die("could not Search!");
			
				$sql1 =mysql_query("SELECT id FROM tbl_department_master
                					WHERE  (PhoneNo='' or PhoneNo is null ) 
               					 	AND School_ID='$school_id' 
									AND (id LIKE '$searchq%' or Dept_code LIKE '$searchq%' or Dept_Name LIKE '$searchq%' or PhoneNo  LIKE '$searchq%' )") or die("could not Search!");   
...
//$result = mysql_query("SELECT id,Dept_code,Dept_Name,PhoneNo  FROM tbl_department_master where( PhoneNo =''OR PhoneNo IS NULL) AND School_ID='$school_id' ");
?>
<html>
<head>
<style>
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('dept_without_phoneno.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('dept_without_phoneno.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<!-- Headline -->
        <div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white">Department Data  without Phone Number</h2>
			</div>
...
<form>
	...
	<div class="col-md-1">
	<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('dept_without_phoneno.php','_self')" />
	</div>	
</form>
<!-- search form END -->	
				...
				<tbody>
					 ...
					 ?>
						<tr>
							...
							<td data-title="PhoneNo"><?php echo $row['PhoneNo']; ?></td>
							<td> <center>
