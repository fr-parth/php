...
and (semester.BranchId ='' or semester.BranchId  is null )
	...
	$sql1=" SELECT id FROM StudentSemesterRecord 
			where(BranchId  ='' or BranchId  is null ) and school_id='$school_id' and AcdemicYear='$Academic_Year' ";
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
		...
		and (semester.BranchId ='' or semester.BranchId  is null )
				...
				$sql1 =mysql_query("SELECT id FROM StudentSemesterRecord 
									WHERE  (BranchId  ='' or BranchId  is null ) 
...
//$result = mysql_query("SELECT student_id , Semesterid, BranchName, DeptName ,SemesterName,BranchId FROM StudentSemesterRecord where(BranchId   ='' or BranchId is null ) and School_ID='$school_id'");
?>
<html>
<head>
<style>
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('studsem_without_branchid.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('studsem_without_branchid.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white"> Student Semester Data without  Branch Id (<?php echo $total_records; ?>)</h2>
        </div>
...
<form>
	...
	<div class="col-md-1">
	<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('studsem_without_branchid.php','_self')" />
	</div>	
</form>
<!-- search form END -->	
