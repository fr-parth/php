...
?>
        		...
        		<tr  style="background-color:#428BCA; color:#FFFFFF; height:30px;">
                    ...
                    <th>Point given to <?php echo $dynamic_student?> as a reward</th>
...
<div style="height:10px;"></div>
        ...
        <h2 style="padding-left:20px; margin-top:2px;color:#666">Green Points Given to <?php echo $dynamic_teacher?> for Distribution</h2>
        </div>
...
//Below query is updated by Sayali Balkawade(PHP Intern) on 24/04/2019 to solve bug SMC-3821 

               $arrs = mysql_query("SELECT t.id,t.t_id, t_complete_name, t.tc_balance_point,  sum(st.sc_point) as sum, st.sc_entites_id FROM tbl_teacher t
						LEFT JOIN tbl_student_point st ON t.t_id = st.sc_teacher_id and t.school_id=st.school_id
						WHERE (`t_emp_type_pid`='133' or `t_emp_type_pid`='134' or `t_emp_type_pid`='135' or `t_emp_type_pid`='137') AND t.school_id = '$school_id' and t.tc_balance_point!=0 group by t.t_complete_name ORDER BY t.t_complete_name ASC ");
			  ...
			  ?>
				...
				<tr>
                    ...
                    <td><?php echo $teacher['t_complete_name'];?></td>
                    <td><?php if($teacher['sum']!=""){echo $teacher['sum'];}else{echo "0";}?></td>
                    <td><?php echo $teacher['tc_balance_point'];?></td>
				</tr>
