...
and semester.`IsCurrentSemester`='1' 
    ...
    and AcdemicYear='$Academic_Year'and(semester.DeptName  ='' or semester.DeptName is null ) 
						LIMIT $start_from, $limit") or die("could not Search!");
 
	$sql1=" SELECT id FROM StudentSemesterRecord 
			where(DeptName ='' or DeptName is null ) and school_id='$school_id'  and AcdemicYear='$Academic_Year'";
	...
	and AcdemicYear='$Academic_Year'and(semester.DeptName  ='' or semester.DeptName is null ) 
				...
				$sql1 =mysql_query("SELECT id FROM StudentSemesterRecord 
									WHERE  (DeptName ='' or DeptName  is null ) 
...
//$result = mysql_query("SELECT student_id , Semesterid, BranchName, DeptName ,SemesterName,DepartmentId FROM StudentSemesterRecord where(DeptName  ='' or DeptName is null ) and School_ID='$school_id'");
?>
<html>
<head>
<style>
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('studsem_without_dept.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('studsem_without_dept.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white"> Student Semester Data without  Department (<?php echo $total_records; ?>)</h2>
</div>
...
<form>
	...
	<div class="col-md-2" style="width:17%;">
		<select name="colname" class="form-control">
			...
			<?php if (($_GET['colname']) == "DepartmentId") {
	...
	<div class="col-md-1">
	<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('studsem_without_dept.php','_self')" />
	</div>	
</form>
<!-- search form END -->	
