	...
	{ 
	 ...
	 $handle = fopen($file, "r");
		...
		//NO VALIDATION FOR SUBJECT CODE
				if($student_id == '' || $school_id == '' ||  $subject_name == '')
				{
					$ok = 0;
					$msg = "Either Student Id, School Id, Subject Code or Subject Name is missing.";

				}

				$myquery = "select std_PRN from $tbl_student where std_PRN='$student_id' and group_member_id = '$group_member_id';";
				$myres = mysql_query($myquery);
				$count = mysql_num_rows($myres);

				$myquery1 = "select school_id,Subject_Code from $school_sub_table where school_id='$school_id' and subject = '$subject_name' and group_member_id='$group_member_id';";
				...
				if($count > 0 )
				{
					...
					if($count2 > 0)
					{
						...
						if($update_res)
						{
							if($count1 > 0)
							{
								$sub_update = "UPDATE $main_table a,$school_sub_table b SET a.subjcet_code = b.Subject_Code where a.school_id = b.school_id and a.subjectName = b.subject and a.student_id = '$student_id' and a.group_member_id=b.group_member_id and a.group_member_id = '$group_member_id';";
								$sub_update_res = mysql_query($sub_update);
							}
							$a++;
						}
					}
				...
				{
					$ok = 0;
					$msg .= "Subject Code does not exist for school.";
				}*/
...
?>
	<?php if($j > 0){?>
		...
		$myquery1 = "select error_msg,school_id, student_id, subjcet_code,subjectName,upload_date from $raw_table where group_member_id = '$group_member_id' and upload_date > timestamp(DATE_SUB(NOW(), INTERVAL 10 MINUTE)) order by upload_date DESC";
		$myres1 = mysql_query($myquery1);
		$k = 1;
