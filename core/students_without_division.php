...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT std_PRN, std_complete_name, std_semester, std_class, std_div, parent_id, std_email, std_phone FROM tbl_student 
						where (std_div ='' or std_div is null) and school_id='$school_id' and Academic_Year='$Academic_Year'
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
		...
		if ($colname != ''and $colname != 'Select')
			...
			$query1=mysql_query(" SELECT std_PRN, std_complete_name, std_semester, std_class, std_div, parent_id, std_email, std_phone FROM tbl_student 
									WHERE (std_div ='' or std_div is null ) 
				...
				$query1=mysql_query("SELECT std_PRN, std_complete_name, std_semester, std_class, std_div, parent_id, std_email, std_phone FROM tbl_student 
									WHERE (std_div ='' or std_div is null ) and Academic_Year='$Academic_Year'
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('students_without_division.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('students_without_division.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<!-- Headline -->
	<div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white"><?php echo $dynamic_student;?> Data without Division (<?php echo $total_records; ?>)</h2>
			</div>
...
<!--End of dropdown ----------------------- --- -->
		...
		<form>
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('students_without_division.php','_self')" />
			</div>	
        </form>
		<!-- search form END -->	
			...
			<table id="example" class="display" width="100%" cellspacing="0">
					...
					<tbody>
						...
						<tr>
							...
							<?php }?>
							<td>
                     			<a style="cursor: pointer;" href="edit_student_data_quality.php?std_prn=<?php echo $row['std_PRN']; ?> &flag=Class_Division#id_div"><span class="glyphicon glyphicon-pencil"></span></a>
                        	</td>
						</tr>
						<?php $i++;} ?>     
					</tbody>
