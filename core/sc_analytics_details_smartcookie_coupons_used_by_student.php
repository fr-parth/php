...
if (!($_GET['Search'])){
	...
	$sql=mysql_query("SELECT * FROM tbl_coupons as student_smartcookie_used where (status = 'no' or status = 'p') AND school_id = '$school_id' LIMIT $start_from, $webpagelimit") or die("Could not Search!");

	$sql1 =mysql_query("SELECT * FROM tbl_coupons as student_smartcookie_used where (status = 'no' or status = 'p') AND school_id = '$school_id'");
...
else
{
	...
	//$colname=mysql_real_escape_string($_GET['colname']);
			...
			$sql = mysql_query("SELECT * FROM tbl_coupons as student_smartcookie_used where (status = 'no' or status = 'p') AND school_id = '$school_id'  AND (stud_complete_name LIKE '%$searchq%' or school_id LIKE '%$searchq%' or cp_code LIKE '%$searchq%' or amount LIKE '%$searchq%') LIMIT $start_from, $webpagelimit") or die("Could not Search!");
							
			$sql1 = mysql_query("SELECT * FROM tbl_coupons as student_smartcookie_used where (status = 'no' or status = 'p') AND school_id = '$school_id'  AND (stud_complete_name LIKE '%$searchq%' or school_id LIKE '%$searchq%' or cp_code LIKE '%$searchq%' or amount LIKE '%$searchq%')");
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
			...
			window.location.assign('sc_analytics_details_smartcookie_coupons_used_by_student.php?page='+page);
        });
    });
</script>
<?php }else{
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('sc_analytics_details_smartcookie_coupons_used_by_student.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
        });
    });
</script>
<?php }?>
...
<div align="center">
    <div class="container" style="width:100%;">
        <div style="padding-top:30px;">
            ...
            <h2 style="padding-left:20px; margin-top:2px;color:#666"> SmartCookie Coupons Used By <?php echo $dynamic_student;?></h2>
		...
		<form style="margin-top:5px;">
			...
			<div class="col-md-1" >
			<input type="button" class="btn btn-info" value="Reset" onclick="window.open('sc_analytics_details_smartcookie_coupons_used_by_student.php','_self')" />
			</div>
			<div class="col-md-1" >
...
<div class="container">
	<div class="panel panel-default">
		<div class="panel-heading" align='center'><h2>Smartcookie coupons used By <?php echo $dynamic_student;?></h2></div>
		<div class="panel-body">
			<div id="no-more-tables" style="padding-top:20px;">
			<table id="example" class="display" width="100%" cellspacing="0">
				<thead style="background-color:#FFFFFF;">
									
					<tr><!-- Camel casing done for Coupon ID& School ID by Pranali-->
					<th>Sr.No.</th>
