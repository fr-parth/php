 ...
 $to_date = date("d-m-Y H:i",mktime(23,59));//hour, minute, second
  ...
  // print_r($_POST); exit;
     ...
     // $where .= " AND date_format(ls.LatestLoginTime,'%Y-%m-%d')='$current_dt'";
      ...
      $student_sql = "SELECT count(DISTINCT(ls.EntityID)) as cnt, ls.school_id, sa.school_name FROM tbl_LoginStatus as ls JOIN tbl_student as s ON ls.EntityID=s.id and ls.school_id=s.school_id JOIN tbl_school_admin as sa ON ls.school_id=sa.school_id where ls.school_id='$school_id' AND (ls.Entity_type='105' OR ls.Entity_type='205') AND ls.LatestMethod='$type_nm' AND ls.LatestLoginTime between '$from_dt' AND '$to_dt' group by ls.school_id order by sa.school_name";
...
<div class="container">
  <div class="row">
        ...
        <div class="radius" style="height:50px; background-color:#428BCA;" align="center">
          <h2 style="padding-left:20px;padding-top:10px;color:white"><?= $type_nm; ?> Login-Logout School-wise <?php echo $dynamic_student; ?> Statistics</h2>
        </div>
  </div>
  ...
  <div class="row" style="padding-top:10px;">
      ...
      <form method="post" action="">
        ...
        <div class="col-md-5">
          ...
          <div class="col-sm-9" id="typeedit">
            <input type="text" class="form-control datetimepic" name="fd" autocomplete="off" id="fromdt" value="<?php if(isset($_POST)){if($f_dt!=""){echo date('d-m-Y H:i',strtotime($f_dt)); }else{ echo $fr_date; }}  else{ echo $fr_date; } ?>">    
          </div>
        </div>
        <div class="col-md-5">
    ...
    <br>
    <form method="post" class="detail_color" style="cursor: pointer; " action="login-logout-Status.php">
    ...
    <div class="col-md-12">
      <table class="table-bordered  table-condensed cf" id="example" width="100%;" >
                                ...
                                <?php
                                    ...
                                    ?>
                                    <form method="post" class="detail_color" style="cursor: pointer;" action="Login_logout_school_student1.php">
                                      ...
                                      <button type="submit" id="student_web<?= $i;?>" style="display: none;"></button>
                                      <tr style="color:#808080;" class="active detail_color" onclick="student_web(<?= $i;?>);">
...
<script type="text/javascript">
   ...
   function student_web(i) {
        $("#student_web"+i).trigger( "click" );
    }
</script>
</body>
</html>