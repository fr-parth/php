		...
		if ($colname != ''and $colname != 'Select')
			...
			$query1=mysql_query(" SELECT  ts.ExtSchoolSubjectId, ts.subjectName, ts.teacher_id, t.t_complete_name ,ts.ExtSemesterId, ts.Semester_id 
				...
				$query1=mysql_query("SELECT  ts.ExtSchoolSubjectId, ts.subjectName, ts.teacher_id, t.t_complete_name ,ts.ExtSemesterId, ts.Semester_id 
										...
										WHERE (ts.ExtSemesterId ='' or ts.ExtSemesterId is null ) 
										AND  ts.school_id = '$school_id' and ts.AcademicYear='$Academic_Year' 
									AND (ts.ExtSemesterId LIKE '$searchq%' or ts.ExtSchoolSubjectIdail_Id LIKE '$searchq%' or ts.subjectName  LIKE '%$searchq%' or ts.teacher_id  LIKE '%$searchq%' or ts.Semester_id  LIKE '%$searchq%'or t.t_complete_name LIKE '%$searchq%') LIMIT $start_from, $limit") or die("could not Search!");
				...
				WHERE (ts.ExtSemesterId ='' or ts.ExtSemesterId is null ) 
				AND  ts.school_id = '$school_id' and ts.AcademicYear='$Academic_Year' 
									AND (ts.ExtSemesterId LIKE '$searchq%' or ts.ExtSchoolSubjectId LIKE '$searchq%' or ts.subjectName LIKE '%$searchq%' or ts.teacher_id  LIKE '%$searchq%' or ts.Semester_id LIKE '%$searchq%'or t.t_complete_name  LIKE '%$searchq%' )") or die("could not Search!");   
...
//$result = mysql_query("SELECT * FROM tbl_teacher_subject_master where(ExtSemesterId='' or ExtSemesterId is null ) and School_ID='$school_id'");
?>
<html>
<head>
<style> 
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('teacher_subject_without_semester.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('teacher_subject_without_semester.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<!-- Headline -->
	<div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white"><?php echo $dynamic_teacher;?> <?php echo $dynamic_subject;?> Data without <?php echo $dynamic_semester;?> (<?php echo $total_records; ?>) </h2>
			</div>
...
<!--End of dropdown ----------------------- --- -->
		...
		<form>
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('teacher_subject_without_semester.php','_self')" />
			</div>	
        </form>
		<!-- search form END -->	
