...
Date : 13-05-20
This file was created for displaying Student's brown point log in School Admin 
*/
...
if (!isset($_GET['submit'])) {
	...
	$result = mysql_query("SELECT sp.activity_type,s.id,sp.sc_stud_id,sp.sc_entites_id,sp.sc_point,sp.reason,sp.point_date,sp.source,sp.category,sp.sub_category, (CASE WHEN s.std_complete_name!='' THEN s.std_complete_name
	ELSE CONCAT_WS(' ', s.std_name , s.std_Father_name, s.std_lastname)    
    END) as stud_name
	from tbl_student s
	join tbl_student_point sp
	ON s.std_PRN = sp.sc_stud_id  AND  s.school_id = sp.school_id 
	where (sp.type_points='Brown' or sp.type_points='BrownPoints' or sp.type_points='Brown Points') and sp.school_id='$school_id' ORDER BY sp.id DESC LIMIT $start_from,$webpagelimit");
	...
	join tbl_student_point sp
	ON s.std_PRN = sp.sc_stud_id  AND  s.school_id = sp.school_id 
	where (sp.type_points='Brown' or sp.type_points='BrownPoints' or sp.type_points='Brown Points') and sp.school_id='$school_id' ORDER BY sp.id DESC "); 

	//$result1 = mysql_query($arr1); //count 
...
else
{
	...
	$to_date2=mysql_real_escape_string($_GET['to_date']);
	...
	$stud_member_id=mysql_real_escape_string($_GET['stud_member_id']); 
	$stud_prn=mysql_real_escape_string($_GET['stud_prn']);  
//print_r($_GET);
	// if($from_date2==''){
	...
	$to_date .=  ' 23:59:59';
	$arr = "SELECT sp.activity_type,s.id,sp.sc_stud_id,sp.sc_entites_id,sp.sc_point,sp.reason,sp.point_date,sp.source,sp.category,sp.sub_category, (CASE WHEN s.std_complete_name!='' THEN s.std_complete_name
	ELSE CONCAT_WS(' ', s.std_name , s.std_Father_name, s.std_lastname)    
    END) as stud_name  
	from tbl_student s
	join tbl_student_point sp
	ON s.std_PRN = sp.sc_stud_id  AND  s.school_id = sp.school_id 
	where (sp.type_points='Brown' or sp.type_points='BrownPoints' or sp.type_points='Brown Points') and sp.school_id='$school_id' ";
	$cond='';
	if($colname!="Select"){
		$cond .= " AND sp.activity_type LIKE '%$colname%' ";
	}
	if($searchq!=''){
		$cond .= " AND s.std_complete_name LIKE '%$searchq%' ";
	}
	...
	if($stud_member_id!=''){
		$cond .= " AND s.id  LIKE '%$stud_member_id%' ";
	}	
	if($stud_prn!=""){
		$cond .= " AND sp.sc_stud_id LIKE '%$stud_prn%' ";
	}
	...
	join tbl_student_point sp
	ON s.std_PRN = sp.sc_stud_id  AND  s.school_id = sp.school_id 
	where (sp.type_points='Brown' or sp.type_points='BrownPoints' or sp.type_points='Brown Points') and sp.school_id='$school_id'  $cond ORDER BY sp.id DESC"); 
		...
		<script type="text/javascript">
			$(function () {
				...
				window.pagObj = $('#pagination').twbsPagination({
					...
					window.location.assign('student_brown_point_log.php?page='+page);
				});
			});
		</script>
	<?php }else{
		...
		<script type="text/javascript">
			$(function () {
				...
				window.pagObj = $('#pagination').twbsPagination({
					...
					console.info(page + '(from event listening)');
					window.location.assign('student_brown_point_log.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page+'&submit=<?php echo $_GET['submit']?>&from_date=<?php echo $from_date2; ?>&to_date=<?php echo $to_date2;?>&source=<?php echo $source; ?>&category=<?php echo $category; ?>&sub_category=<?php echo $sub_category; ?>&reason=<?php echo $reason; ?>&stud_member_id=<?php echo $stud_member_id; ?>&stud_prn=<?php echo $stud_prn; ?>');
				});
			});
		</script>
	<?php }?>
			...
			<div style="border:1px solid #CCCCCC; border:1px solid #CCCCCC;box-shadow: 0px 1px 3px 1px #C3C3C4;">
				<div style="background-color:#F8F8F8 ;">
					<div class="row">
						...
						<div align="center"  >
							<div style="font-size:34px;">Student Brown Point Log </div>
								...
								<div class="form-group col-md-4">
									<label class='control-label' for='colname' style="text-align:left;">Activity Type :</label>&emsp;
										...
										<?php	}
											...
											<?php if (($_GET['colname']) == "activity") {
												echo $_GET['colname']; ?> selected="selected" <?php } ?>>  Activity </option> 
											<option value="subject"
											<?php if (($_GET['colname']) == "subject") {
												echo $_GET['colname']; ?> selected="selected" <?php } ?>>  Subject </option> -->
							...
							<div class="row">
								...
								<div class="form-group col-md-3">
									&emsp;
									<label class='control-label' for='stud_member_id' > Student Member ID :</label>&emsp;
												<input type="text" class="form-control" name="stud_member_id" id="stud_member_id" value="<?php echo $stud_member_id; ?>" placeholder="Search Student Member ID.."  autocomplete="off">
							...
							<div class="row">
								<div class="form-group col-md-3">
										<label class='control-label' for='stud_prn' style="text-align:left;"> Student PRN :</label>&emsp;
												<input type="text" class="form-control" name="stud_prn" id="stud_prn" value="<?php echo $stud_prn; ?>" placeholder="Search Student PRN.."  autocomplete="off">
												&emsp;
								</div>
								<div class="form-group col-md-3">

											<label class='control-label' for='Search' style="text-align:left;"> Student Name :</label>&emsp;
												<input type="text" class="form-control" name="Search" id="Search" value="<?php echo $searchq; ?>" placeholder="Search Student Name.."  autocomplete="off">	 
											&emsp;
								</div>
								...
								<div class="form-group col-md-3">
												<input type="button" class="btn btn-info" value="Reset" onclick="window.open('student_brown_point_log.php','_self')" style="margin-top:27px;"/>
								...
								else
								{
									...
									<div class="row" style="padding:10px;">
										<div id="no-more-tables" class="col-md-12  ">
											...
											<td data-title="prn"><?php echo $res['stud_name']."<br>".$res['sc_stud_id'] ?> </td>
											<!-- <td data-title="stud_name"><?php echo $res['stud_name'] ?></td>	 -->
