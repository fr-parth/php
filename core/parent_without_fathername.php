...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT Id,Parent_Member_id,Father_name,std_PRN,Name,Father_name,Mother_name 
						FROM  tbl_parent 
						WHERE (Father_name ='' or Father_name is null) 
							AND school_id = '$school_id'
						LIMIT $start_from, $limit") or die("could not Search!");;
 
	$sql1=" SELECT Parent_Member_id,Father_name,std_PRN,Name,Father_name,Mother_name 
			FROM  tbl_parent 
			WHERE (Father_name ='' or Father_name is null) 
			AND school_id = '$school_id' ";
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };  
		...
		if ($colname != ''and $colname != 'Select')
				{
			$query1=mysql_query("SELECT Id,Parent_Member_id,Father_name,std_PRN,Name,Father_name,Mother_name 
								FROM  tbl_parent 
								WHERE (Father_name ='' or Father_name is null) 
									AND school_id = '$school_id'
									AND `$colname` LIKE '%$searchq%' LIMIT $start_from, $limit") or die("could not Search!");

				$sql1 =mysql_query("SELECT Parent_Member_id,Father_name,std_PRN,Name,Father_name,Mother_name 
									FROM  tbl_parent 
									WHERE (Father_name ='' or Father_name is null) 
										AND school_id = '$school_id'
				...
				$query1=mysql_query("SELECT Id,Parent_Member_id,Father_name,std_PRN,Name,Father_name,Mother_name 
									...
									WHERE (Father_name ='' or Father_name is null) 
										AND school_id = '$school_id'
										AND (Parent_Member_id LIKE '$searchq%' or std_PRN LIKE '$searchq%' or Name LIKE '$searchq%' or Mother_name LIKE '$searchq%'  or Father_name LIKE '$searchq%' ) LIMIT $start_from, $limit") or die("could not Search!");
			
				$sql1 =mysql_query("SELECT Parent_Member_id,Father_name,std_PRN,Name,Father_name,Mother_name 
									FROM  tbl_parent 
									WHERE (Father_name ='' or Father_name is null) 
										AND school_id = '$school_id'
										AND (Parent_Member_id LIKE '$searchq%' or std_PRN LIKE '$searchq%' or Name LIKE '$searchq%' or Mother_name LIKE '$searchq%'  or Father_name LIKE '$searchq%' )") or die("could not Search!");   
  	...
  	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('parent_without_fathername.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('parent_without_fathername.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
	});
	});
</script>
<?php }?>
...
<body>
	...
	<div class="container">
		<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white">Parent Data without Father Name</h2>
		</div>
						...
						<tbody>
							...
							?>
								<tr>
									...
									<td> <center>
                     					<a style="cursor: pointer;" href="edit_parent_data_quality.php?id=<?php echo base64_encode($row['Id']); ?>&flag=fathername"><span class="glyphicon glyphicon-pencil"></span></a>
										</center>
									</td>
								</tr>
							<?php $i++;} ?>     
