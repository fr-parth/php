...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT std_PRN, std_complete_name, std_semester, std_class, std_div, parent_id, std_dob FROM tbl_student 
						where (std_dob ='' or std_dob is null or std_dob = '0000-00-00') and school_id='$school_id' and Academic_Year='$Academic_Year'
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
		...
		if ($colname != '' and $colname != 'Select')
				{
			$query1=mysql_query(" SELECT std_PRN, std_complete_name, std_semester, std_class, std_div, parent_id, std_dob FROM tbl_student 
									WHERE  (std_dob ='' or std_dob is null or std_dob = '0000-00-00')
				...
				$sql1 =mysql_query("SELECT id FROM tbl_student 
									WHERE  (std_dob ='' or std_dob is null or std_dob = '0000-00-00')
									and school_id='$school_id' and Academic_Year='$Academic_Year'
									AND `$colname` LIKE '%$searchq%'") or die("set could not Search!");
				$total_records = mysql_num_rows($sql1);   
				$total_pages = ceil($total_records / $limit);
		}else{	
				$query1=mysql_query("SELECT std_PRN, std_complete_name, std_semester, std_class, std_div, parent_id, std_dob 
				FROM tbl_student 
				WHERE (std_dob ='' or std_dob is null or std_dob = '0000-00-00') 
				AND  school_id='$school_id' and Academic_Year='$Academic_Year' AND (std_complete_name LIKE '%$searchq%' or std_email like '%$searchq%')  LIMIT $start_from, $limit") or die("could not Search!");
				...
				$sql1 =mysql_query("SELECT id FROM tbl_student 
									WHERE (std_dob ='' or std_dob is null or std_dob = '0000-00-00') 
				AND  school_id='$school_id' and Academic_Year='$Academic_Year' AND (std_complete_name LIKE '%$Search%' or std_email like '%$searchq%')") or die("could not Search!");   
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('students_without_dob.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('students_without_dob.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<!-- Headline -->
	<div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white"><?php echo $dynamic_student;?> Data without Date of Birth (<?php echo $total_records; ?>)</h2>
			</div>
...
<!--End of dropdown ----------------------- --- -->
		...
		<form>
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('students_without_dob.php','_self')" />
			</div>	
        </form>
		<!-- search form END -->	
			...
			<table id="example" class="display" width="100%" cellspacing="0">
					<thead style="background-color:#FFFFFF;">
						...
						<tr><!-- Camel casing done for Member ID and School ID by Pranali-->
							...
							<th>Parent Id</th>
							<?php }?>
							<th><?php echo $dynamic_student;?> Dob</th>
							<th>Edit</th>
						</tr>
					</thead>
					<tbody>
						...
						<tr>
							...
							<?php }?>
							<td data-title="std_dob"><?php echo $row['std_dob']; ?></td>
							<td>
                     			<a style="cursor: pointer;" href="edit_student_data_quality.php?std_prn=<?php echo $row['std_PRN']; ?>&flag=dob#dob"><span class="glyphicon glyphicon-pencil"></span></a>
                        	</td>
						</tr>
						<?php $i++;} ?>     
					</tbody>
