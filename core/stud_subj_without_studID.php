	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };  
		...
		if ($colname != ''and $colname != 'Select')
			...
			$query1=mysql_query("SELECT ss.id,ss.Stud_Member_Id,ss.ExtSemesterId,ss.subjcet_code,ss.subjectName,ss.student_id ,
				...
				$sql1 =mysql_query("SELECT id,Stud_Member_Id,ExtSemesterId,subjcet_code,subjectName,student_id 
									FROM tbl_student_subject_master 
									WHERE (student_id = '' OR student_id is NULL) 
										AND school_id = '$school_id' and AcademicYear='$Academic_Year'
				...
				$query1=mysql_query("SELECT ss.id,ss.Stud_Member_Id,ss.ExtSemesterId,ss.subjcet_code,ss.subjectName,ss.student_id ,
										...
										(select std_email from tbl_student where school_id = '$school_id' and AcademicYear='$Academic_Year' and std_PRN = ss.student_id ) as std_email
											...
											WHERE (ss.student_id = '' OR ss.student_id is NULL)         
												AND ss.school_id = '$school_id' and ss.AcademicYear='$Academic_Year'
										AND (ss.Parent_Member_id LIKE '$searchq%' or ss.std_PRN LIKE '$searchq%' or ss.Name LIKE '$searchq%') LIMIT $start_from, $limit") or die("could not Search!");
			
				$sql1 =mysql_query("SELECT id,Stud_Member_Id,ExtSemesterId,subjcet_code,subjectName,student_id 
									FROM tbl_student_subject_master 
									WHERE (student_id = '' OR student_id is NULL) 
										AND school_id = '$school_id' and AcademicYear='$Academic_Year'
										AND (Stud_Member_Id LIKE '$searchq%' or ExtSemesterId LIKE '$searchq%' or subjcet_code LIKE '$searchq%' )") or die("could not Search!");   
...
//$result = mysql_query("SELECT Parent_Member_id,Father_name,std_PRN,Name,Father_name,Mother_name FROM  tbl_parent where (Father_name ='' or Father_name = null) AND school_id = '$school_id'");
?>
<html>
<head>
<style>
  	...
  	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('stud_subj_without_studID.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('stud_subj_without_studID.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
	});
	});
</script>
<?php }?>
...
<body>
	...
	<div class="container">
		<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white">Student Subject Data without Student ID (<?php echo $total_records; ?>)</h2>
		</div>
...
<!--End of dropdown ----------------------- --- -->
	...
	<!-- search form -->
		...
		<form>
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('stud_subj_without_studID.php','_self')" />
			</div>
						...
						<tbody>
							...
							?>
								<tr>
									...
									<td> <center>
                     						<a style="cursor: pointer;" href="edit_student_subject_data_quality.php?id=<?php echo $row['id']; ?>" ><span class="glyphicon glyphicon-pencil"></span></a>
										</center>
									</td>
								</tr>
							<?php $i++;} ?>     
