...
if ($std_PRN != '') {
    ...
    if (isset($_POST['update'])) {
        if ($std_PRN != '') {
            $stud_member_id=$_POST['member_id'];
...
list($month,$day,$year) = explode("/",$std_dob);
    ...
    $dateformat=$year. "-" .$month. "-" .$day;
     $result1 = mysql_query("select * from tbl_teacher where t_id='$std_PRN' AND school_id='$school_id'");
     $var=mysql_num_rows($result1);
     
          if ($var <= 0) {
            /*Insert query updated by Rutuja for adding school_type & entity_type_id for solving the issue SMC-4810 on 05-09-2020*/  
                $sqls = "INSERT INTO `tbl_teacher`(t_id,t_name,t_middlename,t_lastname ,t_complete_name,school_id 
      ,t_designation,t_class, t_dept,t_DeptID,t_address, t_city, t_dob, t_gender,t_age, t_country, t_email,t_date,state,t_phone,t_password,t_permanent_village,t_permanent_taluka,t_permanent_district,t_permanent_pincode,Stud_Member_Id,t_emp_type_pid,school_type,entity_type_id) VALUES ('$std_prn','$fname','$mname','$lname','$full_name','$school_id','$Specialization','$std_class','$DeptName','$Deptid','$std_p_address','$std_cty','$std_dob','$std_gen','$age' ,'$std_country', '$std_email_id','$dates','$std_state','$std_mob','$password','$PermanentVillage','$PermanentDistrict','$PermanentDistrict','$PermanentPincode','$stud_member_id','$t_emp','organization','203')";

                $count = mysql_query($sqls) or die(mysql_error());
                $insert_id=mysql_insert_id();
                if ($count >= 1) {
                    $sql_email=mysql_query("select * from tbl_email_parameters where e_id='3'");
                    $res_email=mysql_fetch_assoc($sql_email);
                    $sql_update11 = "UPDATE `tbl_student` SET promotion='1',Teacher_Member_Id='$insert_id' WHERE std_PRN='$std_PRN' AND school_id='$school_id'";
                    $retval11 = mysql_query($sql_update11) or die('Could not update data: ' . mysql_error());
                    
                    $to = $email;
                    // $from = "smartcookiesprogramme@gmail.com";
                    $from = $res_email['email_id'];
                    $subject = "You are promoted as a Manager";
                    ...
                    echo "<script type=text/javascript>alert('Employee Promoted Successfully '); window.location='studentlist.php'</script>";
                }
            } else {
                
                echo "<script type=text/javascript>alert('Employee allready promoted to manager'); window.location='studentlist.php'</script>";
...
<div style="font-size:15px;font-weight:bold;margin-top:5px;" class="col-md-offset-6"><div style="color:#F00"><?php if(isset($_GET['errorreport'])){ echo $_GET['errorreport']; };echo $errorreport;?></div><div style="color:#090"><?php if(isset($_GET['successreport'])){ echo $_GET['successreport']; };echo $successreport;?></div></div>
    <div class='panel-heading'>
        <h3 align="center">Employee To Manager Promotion</h3>
      ...
      <div class='panel-body'>
        <form class='form-horizontal' role='form' method="POST" action="" >
                ...
                <div class='form-group'>
                     <label class='control-label col-md-2 col-md-offset-2' style="text-align:left;">Manager ID  <span class="error"><b style="color:red";> *</b></span></label>
                    
                    <input class='form-control' id='member_id' name="member_id" type='hidden' value="<?php echo $Student_Member_Id; ?>">
               ...
               <!--     Changes done (added error1 in class) by Pranali on 30-06-2018 for bug SMC-3201 -->
                 ...
                 <div class="row">
                    <label class='control-label col-md-2 col-md-offset-2'  style="text-align:left;"><?php echo $dynamic_teacher;?> Name <span class="error"><b style="color:red";> *</b></span></label>
                     <div class='col-md-3' style="text-align:left;">
    ...
    <!--     <div class='form-group'>
                ...
                <div class='col-md-3'>
                    <select name='CourseLevel' id='CourseLevel' class='form-control' onChange="Relationfunction(this.value,'fun_course')">
                     <option value="<?php//echo $Course_level; ?>"><?php// echo $Course_level; ?></option>
                        <?php 
                /*      $sql = "select DISTINCT CourseLevel  from tbl_CourseLevel where school_id='$school_id' and ExtCourseLevelID !=''";
            ...
            -->
                  ...
                  <div class='form-group'>
                <label class='control-label col-md-2 col-md-offset-2' for='id_accomodation' style="text-align:left;">Manager Type<b style="color:red";> *</b></span></label>
                <div class='col-md-3'>
                    <select name='t_emp_type_pid' id='t_emp_type_pid' class='form-control' onChange="Myfunction(this.value,'fun_emp_type_edit')" required>
                         <?php
                       // echo $t_emp_type;
                         //echo $t_emp_type;
                        if ($t_emp_type != '') {
                        ...
                        <?php if ($t_emp != $dynamic_teacher ) { ?>
                            <option <?php if($school_type=="organization"){ echo "value='134'";}else{ echo "value='133'";}?>><?php echo $dynamic_teacher; ?></option>
                        <?php }
                ...
                <div class='col-md-3'>
                    ...
                    <option value="<?php echo $ExtDeptId.','.$std_dept; ?>"><?php echo $std_dept; ?></option>
                        <?php 
                ...
                <div class='col-md-3'>
                    <select name='BranchName' id='BranchName' class='form-control' onChange="Relationfunction(this.value,'fun_branch')">
                     <option value="<?php echo $ExtBranchId.','.$std_branch;?>"><?php echo $std_branch;?></option>
                        <?php 
                        $sql = "select * from tbl_branch_master where school_id='$school_id'";
                        $result = mysql_query($sql);
                        while($row=mysql_fetch_array($result))
                        {
                ...
                <?php if ($_SESSION['usertype'] == 'School Admin') {?>  
                  ...
                  <select name='SemesterName' id='SemesterName' class='form-control'>
                    <option value="<?php echo $ExtSemesterId.','.$std_semester;?>"><?php echo $std_semester;?></option>
                        <?php 
                        $sql = "select * from tbl_semester_master where school_id='$school_id'";
                        $result = mysql_query($sql);
                        while($row=mysql_fetch_array($result))
                        {
                ...
                <div class='col-md-3' style="text-align:left;">
                    <select name='id_div' id='id_div' class='form-control'>
                     <?php
                        ...
                        <?php } ?>
                        <?php $arr = mysql_query("select * from Division where school_id='$school_id' and DivisionName != ''"); 
                         while ($row = mysql_fetch_array($arr)) { 
                 ...
                 <?php
                  ...
                  echo $reporting_manager_id;
                        if ($reporting_manager_id != '') {
                            ...
                            <option value='-1'>Select Reporting Manager</option>
                        <?php } ?>
                        <?php $arr = mysql_query("select t_id,t_complete_name,t_emp_type_pid from tbl_teacher where school_id='$school_id'  order by t_complete_name"); 
                         while ($row = mysql_fetch_array($arr)) { 
                ...
                <div class='col-md-3' style="text-align:left;">
                 <select name='Intruduce_YeqarID' id='Intruduce_YeqarID' class='form-control'>
                     <?php
                        ...
                        ?>
                            <option <?php if ($Academic_Year=$AcademicYear) {echo "selected"; } ?> value='<?php echo $AcademicYear;?>'><?php echo $AcademicYear;?></option>
                         <?php }?>
