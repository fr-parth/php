...
switch($_GET['fn']){
	...
	case 'fun_course':
		$row=mysql_query("select * from  tbl_department_master where school_id='$sc_id' "); 
		...
		$b = explode (",", $a);
		$c = $b[1];
		$row=mysql_query("select * from  tbl_semester_master where Branch_name='$c' and school_id='$sc_id'"); 
			echo "<option>select</option>";
		  while($val=mysql_fetch_array($row))
		  {
