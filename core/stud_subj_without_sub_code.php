...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT ss.id,ss.Stud_Member_Id,ss.ExtSemesterId,ss.subjcet_code,ss.subjectName,ss.student_id ,
						(select std_complete_name from tbl_student where school_id = '$school_id' and AcademicYear='$Academic_Year' and std_PRN = ss.student_id ) as std_name,
						(select std_email from tbl_student where school_id = '$school_id' and AcademicYear='$Academic_Year' and std_PRN = ss.student_id ) as std_email

							FROM tbl_student_subject_master ss       
							WHERE (ss.subjcet_code = '' OR ss.subjcet_code is NULL)         
								AND ss.school_id = '$school_id' and ss.AcademicYear='$Academic_Year'
						LIMIT $start_from, $limit") or die("could not Search!");;
 
	$sql1=" SELECT id
			FROM tbl_student_subject_master 
			WHERE (subjcet_code = '' OR subjcet_code is NULL) 
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };  
		...
		if ($colname != ''and $colname != 'Select')
				{
			$query1=mysql_query("SELECT ss.id,ss.Stud_Member_Id,ss.ExtSemesterId,ss.subjcet_code,ss.subjectName,ss.student_id ,
									(select std_complete_name from tbl_student where school_id = '$school_id' and AcademicYear='$Academic_Year' and std_PRN = ss.student_id ) as std_name,
									(select std_email from tbl_student where school_id = '$school_id' and AcademicYear='$Academic_Year' and std_PRN = ss.student_id ) as std_email
										...
										WHERE (ss.subjcet_code = '' OR ss.subjcet_code is NULL)         
											AND ss.school_id = '$school_id' and ss.AcademicYear='$Academic_Year'
									AND ss.$colname LIKE '%$searchq%' LIMIT $start_from, $limit") or die("could not Search!");

				$sql1 =mysql_query("SELECT id,Stud_Member_Id,ExtSemesterId,subjectName,student_id,subjcet_code 
				                	FROM tbl_student_subject_master 
									WHERE (subjcet_code = '' OR subjcet_code is NULL)
										AND school_id = '$school_id' and AcademicYear='$Academic_Year'
				...
				$query1=mysql_query("SELECT ss.id,ss.Stud_Member_Id,ss.ExtSemesterId,ss.subjcet_code,ss.subjectName,ss.student_id ,
										...
										(select std_email from tbl_student where school_id = '$school_id' and AcademicYear='$Academic_Year' and std_PRN = ss.student_id ) as std_email
											...
											WHERE (ss.subjcet_code = '' OR ss.subjcet_code is NULL)         
												AND ss.school_id = '$school_id' and ss.AcademicYear='$Academic_Year'
										AND (ss.Stud_Member_Id LIKE '$searchq%' or ss.ExtSemesterId LIKE '$searchq%' or ss.subjcet_code LIKE '$searchq%' or ss.subjectName LIKE '$searchq%'  or ss.student_id LIKE '$searchq%' ) LIMIT $start_from, $limit") or die("could not Search!");
			
				$sql1 =mysql_query("SELECT id,Stud_Member_Id,ExtSemesterId,subjectName,student_id,subjcet_code 
									FROM tbl_student_subject_master 
									WHERE (subjcet_code = '' OR subjcet_code is NULL)
										AND school_id = '$school_id' and AcademicYear='$Academic_Year'
										AND (Stud_Member_Id LIKE '$searchq%' or ExtSemesterId LIKE '$searchq%' or subjcet_code LIKE '$searchq%' or subjectName LIKE '$searchq%'  or student_id LIKE '$searchq%' ) ") or die("could not Search!");   
  	...
  	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('stud_subj_without_stud_code.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('stud_subj_without_stud_code.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
	});
	});
</script>
<?php }?>
...
<body>
	...
	<div class="container">
		<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white">Student Subject Data without Subject Code (<?php echo $total_records; ?>)</h2>
		</div>
...
<!--End of dropdown ----------------------- --- -->
	...
	<!-- search form -->
		...
		<form>
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('stud_subj_without_stud_code.php','_self')" />
			</div>
						...
						<tbody>
							...
							?>
								<tr>
									...
									<td><?php echo $row['std_name']; ?> <br> <?php echo $row['std_email']; ?> </td>
									<td><?php echo $row['subjcet_code']; ?></td>
									<td> <center>
                     						<a style="cursor: pointer;" href="edit_student_subject_data_quality.php?id=<?php echo $row['id']; ?>&FLAG=SubjectCode" ><span class="glyphicon glyphicon-pencil"></span></a>
										</center>
									</td>
								</tr>
							<?php $i++;} ?>     
