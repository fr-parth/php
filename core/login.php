...
if (xss_clean(mysql_real_escape_string(isset($_POST['submit'])))) {
    ...
    $memberId = substr($inputMemberId, 0);
 ...
 //print_r($_POST); exit;
    ...
    if ($entity != 0 and $Password != "" and (($EmailID != "" and $entity != 2) or ($EmailID != "" and $entity = 2 and $school_id != '') or ($CountryCode != "" and $PhoneNumber != "") or ($OrganizationID != "" and $EmployeeID != "") or $memberId != "" )) {
        if ($EmailID != "" && $LoginOption == 'EmailID') {
...
//Get the operating_system
 ...
 //condition for entity added by Sayali for SMC-4944 on 10/11/2020
                    ...
                    if($entity==1)
                    {
                            ...
                            {
                                $querycheck=mysql_query("select email,password from tbl_school_admin where email='$EmailID' AND school_id='$school_id'");
                            ...
                            else if($LoginOption=='PhoneNumber')
                            {
                                ...
                                $qrCount=mysql_num_rows($querycheck);
            //Checked country code for respective phone number is correct or not by Pranali for SMC-5062
                                $querycheck1=mysql_query("select mobile from tbl_school_admin where mobile='$PhoneNumber' and CountryCode='$CountryCode'");
                                $qrCount1=mysql_num_rows($querycheck1);
                                if($qrCount <= 0)
                                {
                    ...
                    if($entity==5)
                    {
                            if($LoginOption=='EmailID')
                            {
                                $querycheck=mysql_query("select email_id,Password from tbl_parent where email_id='$EmailID' AND school_id='$school_id'");
                                $qrCount=mysql_num_rows($querycheck);
                                echo $qrCount;
                                if($qrCount <= 0)
                                {
                            ...
                            {
                                $querycheck=mysql_query("select Phone from tbl_parent where Phone='$PhoneNumber'");
                                $qrCount=mysql_num_rows($querycheck);
            //Checked country code for respective phone number is correct or not by Pranali for SMC-5062
                                $querycheck1=mysql_query("select Phone from tbl_parent where Phone='$PhoneNumber' and CountryCode='$CountryCode'");
                                $qrCount1=mysql_num_rows($querycheck1);
                                if($qrCount <= 0)
                                {
                                $errorMsg='Phone Number';
                            ...
                            {
                                $querycheck=mysql_query("select Id from tbl_parent where Id='$memberId'");
                                $qrCount=mysql_num_rows($querycheck);
                                if($qrCount <= 0)
                                {
                                $errorMsg='Member ID ';
                            ...
                            if($school_id !='')
                            {
                                $querycheck=mysql_query("select school_id from tbl_parent where school_id='$school_id'");
                                $qrCount=mysql_num_rows($querycheck);
                                if($qrCount <= 0)
                                {
                                $errorMsg='School ID ';
                    ...
                    else if($entity==7)
                    {
                            ...
                            else if($LoginOption=='PhoneNumber')
                            {
                                ...
                                $qrCount=mysql_num_rows($querycheck);
                                $querycheck1=mysql_query("select phone from tbl_school_adminstaff where phone='$PhoneNumber' and CountryCode='$CountryCode'");
                                $qrCount1=mysql_num_rows($querycheck1);
                                if($qrCount <= 0)
                                {
                                $errorMsg='Phone Number';
                    ...
                    if($errorMsg !='') //   8 cookie 7 school 71 hr 
                    {
                $report = "<span id='error' class='red'>This $errorMsg is not registered in system ! !</span>";
                    }else {
                         $report = "<span id='error' class='red'>Invalid  Passwords !</span>";
                    }
            ...
            <div class='panel panel-primary'>
                <div class='panel-body'>
                    ...
                    <div class='row bg-red text-center title-text'>
                        <span class='panel-title color-white'><?php echo xecho($user); ?> Login</span>
                    </div>
                            ...
                            <div class='col-md-12'>
                                ...
                                <div class='form-group' id='PasswordInput'>
                                    <input type='password' name='Password' id='Password' class='form-control'
                                           value='<?php if(isset($_COOKIE["pass_login"])) { echo $_COOKIE["pass_login"]; }else{ echo xecho($Password); } ?>' placeholder='Password'/><i class="fas fa-eye-slash" onclick="myFunction()"></i>
                                </div>
