...
if(isset($_POST['submit']))
{
	  ...
	  $count=$_POST['count'];
				...
				for($i=0;$i<$count;$i++)
				          {
							  $activity=$_POST[$i];
							  $results=mysql_query("select * from tbl_thanqyoupointslist where school_id='0' and group_member_id='0' and t_list like '$activity'  ");
							  //check already class exist or not
								 if(mysql_num_rows($results)==0 && $activity!="")
									{
										$query="insert into tbl_thanqyoupointslist(t_list,school_id,group_member_id) values('$activity','0','0') ";
