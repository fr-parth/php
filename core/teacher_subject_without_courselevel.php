...
if (!($_GET['Search']))  
{
	...
	$query=mysql_query("SELECT  ts.ExtSchoolSubjectId, ts.subjectName, ts.teacher_id, t.t_complete_name , ts.CourseLevel
                        FROM tbl_teacher t 
	...
	if (isset($_GET["spage"])){ $spage  = mysql_real_escape_string($_GET["spage"]); } else { $spage=1; };   
		...
		if ($colname != ''and $colname != 'Select')
				{
			$query1=mysql_query(" SELECT  ts.ExtSchoolSubjectId, ts.subjectName, ts.teacher_id, t.t_complete_name , ts.CourseLevel
                                    FROM tbl_teacher t 
                                        JOIN tbl_teacher_subject_master ts ON ts.school_id =t.school_id AND ts.teacher_id=t.t_id 
                                    WHERE (ts.CourseLevel ='' OR ts.CourseLevel is null ) 
                                        AND ts.school_id = '$school_id'  and ts.AcademicYear='$Academic_Year'   
			...
			WHERE (ts.CourseLevel ='' OR ts.CourseLevel is null ) 
				...
				$query1=mysql_query("SELECT  ts.ExtSchoolSubjectId, ts.subjectName, ts.teacher_id, t.t_complete_name , ts.CourseLevel
                                        ...
                                        WHERE (ts.CourseLevel ='' OR ts.CourseLevel is null ) 
                                            AND ts.school_id = '$school_id'  and ts.AcademicYear='$Academic_Year' 
									AND (ts.CourseLevel LIKE '$searchq%' or ts.ExtSchoolSubjectId LIKE '$searchq%' or ts.subjectName  LIKE '%$searchq%' or ts.teacher_id  LIKE '%$searchq%' or t.t_complete_name  LIKE '%$searchq%') LIMIT $start_from, $limit") or die("could not Search! 4");
			...
			WHERE (ts.CourseLevel ='' OR ts.CourseLevel is null ) 
				AND ts.school_id = '$school_id' and ts.AcademicYear='$Academic_Year' 
									AND (ts.CourseLevel LIKE '$searchq%' or ts.ExtSchoolSubjectId LIKE '$searchq%' or ts.subjectName LIKE '%$searchq%' or ts.teacher_id  LIKE '%$searchq%' or t.t_complete_name  LIKE '%$searchq%' ) LIMIT $start_from, $limit") or die("could not Search! 5");   
	...
	<?php if (!($_GET['Search'])){ ?>
		<script type="text/javascript">
    		$(function () {
				...
				window.location.assign('teacher_subject_without_courselevel.php?page='+page);
    });
  });
  </script>
	<?php }else{
		...
		<script type="text/javascript">
    		$(function () {
            	...
            	console.info(page + '(from event listening)');
				window.location.assign('teacher_subject_without_courselevel.php?colname=<?php echo $colname;?>&Search=<?php echo $searchq; ?>&spage='+page);
    		});
		});
   </script>
<?php }?>
...
<!-- Headline -->
	<div class="container">
			<div style="padding-top:30px" align="center" >
        		<h2 style="padding-left:20px;padding-top:10px;padding-bottom:10px;background-color:#428BCA; margin-top:2px;color:#666;color:white"><?php echo $dynamic_teacher;?> <?php echo $dynamic_subject;?> Data without Course Level (<?php echo $total_records; ?>)</h2>
			</div>
...
<!--End of dropdown ----------------------- --- -->
		...
		<form>
			...
			<div class="col-md-1">
			<input type="button" class="btn btn-danger" value="Reset" onclick="window.open('teacher_subject_without_courselevel.php','_self')" />
			</div>	
        </form>
		<!-- search form END -->	
			...
			<table id="example" class="display" width="100%" cellspacing="0">
					...
					<tbody>
						...
						<td data-title="id"><center><?php echo $i; ?></center></td>
                            ...
                            <td data-title="Department_id" ><?php echo $row['CourseLevel']; ?></td>
							<td> <center>
