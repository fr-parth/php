...
if (!($_GET['Search'])) {
	...
	$sql = mysql_query("SELECT tvc.coupon_id, tvc.timestamp,tvc.code, tvc.user_id,t.t_complete_name,t.school_id  
    FROM tbl_selected_vendor_coupons tvc 
    INNER JOIN tbl_teacher t ON tvc.user_id=t.id and t.school_id=tvc.school_id where  (tvc.timestamp BETWEEN '$from' AND '$to') and tvc.entity_id='2' and  t.school_id='$school_id'     
   LIMIT $start_from, $webpagelimit");

	$sql1 = "SELECT count(t.t_complete_name),tvc.coupon_id,tvc.timestamp, tvc.code, tvc.user_id,t.school_id  
    FROM tbl_selected_vendor_coupons tvc 
    INNER JOIN tbl_teacher t ON tvc.user_id=t.id and t.school_id=tvc.school_id where (tvc.timestamp BETWEEN '$from' AND '$to') and tvc.entity_id='2' and t.school_id='$school_id'";
...
} else 
{
    ...
    if($searchq !=''){
	//$dynamic_school_admin added by Sayali Balkawade for SMC-4254 on 11/12/2019
	$query1=mysql_query("SELECT t.t_complete_name,p.date,p.user_id,p.school_id ,s.rewardType FROM purcheseSoftreward 
        as p 
        inner join softreward as s on p.reward_id=s.softrewardId 
         inner join tbl_teacher t on t.id=p.user_id and p.school_id=t.school_id
        where (p.date BETWEEN '$from' AND '$to') and (userType='Teacher')  and  (t.school_id='$school_id') 
         ...
         and
                    ...
                    $sql1 ="SELECT count(t.t_complete_name),p.date,p.user_id,p.school_id ,s.rewardType FROM purcheseSoftreward 
                    as p 
                    inner join softreward as s on p.reward_id=s.softrewardId 
                     inner join tbl_teacher t on t.id=p.user_id and p.school_id=t.school_id
                    where (p.date BETWEEN '$from' AND '$to') and (userType='Teacher')  and  (t.school_id='$school_id')
...
$total_pages = ceil($total_records / $webpagelimit);
    ...
    $query1=mysql_query("SELECT t.t_complete_name,p.user_id,p.date,p.school_id ,s.rewardType FROM purcheseSoftreward 
    as p 
    inner join softreward as s on p.reward_id=s.softrewardId 
     inner join tbl_teacher t on t.id=p.user_id and p.school_id=t.school_id
    where (p.date BETWEEN '$from' AND '$to') and (userType='Teacher') and  (t.school_id='$school_id')
	...
	$sql1 =mysql_query("SELECT count(t.t_complete_name),p.user_id,p.date,p.school_id ,s.rewardType FROM purcheseSoftreward 
as p 
inner join softreward as s on p.reward_id=s.softrewardId 
		...
		<script type="text/javascript">
			$(function() {
				...
				window.pagObj = $('#pagination').twbsPagination({
					...
					window.location.assign('vendor_coupon_purchase_teacher.php?from=<?php echo $from; ?>&to=<?php echo $to; ?>&page='+page);
				});
			});
		</script>
	<?php } else {
		...
		<script type="text/javascript">
			$(function() {
				...
				window.pagObj = $('#pagination').twbsPagination({
					...
					console.info(page + '(from event listening)');
					window.location.assign('vendor_coupon_purchase_teacher.php?Search=<?php echo $searchq; ?>&from=<?php echo $from; ?>&to=<?php echo $to; ?>&spage='+page);
				});
			});
		</script>
	<?php } ?>
	...
	<div align="center">
		<div class="container" style="width:100%;">
			<div style="padding-top:30px;">
				...
				<!-- $dynamic_school_admin added by Sayali Balkawade for SMC-4254 on 11/12/2019 -->
				<h2 style="padding-left:20px; margin-top:2px;color:#666">Vendor Coupons Used By <?php echo $dynamic_teacher ?></h2>
			...
			<input type="button" class="btn btn-info" value="Reset" onclick="window.open('green_point_distributed_by_cookieadmin.php','_self')" />
					 ...
					 <div style="margin-left: 800px;">
						...
						<input type="button" value="Reset" onclick="window.open('vendor_coupon_purchase_teacher.php','_self')" />
					</div> -->
				</form>
			</div>
