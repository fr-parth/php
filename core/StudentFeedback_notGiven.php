...
<head>
    ...
    <title>Students-Feedback Not Given</title>
...
if(($yr!='' and $yr!='ALL') and ($dpt!='' and $dpt!='ALL')){
   ...
   FROM tbl_student st
    where st.school_id='$school_id' and st.Academic_Year='$yr' and st.std_dept='$dpt' and st.std_PRN NOT IN
    (
...
elseif(($yr!='ALL' and $yr!='') and ($dpt=='' or $dpt=='ALL')){
  $sql3=mysql_query(" SELECT distinct st.std_PRN,st.std_complete_name,st.Academic_Year,
   st.std_branch,st.std_dept,st.std_class,st.std_div
  FROM tbl_student st
   where st.school_id='$school_id' and st.Academic_Year='$yr' and st.std_PRN NOT IN
   (
  select sf.stu_feed_student_ID
  from tbl_student_feedback sf where sf.stu_feed_school_id='$school_id' and sf.stu_feed_student_ID is not null )
...
elseif(($yr=='ALL' or $yr=='') and ($dpt!='' and $dpt!='ALL')){
   ...
   st.std_branch,st.std_dept,st.std_class,st.std_div
  FROM tbl_student st
   where st.school_id='$school_id' and st.std_dept='$dpt' and st.std_PRN NOT IN
   (
  select sf.stu_feed_student_ID
  from tbl_student_feedback sf where sf.stu_feed_school_id='$school_id' and sf.stu_feed_student_ID is not null)
  ...
  $sql3=mysql_query(" SELECT distinct st.std_PRN,st.std_complete_name,st.Academic_Year,
   st.std_branch,st.std_dept,st.std_class,st.std_div
  FROM tbl_student st
   where st.school_id='$school_id' and st.std_PRN NOT IN
   (
  select sf.stu_feed_student_ID
  from tbl_student_feedback sf where sf.stu_feed_school_id='$school_id' and sf.stu_feed_student_ID is not null )
...
$yr=$_GET['yr']; 
 $dpt=$_GET['dpt']; 
  ...
  if((isset($_GET['yr'])) or (isset($_GET['dpt']))){
  if(($yr!='' and $yr!='ALL') and ($dpt!='' and $dpt!='ALL')){
    $query3="SELECT distinct st.std_PRN,st.std_complete_name,st.Academic_Year,
    st.std_branch,st.std_dept,st.std_class,st.std_div
    FROM tbl_student st
    where st.school_id='$school_id' and st.Academic_Year='$yr' and st.std_dept='$dpt' and st.std_PRN NOT IN
    (
    select sf.stu_feed_student_ID
    from tbl_student_feedback sf where sf.stu_feed_school_id='$school_id' )
...
$query1=mysql_query($query3) or die("could not Search!");
 }
...
<?php if (!($_GET['Search'])){?>
    ...
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.log(page);
        			window.location.assign('StudentFeedback_notGiven.php?yr=<?php echo $yr."&dpt=".$dpt;?>&page='+page);
    ...
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            <?php if($_GET['yr'] or $_GET['dpt']){ ?>
      			window.location.assign('StudentFeedback_notGiven.php?yr=<?php echo $yr."&dpt=".$dpt; ?>&Search=<?php echo $searchq; ?>&spage='+page);
                <?php } else{ ?>
            window.location.assign('StudentFeedback_notGiven.php?Search=<?php echo $searchq; ?>&spage='+page);
                <?php } ?>
        });
    });
</script>
...
<div align="center">
    <div class="container" style="width:100%;">
       ...
       <input type='hidden' name='dpt' value='<?php echo $_GET['dpt']?>'> </input>
			...
			<div class="col-md-1" >
			<input type="button" class="btn btn-info" value="Reset" onclick="window.open('StudentFeedback_notGiven.php?yr=<?php echo $yr; ?>&dpt=<?php echo $dpt; ?>','_self')" />
			</div>
  ...
  if((isset($_GET['yr'])) or (isset($_GET['dpt']))){
  if(($yr!='' and $yr!='ALL') and ($dpt!='' and $dpt!='ALL')){
    $query2="SELECT distinct st.std_PRN,st.std_complete_name,st.Academic_Year,
    st.std_branch,st.std_dept,st.std_class,st.std_div
    FROM tbl_student st
    where st.school_id='$school_id' and st.Academic_Year='$yr' and st.std_dept='$dpt' and st.std_PRN NOT IN
    (
...
$query=mysql_query($query2) or die("could not Search!");
 }
elseif (($yr!='' and $yr!='ALL') and ($dpt=='' or $dpt=='ALL')) {
  $query2="SELECT distinct st.std_PRN,st.std_complete_name,st.Academic_Year,
  st.std_branch,st.std_dept,st.std_class,st.std_div
