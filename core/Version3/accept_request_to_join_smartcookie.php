...
{ 
		...
		if($receiver_entity_id ==103)
		{
			...
			$user_id = "t_id";
			$point_type='Brown';
		echo '<center><div><h1 class="textstyle" style="display:none">Request Accepted</h1><hr style="border : 1px solid red;width:300px"/><div id="div"></div></div></center>';
...
//change $request_status value by matching database	by sachin	
...
$points = (integer)$points_query_result['points'];
		...
		if($insert)
		{
			...
			$point_insert_query = mysql_query("INSERT INTO $tbl_point ($point_insert_column) VALUES ('$id_prn','$sender_entity_id','','$request_status','$points','$issue_date','$school_id','$point_type','$referral_id')");
				...
				$url=SEND_SMS_PATH."?user=$dynamic_user&password=$dynamic_pass&sender=$dynamic_sender&PhoneNumber=$user_mobile_number&Text=$Text&msgType=$msgType&Pe_id=$pe_id&template_id=$template_id";
