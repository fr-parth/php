...
//validation of input
  ...
  function makeRegistration($collected)
  {
		...
		else if($key==2)
		{
			...
			*/	
				// Start SMC-3453 Modify By yogesh 2018-09-25 07:04 PM 
				//$date = date('d-m-Y')." ".$indian_time;
				$date = CURRENT_TIMESTAMP; // define in core/securityfunctions.php
				//Start SMC-3450
				
				$your_date =date('Y-m-d H:i:s', strtotime("+10 days"));
				$datediff = $now - $your_date;
				$date_diff_days=abs(floor($datediff/(60*60*24)));
				$days=$date_diff_days+1;
  ...
  function sendSMSMessgae($countrycode,$phonenumber,$password,$user)
  {
	               ...
	               if(preg_match($mob, $phonenumber))
				   {
						...
						$dynamic_sender = $dynamic_fetch['otp'];
											 ...
											 //echo "MSG sent";
											$Text="CONGRATULATIONS%21,+You+are+now+a+registered+User+of+".$user."+-+Program.+Your+Username+is+".$phonenumber."+and+Password+is+".$password."."; //Added SEND_SMS_PATH and &msgType=$msgType&Pe_id=$pe_id&template_id=$template_id by Pranali for SMC-5166
