...
if ($obj->{'User_Name'} != '' && $obj->{'User_email'} != '' && $obj->{'User_pass'} != '' && $obj->{'sp_phone'} != '' && $obj->{'Lattitude'} && $obj->{'Longitude'} != '' && $obj->{'country_code'} != '') {
    ...
    $test = mysql_query("INSERT INTO `tbl_sponsorer` (sp_company,sp_email,v_status,v_category, sp_password,sp_address,sp_phone,sp_city,sp_state,sp_country,sales_person_id,lat,lon,sp_date,expiry_date,amount,pin) VALUES('$sponsor_name','" . $obj->{'"sp_status" '} . "','" . $obj->{'"sp_product_category" '} . "', '" . $obj->{'User_email'} . "','" . $obj->{'User_pass'} . "','" . $obj->{'sp_address'} . "','" . $obj->{'sp_phone'} . "','" . $obj->{'city'} . "','" . $obj->{'state'} . "','" . $obj->{'country'} . "','" . $obj->{'sales_person_id'} . "','$lat','$lon','$dates','$date1','$amount','$pin')");
...
//	function emailUser($sp_email,$pass){
							...
							//$html_message = 'jhtruit';
							//$mail =mail($to, $subject, $html_message);
						//	$mail = mail('nikitas@roseland.com','smartcookie','yrtgfuyrg');
						//	if($mail){
						//		return 'abc';
						//	}else{
					//			return '123';
