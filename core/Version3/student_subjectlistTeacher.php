...
//sm.teacher_ID!='' / t.t_id!='' condition added by Pranali for SMC-3757 on 14-5-19	 	
		...
		*/
		...
		WHERE sm.student_id = '$std_PRN' and sm.school_id='$school_id' AND sm.teacher_ID!='' group by sm.subjcet_code order by sm.id desc LIMIT $limit OFFSET $offset");
			...
			{	
			$arr= mysql_query("select distinct t.t_id as teacher_id, t.t_complete_name, t.t_pc,a.Year as AcademicYear from tbl_teacher t left join tbl_student s on t.school_id=s.school_id  join tbl_academic_Year a ON a.school_id = t.school_id  where s.std_PRN='$std_PRN' and t.school_id='$school_id' and s.school_id='$school_id' and a.Enable = '1' AND t.t_id!=''");
	...
	$arr = mysql_query($quer1);
  			...
  			else if($count > 0) 
			{
    			...
    			while($post = mysql_fetch_assoc($arr)) {
					...
					$posts[] = array(
					'subjcetId'=>$subjcet_id, // End SMC-3661 
