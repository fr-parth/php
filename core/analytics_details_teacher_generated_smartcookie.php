...
$sql=mysql_query(" SELECT t.school_id,t.t_complete_name,t.id,tc.issue_date,tc.status,tc.user_id
FROM tbl_teacher t
...
$sql1 =" SELECT  count(t.t_complete_name),t.school_id,t.id,tc.issue_date,tc.status,tc.user_id
FROM tbl_teacher t
INNER JOIN tbl_teacher_coupon tc ON t.id=tc.user_id
where status ='unused' order by t.id desc";  
...
if($searchq !='')
	...
	{
		$query1=mysql_query("SELECT t.t_complete_name,t.school_id,t.id,tc.issue_date,tc.status,tc.user_id
FROM tbl_teacher t
INNER JOIN tbl_teacher_coupon tc ON t.id=tc.user_id
where (status ='unused') 
 and
 
            (t_complete_name LIKE '%$searchq%' or 
			t.school_id LIKE '%$searchq%'  or
			tc.user_id LIKE '%$searchq%' or
			tc.issue_date LIKE '%$searchq%')
...
where (status ='unused') 
 and 
     ($colname LIKE '%$searchq%') order by t.id desc LIMIT $start_from, $webpagelimit") 
		or die("could not Search!");
					//echo $query1;
...
where (status ='unused') 
 and 
     ($colname LIKE '%$searchq%') order by t.id desc "; 
					$rs_result = mysql_query($sql1);  
					$row1 = mysql_fetch_row($rs_result);  
					$total_records = $row1[0];  
					$total_pages = ceil($total_records / $webpagelimit);
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
			...
			window.location.assign('analytics_details_teacher_generated_smartcookie.php?page='+page);
        });
    });
</script>
<?php }else{
...
<script type="text/javascript">
    $(function () {
        ...
        window.pagObj = $('#pagination').twbsPagination({
            ...
            console.info(page + '(from event listening)');
			window.location.assign('analytics_details_teacher_generated_smartcookie.php?Search=<?php echo $searchq; ?>&spage='+page);
        });
    });
</script>
...
<div align="center">
    <div class="container" style="width:100%;">
        <div style="padding-top:30px;">
            ...
            <h2 style="padding-left:20px; margin-top:2px;color:#666">SmartCookie Coupons Generated By Teachers</h2>
		...
		<div class="col-md-2" style="width:17%;">
			...
			<div class="col-md-1" >
			<input type="button" class="btn btn-info" value="Reset" onclick="window.open('analytics_details_teacher_generated_smartcookie.php','_self')" />
			</div>
            ...
            <table id="example" class="col-md-12 table-bordered " align="center">
                ...
                while($result = mysql_fetch_array($query1)) {
                    ...
                    <tr>
						...
						<td data-title="Coupon Generate Date"><?php echo $result['issue_date']; ?></td>
