...
class Csalesperson extends CI_Controller {	
	...
	{		
		...
		//regex for Sponsor Name & Company Name added by Pranali for bug SMC-2262 
		
		$this->form_validation->set_rules('sponsor_name', 'Sponsor Name', 'required|trim|max_length[50]|regex_match[/^([A-z][A-Za-z]*\s*[A-Za-z]*)$/]');			
		$this->form_validation->set_rules('sp_company', 'Company Name', 'required|trim|max_length[50]|regex_match[/^[a-zA-Z\s-0-9]+$/]');	

		// Changes by swapnali patil
		$this->form_validation->set_rules('sp_phone', 'Mobile', 'required|trim|is_numeric|max_length[10]');						
		$this->form_validation->set_rules('sp_pin', 'Pin', 'required|trim|is_numeric|max_length[6]');
		
		$this->form_validation->set_rules('sp_landline', 'Landline', 'trim|is_numeric|max_length[20]');	
		
		//$this->form_validation->set_rules('sp_email', 'Email', 'trim|required|valid_email|max_length[60]|is_unique[tbl_sponsorer.sp_email]');	

		$this->form_validation->set_rules('sp_email', 'Email', 'trim|required|valid_email|max_length[60]');	
		...
		$this->form_validation->set_rules('discount', 'Discount', 'required|is_natural|trim|max_length[3]|greater_than[0]');
...
//validations for Revenue Percent & Revenue Per Visit added by Pranali for SMC-3678
		$this->form_validation->set_rules('revenue_percent', 'Revenue', 'required|is_natural|trim|max_length[3]|greater_than[0]');
		$this->form_validation->set_rules('revenue_per_visit', 'Revenue Per Visit', 'is_natural|trim');
		...
		{	 	
			$latlon=$this->calLatLongByAddress($this->input->post('sp_country'), 
										...
										$calldate=$this->input->post('sp_callback_date');
										$calltime=$this->input->post('sp_callback_time');
										$callbackdatetime = $calldate.' '.$calltime; 
...
//current_marketing,current_market_val, discount,discount_val, digital_marketing,digital_market_val passed to $form_data array by Pranali for SMC-3678 
...
//shop level/minimum discount entered into tbl_sponsorer by Pranali
			...
			$form_data = array(
							...
							'calback_date_time' => $callbackdatetime,
							'current_market_val' => $curr_market,
							'digital_market_val' => $digital_market,
							'discount_val' => $dis_coupons,
							'current_marketing' => $curr_marketing,
							'digital_marketing' => $dig_marketing,
							'discount' => $discount_coupons1,
							'minimum_discount'=>$idiscount,
							'points_for_minimum_discount'=>$pointsd
			...
			if($this->input->post('editID')!='' and $this->input->post('editID')!=0){
				...
				if($this->input->post('revenue_percent')==''){
					$revenue_percent=0;
				}else{
					$revenue_percent=$this->input->post('revenue_percent');
				}
				
				if($this->input->post('revenue_per_visit')==''){
					$revenue_per_visit=0;
				}else{
					$revenue_per_visit=$this->input->post('revenue_per_visit');
				}
				if($this->input->post('sp_country')=='India'){
					
					$currency=4;
				}
//revenue_percent & revenue_per_visit added by Pranali for SMC-3678 on 10-12-18
				if($ins!= FALSE && $idiscount!=0 && $revenue_percent!=0 && $revenue_per_visit!=0){
					$this->sponsor->add_product($ins, 'discount', $idiscount, $idiscount, $pointsd, '',$currency,'', $revenue_percent, $revenue_per_visit);
				}
...
//EditSponsorProfile() added by Pranali for SMC-3697 to Edit Sponsor's Profile	
	public function EditSponsorProfile($id)
	{
		$this->load->helper('url');
		$data = $this->headerData($this->id);
		$data['id']=$id;
		$data['salesperson'] = $this->Msalesperson->headerData($this->id);
		
	    $personName=$data['salesperson'][0]->p_name;
		$this->load->model('sp/sponsor');
		$this->load->model('slp/Msalesperson');
			
		
		$data['sponsor_details']=$this->Msalesperson->getSponsorDetails($id);
			
		
		$this->form_validation->set_rules('sponsor_name', 'Sponsor Name', 'required|trim|max_length[50]|regex_match[/^([A-z][A-Za-z]*\s*[A-Za-z]*)$/]');			
		$this->form_validation->set_rules('sp_company', 'Company Name', 'required|trim|max_length[50]|regex_match[/^[a-zA-Z\s-0-9]+$/]');	
		
		// $this->form_validation->set_rules('sp_phone', 'Mobile', 'required|trim|is_numeric|regex_match[/^[789]{1}[0-9]{9}$/]|max_length[10]|is_unique[tbl_sponsorer.sp_phone]');		
		...
		$this->form_validation->set_rules('oldsp_password', 'Old Password', 'required|trim|max_length[50]');
		
		$this->form_validation->set_rules('sp_password', 'New Password', 'required|trim|max_length[50]');
		...
		$new_password=$this->input->post('sp_password');
		$confirm_password=$this->input->post('sp_password1');
		...
		if (($this->form_validation->run() == FALSE) or ($data['error']!='')) // validation hasn't been passed
		{	
			$this->load->view('slp/EditSponsorProfile',$data); 			
		}
		else 
		{	 	
			...
			// run insert model to write data to db
				...
				$edit=$this->Msalesperson->EditSponsorProfile($form_data,$id);
						
			if ($edit != '') // the information has therefore been successfully edited in the db
			{ 
						...
						//mail
								...
								$this->email->initialize($config);		
									...
									->to($this->input->post('sp_email'))
									->cc('vivekj@blueplanetinfosolutions.com')
									->subject('SmartCookie Sponsor Updated Profile by Salesperson')
									->message("Dear Sponsor,<br/>".
									  "Your Profile is updated by Salesperson:".$personName."<br/>".
									  "Your New Credentials are as follows: "."<br/>".
									  "Your Username is: ".$this->input->post('sp_email')."<br/>".
									  "Password is: ".$this->input->post('sp_password')."<br/>Login into your Profile to view updated details <a href=".base_url()."Clogin/login/sponsor>".base_url()."Clogin/login/sponsor</a><br/>")
									->set_mailtype('html');	
			...
			echo "<script>alert('Sponsor Profile Updated Successfully');</script>";
			$this->page('RegisteredSponsorsList');
				
			}
			else
			{
				echo "<script>alert('Sponsor Profile Not Updated!!');</script>";
				$this->load->view('slp/EditSponsorProfile',$data);
			}
		}
